...
function renderDesignStudioPage(){
  ...
  renderDesignStudioLibrary();renderDesignStudioInspector();initDesignStudioCanvas();requestAnimationFrame(()=>fitDesignStudioCanvas());
}
...
function designStudioHandleDefinitions(){return[{key:'nw',hx:-1,hy:-1,cursor:'nwse-resize'},{key:'n',hx:0,hy:-1,cursor:'ns-resize'},{key:'ne',hx:1,hy:-1,cursor:'nesw-resize'},{key:'e',hx:1,hy:0,cursor:'ew-resize'},{key:'se',hx:1,hy:1,cursor:'nwse-resize'},{key:'s',hx:0,hy:1,cursor:'ns-resize'},{key:'sw',hx:-1,hy:1,cursor:'nesw-resize'},{key:'w',hx:-1,hy:0,cursor:'ew-resize'}]}
...
function designStudioHitTest(point){
  ...
  const selected=designStudioSelected();if(selected){const rect=designStudioElementRect(selected,print),local=designStudioRotatedLocalPoint(point,rect,selected.rotation),handle=designStudioHandleDefinitions().find(candidate=>Math.hypot(local.x-candidate.hx*rect.w/2,local.y-candidate.hy*rect.h/2)<=24);if(handle)return{item:selected,rect,lx:local.x,ly:local.y,handle}}
...
function designStudioPointerDown(event){
  if(designStudioState.view!=='edit')return;event.preventDefault();const point=designStudioCanvasPoint(event),hit=designStudioHitTest(point);
...
function syncDesignStudioHistoryButtons(){const undo=document.querySelector('.design-studio-history button[data-studio-action="undo"]'),redo=document.querySelector('.design-studio-history button[data-studio-action="redo"]');if(undo)undo.disabled=!designStudioState.history.length;if(redo)redo.disabled=!designStudioState.future.length}
...
async function addDesignStudioToCart(goCheckout=false){
  ...
  try{
    ...
    saveCart();updateCartUI();showToast(I18n.msg`${isBag?I18n.t('Sac'):I18n.t('Toile')} personnalisé${isBag?'':'e'} ajouté${isBag?'':'e'} au panier`,'success');if(goCheckout)setTimeout(()=>goToCheckout(),250);
