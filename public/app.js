...
function viewProfilePrivateEvent(eventId){
  ...
  const paid=request.paymentStatus==='paid',address=request.address||{},status=paid?I18n.t('Paiement confirmé'):I18n.status(request.status||'nouvelle');
  ...
  detail.innerHTML=I18n.html`<div class="account-detail-head"><span>${safeText(request.reference||I18n.t('Événement privé'))}</span><h2 id="profileOrderTitle">${safeText(request.eventType||I18n.t('Événement privé'))}</h2><div><small>Demande créée le ${profileDate(request.createdAt)}</small><b class="account-status ${paid?'paid':'pending'}">${safeText(status)}</b></div></div>
    ...
    ${Number(request.quoteAmount)>0?`<div class="account-detail-totals"><div><span>${I18n.t('Sous-total')}</span><strong>${I18n.currency(toMoney(request.quoteSubtotal))}</strong></div><div><span>${I18n.t('Livraison / déplacement')}</span><strong>${I18n.currency(toMoney(request.quoteShipping))}</strong></div>${taxLines}<div class="total"><span>${I18n.t('Total')}</span><strong>${I18n.currency(toMoney(request.quoteAmount))}</strong></div></div>`:''}</section>
...
function renderProfileOrders(){
  ...
  const paidPrivate=profilePrivateEvents.filter(request=>request.paymentStatus==='paid');
  const total=profileOrders.reduce((sum,order)=>sum+Number(order.total||0),0)+paidPrivate.reduce((sum,request)=>sum+Number(request.quoteAmount||0),0);
...
function adminEventRequestStatusOptions(current){return [['nouvelle',I18n.t('Nouvelle')],['en étude',I18n.t('En étude')],['contactée',I18n.t('Contactée')],['devis préparé',I18n.t('Devis préparé')],['paiement prêt',I18n.t('Paiement prêt')],['payée',I18n.t('Payée')],['fermée',I18n.t('Fermée')]].map(([value,label])=>`<option value="${value}" ${current===value?'selected':''}>${label}</option>`).join('')}
function eventRequestPaymentLabel(request){return {not_created:I18n.t('Aucun lien'),ready:I18n.t('Lien prêt'),pending:I18n.t('Paiement commencé'),processing:I18n.t('En traitement'),paid:I18n.t('Payé'),failed:I18n.t('Échec'),cancelled:I18n.t('Annulé')}[request.quotePaymentStatus||'not_created']||I18n.t('À confirmer')}
