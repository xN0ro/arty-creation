...
function renderProfilePrivateEvents(){
  ...
  wrap.innerHTML=profilePrivateEvents.map(request=>{
    ...
    const items=(request.inventoryItems||[]).map(item=>`<li><span>${safeText(item.name)}</span><strong>× ${Number(item.quantity)||1}</strong></li>`).join('');
    const custom=request.customKit?`<li><span>${safeText(request.customKit.productLabel||I18n.t('Création personnalisée'))} ${safeText(request.customKit.sizeLabel||'')}</span><strong>× ${Number(request.customKit.quantity)||1}</strong></li>`:'';
    const taxLines=(request.quoteTaxLines||[]).map(line=>`<div><span>${safeText(line.label)} ${Number(line.rate)||0}%</span><strong>${I18n.currency(toMoney(line.amount))}</strong></div>`).join('');
    return I18n.html`<article class="account-private-event-card ${paid?'paid':''}">
      <div class="account-private-event-head"><div><span>${safeText(request.reference||'')}</span><h3>${safeText(request.eventType||I18n.t('Événement privé'))}</h3><p>${profileDate(request.createdAt)} · ${safeText(profilePrivateEventPathLabel(request.servicePath))}</p></div><b class="account-status ${paid?'paid':'pending'}">${safeText(status)}</b></div>
      <div class="account-private-event-facts"><div><small>Date</small><strong>${safeText(request.preferredDate||I18n.t('À confirmer'))}</strong></div><div><small>Participants</small><strong>${Number(request.guests)||1}</strong></div><div><small>Lieu</small><strong>${safeText(request.location||I18n.t('À confirmer'))}</strong></div></div>
      ${items||custom?`<ul class="account-private-event-items">${items}${custom}</ul>`:request.expertBrief?`<div class="account-private-event-brief">${safeText(request.expertBrief)}</div>`:''}
      ...
      ${Number(request.quoteAmount)>0?`<div class="account-private-event-pricing"><div><span>${I18n.t('Sous-total')}</span><strong>${I18n.currency(toMoney(request.quoteSubtotal))}</strong></div><div><span>${I18n.t('Livraison / déplacement')}</span><strong>${I18n.currency(toMoney(request.quoteShipping))}</strong></div>${taxLines}<div class="total"><span>${I18n.t('Total')}</span><strong>${I18n.currency(toMoney(request.quoteAmount))}</strong></div></div>`:''}
      ${paid&&request.paidAt?`<small class="account-private-event-paidat">${I18n.t('Payé le')} ${profileDate(request.paidAt,true)}</small>`:''}
    </article>`;
  }).join('');
}
