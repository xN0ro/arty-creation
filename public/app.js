...
function renderAdminEvents(){
  ...
  panel.innerHTML=`<div class="admin-events-heading"><div><span>Billetterie et événements</span><h3>Gestion des invités</h3><p>Gérez les événements, retrouvez chaque participant et confirmez les entrées à partir de la même section.</p></div><button class="btn btn-orange" type="button" onclick="document.getElementById('adminEventEditor').open=true;document.getElementById('adminEventEditor').scrollIntoView({behavior:'smooth'})">Créer un événement</button></div>
    ${!transactionalEmailConfigured?'<div class="admin-ticket-config"><strong>Courriels automatiques à configurer</strong><p>Ajoutez RESEND_API_KEY, EMAIL_FROM et ARTY_PUBLIC_URL dans Render pour activer les courriels de compte, commandes, suivi et billets.</p></div>':''}
...
function eventPathDescription(path){return {inventory:'Sélectionnez les modèles et les quantités désirées dans le catalogue actuel.',custom:'Créez librement votre toile ou votre sac dans le Studio ARTY, puis joignez le design à votre demande.',expert:'Expliquez votre idée et notre équipe vous aidera à concevoir la bonne expérience.'}[path]||''}
...
function eventQuoteSolutionSummary(){
  ...
  if(eventBuilderState.servicePath==='custom')return `<div><span>Produit</span><strong>${safeText(eventStudioDesign?.productLabel||'À créer dans le Studio')}</strong></div><div><span>Quantité</span><strong>${Number(eventBuilderState.customKit?.quantity)||eventBuilderState.guests}</strong></div><div><span>Design</span><strong>${eventStudioDesign?'Joint à la demande':eventStudioDraftState?.elements?.length?'Brouillon en cours':'À créer'}</strong></div>`;
...
function validateEventQuoteStep(step){
  ...
  if(step===3){
    ...
    if(eventBuilderState.servicePath==='custom'&&(!eventStudioDesign||!eventCustomTraceData))return showToast('Terminez votre création dans le Studio ARTY','error'),false;
...
function eventSolutionStepHTML(){
  const paths=[['inventory','Kits du catalogue','Choisissez parmi les modèles actuellement offerts et indiquez les quantités.'],['custom','Création personnalisée','Ouvrez le Studio ARTY pour composer une toile ou un sac avec vos images, textes et couleurs.'],['expert','Accompagnement expert','Travaillez avec notre équipe pour concevoir une proposition entièrement adaptée.']];
...
function eventCustomStepHTML(){
  ...
  if(!eventStudioDesign)return `<div class="event-builder-step"><div class="event-step-heading"><span>03</span><div><h3>Créez le design de votre événement</h3><p>Le Studio ARTY vous permet de choisir une toile ou un sac, puis d’ajouter et positionner vos images, textes et formes.</p></div></div><div class="event-studio-launch"><span class="event-studio-launch-icon">${eventExperienceIcon('custom')}</span><div><span>Studio relié à votre demande</span><h4>${draftCount?'Votre brouillon vous attend':'Commencez votre création personnalisée'}</h4><p>${draftCount?`${draftCount} élément${draftCount>1?'s':''} déjà ajouté${draftCount>1?'s':''}. Reprenez votre composition, puis confirmez-la pour revenir ici.`:'Votre événement et vos coordonnées resteront enregistrés pendant la création. Aucun article ne sera ajouté au panier.'}</p><ul><li>Toile ou sac en toile</li><li>Images, textes et formes</li><li>Aperçu peint et tracé de production</li></ul></div><button type="button" class="btn btn-teal" onclick="openEventDesignStudio()">${draftCount?'Continuer dans le Studio':'Ouvrir le Studio ARTY'}</button></div></div>`;
...
function renderEventQuotePaymentContent(){
  ...
  const custom=quote.customKit?`<div class="event-payment-item"><span><strong>${safeText(quote.customKit.productLabel||'Kit personnalisé')}</strong><small>${safeText([quote.customKit.sizeLabel,quote.customKit.orientation].filter(Boolean).join(' · ')||'Format à confirmer')} · ${Number(quote.customKit.quantity)||1} exemplaire(s)</small></span></div>`:'';
...
function openAdminEventRequest(id){
  ...
  const project=request.servicePath==='inventory'?`<div class="admin-event-request-products">${items||'<p>Aucun kit enregistré.</p>'}</div>`:request.servicePath==='custom'?`<div class="admin-event-custom-review">${custom.paintedPreview?`<div><span>Aperçu peint</span><img src="${safeAttr(custom.paintedPreview)}" alt="Aperçu peint de la création"></div>`:''}<div><span>Fichier couleur</span>${custom.sourceImage?`<img src="${safeAttr(custom.sourceImage)}" alt="Composition couleur du client">`:'<p>Non disponible</p>'}</div><div><span>Tracé de production</span>${custom.traceImage?`<img src="${safeAttr(custom.traceImage)}" alt="Aperçu du tracé">`:'<p>Non disponible</p>'}</div></div><div class="admin-event-custom-meta"><strong>${safeText(custom.productLabel||custom.sizeLabel||'Création personnalisée')}</strong><span>${safeText([custom.sizeLabel,custom.orientation].filter(Boolean).join(' · '))}</span><span>${Number(custom.quantity)||request.guests||1} kit(s)</span><span>${Number(custom.elementCount)||1} élément(s)</span><p>${safeText(custom.notes||'Aucune instruction supplémentaire.')}</p></div>`:`<div class="admin-event-expert-brief"><strong>Vision du client</strong><p>${safeText(request.expertBrief||request.message||'Aucune description fournie.')}</p></div>`;
...
function renderDesignStudioPage(){
  ...
  const eventMode=isEventDesignStudio(),primaryLabel=eventMode?'Continuer mon événement':'Ajouter au panier';
  container.innerHTML=`<div class="design-studio-app">
    <header class="design-studio-topbar">
      <div class="design-studio-brand"><button type="button" class="design-studio-exit" onclick="exitDesignStudio()" aria-label="${eventMode?'Retourner à la demande':'Quitter le studio'}">${designStudioIcon('back')}<span>${eventMode?'Retour':'Quitter'}</span></button><i aria-hidden="true"></i><img src="logoarty.png" alt="ARTY"><span><strong>${eventMode?'Studio pour votre événement':'Studio de création'}</strong><small>${eventMode?safeText(eventBuilderState.eventName||'Création liée au devis'):'Votre design, prêt à peindre'}</small></span></div>
      ...
      <div class="design-studio-order"><span><small id="designStudioProductTop">${safeText(designStudioProductLabel())}</small><strong id="designStudioPriceTop">${eventMode?'Devis personnalisé':`$${toMoney(designStudioPrice())}`}</strong></span><button type="button" id="designStudioCartButton" onclick="handleDesignStudioPrimaryAction()">${designStudioIcon(eventMode?'check':'cart')}<b>${primaryLabel}</b></button></div>
    </header>
...
function designStudioOrderCard(){const eventMode=isEventDesignStudio();return `<div class="studio-order-card ${eventMode?'event-mode':''}"><div><span>${eventMode?'Projet de l’événement':'Votre produit'}</span><strong>${safeText(designStudioProductLabel())}</strong></div><label>${eventMode?'Nombre de kits identiques':'Quantité'}<input type="number" min="1" max="${eventMode?'1000':'50'}" value="${designStudioState.quantity}" onchange="setDesignStudioQuantity(this.value)"></label><label>Note pour la production<textarea maxlength="400" placeholder="Une précision pour notre équipe..." oninput="designStudioState.notes=this.value">${safeText(designStudioState.notes||'')}</textarea></label><div class="studio-order-total"><span>${eventMode?'Tarification':'Prix unitaire'}</span><strong>${eventMode?'Incluse au devis':`$${toMoney(designStudioPrice())}`}</strong></div><button type="button" onclick="handleDesignStudioPrimaryAction()">${designStudioIcon(eventMode?'check':'cart')}${eventMode?'Continuer mon événement':'Ajouter au panier'}</button></div>`}
...
function renderDesignStudioScene(ctx,view,showSelection=false){
  ...
  if(designStudioState.product==='bag'){
    ...
    if(view==='edit'){ctx.font='700 15px Outfit, sans-serif';ctx.textAlign='left';ctx.textBaseline='bottom';ctx.fillStyle='#137d8a';ctx.fillText('ZONE IMPRIMABLE',print.x+4,print.y-11)}
  }
...
function refreshDesignStudioProductUI(){const product=document.getElementById('designStudioProductTop'),price=document.getElementById('designStudioPriceTop');if(product)product.textContent=designStudioProductLabel();if(price)price.textContent=isEventDesignStudio()?'Devis personnalisé':'$'+toMoney(designStudioPrice());renderDesignStudioLibrary();renderDesignStudioInspector();drawDesignStudio()}
...
async function completeEventStudioDesign(){
  ...
  if(!designStudioState.elements.length)return showToast('Ajoutez une image, un texte ou une forme à votre création','error');
  const button=document.getElementById('designStudioCartButton');if(button){button.disabled=true;button.querySelector('b').textContent='Préparation...'}setDesignStudioBusy(true,'Préparation de la création pour votre demande...');
  try{
    ...
    eventStudioDesign={studioVersion:1,productType:designStudioState.product,productLabel:designStudioProductLabel(),size:isBag?'standard':designStudioState.size,sizeLabel:isBag?'Format sac standard':sizeInfo.label,orientation:isBag?'verticale':designStudioState.orientation==='landscape'?'horizontale':'verticale',imageCount,textCount,elementCount:designStudioState.elements.length,paintedPreview,tracePreview,colorArtwork,elements:designStudioProductionElements()};
    ...
    eventStudioDraftState=cloneDesignStudioState(designStudioState);eventBuilderState.step=3;navigate('#/event-builder');setTimeout(()=>showToast('Création jointe à votre demande','success'),120);
  }catch(error){console.error(error);showToast('Impossible de préparer votre création','error')}finally{setDesignStudioBusy(false);if(button){button.disabled=false;button.querySelector('b').textContent='Continuer mon événement'}}
}
