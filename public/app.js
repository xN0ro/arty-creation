...
async function handleRoute(){
  ...
  if(h.startsWith('#/product/')){show('page-product');await renderProductPage(parseInt(h.split('/')[2]));window.scrollTo(0,0)}
  else if(h.startsWith('#/event/')){show('page-event');renderEventPage(parseInt(h.split('/')[2]));window.scrollTo(0,0)}
  else if(h.startsWith('#/ticket/')){show('page-ticket');await renderPublicTicket(decodeURIComponent(h.split('/').slice(2).join('/')));window.scrollTo(0,0)}
  else if(h==='#/profile'){if(!currentUser){navigate('#/');openModal('auth');return}show('page-profile');await renderProfilePage();window.scrollTo(0,0)}
  else if(h==='#/admin'){if(!currentUser||!['admin','staff'].includes(currentUser.role)){navigate('#/');showToast(I18n.t('Accès admin requis'),'error');return}show('page-admin');document.getElementById('mainFooter').style.display='none';await loadAdminData();window.scrollTo(0,0)}
  ...
  else if(h.startsWith('#/studio')){prepareDesignStudioRoute(h);show('page-design-studio');document.getElementById('mainFooter').style.display='none';document.body.classList.add('design-studio-open');renderDesignStudioPage();window.scrollTo(0,0)}
  ...
  else if(h.startsWith('#/event-quote/')){show('page-event-quote');await renderEventQuotePaymentPage(decodeURIComponent(h.split('/')[2]?.split('?')[0]||''));window.scrollTo(0,0)}
...
function renderSiteAnnouncement(){
  ...
  message.textContent=visible?I18n.field(siteAnnouncement,'message').trim():'';
  bar.hidden=!visible;
...
function renderHomePopularKits(){
  ...
  document.getElementById('homePopularKits').innerHTML = kits.map(k=>{
    ...
    return `<div class="kit-card" onclick="navigate('#/product/${k.id}')">
      <div class="kit-card-img"><img src="${k.image}" alt="${safeText(I18n.field(k,'name'))}" loading="lazy">${k.featured?I18n.html('<span class="kit-card-badge">Populaire</span>'):''}</div>
      <div class="kit-card-body"><div class="kit-card-category">${cat?cat.name:''}</div><h3 class="kit-card-title">${safeText(I18n.field(k,'name'))}</h3><p class="kit-card-desc">${k.shortDesc||k.description}</p>
      <div class="kit-card-footer"><span class="kit-card-price">${I18n.currency(k.price.toFixed(2))}</span></div></div></div>`;
  }).join('');
}

function renderHomeCats(){
  document.getElementById('homeCatGrid').innerHTML=allCategories.map(c=>
    `<div class="cat-card" onclick="navigate('#/paintings?cat=${c.id}')"><img src="${c.image}" alt="${safeText(I18n.field(c,'name'))}" loading="lazy"><div class="cat-card-overlay"><span class="cat-card-name">${safeText(I18n.field(c,'name'))}</span></div></div>`
  ).join('');
}
...
function renderHomeEvents(){
  ...
  grid.innerHTML = upcoming.map(ev=>{
    ...
    const dateStr = d.toLocaleDateString(I18n.locale(),{weekday:'long',day:'numeric',month:'long'});
    const spotsLeft = ev.maxSpots - ev.bookedSpots;
    return I18n.html`<div class="event-card" onclick="navigate('#/event/${ev.id}')" style="cursor:pointer">
      ...
      <div class="event-card-body">
        ...
        <div class="event-card-footer">
          <span class="event-card-price">${I18n.currency(ev.price.toFixed(2))}</span>
...
function toggleMobile(){setMobileNavigation(!document.getElementById('navDrawer')?.classList.contains('open'))}
...
function updateAuthUI(){
  ...
  const cartH=I18n.html`<button class="btn-cart nav-cart-btn" onclick="openCart()" id="cartBtn" aria-label="Panier" style="${cart.length?'display:flex':'display:none'}"><span class="cart-icon">🛒</span><span id="cartCount" class="cart-count-badge">${cart.reduce((s,i)=>s+i.qty,0)}</span></button>`;
  if(currentUser){
    const av=currentUser.picture?`<img src="${currentUser.picture}">`:`${currentUser.name.charAt(0).toUpperCase()}`;
    const adm=['admin','staff'].includes(currentUser.role)?`<a href="#/admin" class="admin-link">${currentUser.role==='staff'?I18n.t('Espace équipe'):I18n.t('Admin')}</a>`:'';
    a.innerHTML=I18n.html`${cartH}<div class="nav-dropdown"><div class="nav-user" onclick="toggleDropdown()"><div class="nav-user-avatar">${av}</div><span class="nav-user-name">${currentUser.name.split(' ')[0]}</span></div><div class="nav-dropdown-menu"><a href="#/profile">Mon Profil</a>${adm}<button class="logout-btn" onclick="logout()">Déconnexion</button></div></div>`;
  }else{a.innerHTML=I18n.html`${cartH}<button class="btn btn-ghost btn-sm" onclick="openModal('auth')">Connexion</button><button class="btn btn-orange btn-sm" onclick="openModal('auth','register')">S'inscrire</button>`}
}
...
async function loadAdminBookings(){try{adminBookings=await(await artyFetch('/api/admin/bookings',{headers:authH()})).json()}catch{adminBookings=[]}}
async function loadAdminOrders(){try{adminOrders=await(await artyFetch('/api/admin/orders',{headers:authH()})).json()}catch{adminOrders=[]}}
async function loadEventRequests(){try{eventRequests=await(await artyFetch('/api/admin/event-requests',{headers:authH()})).json()}catch{eventRequests=[]}}
function formatEventDate(ev,withYear=false){
  if(!ev?.date)return I18n.t('Date à confirmer');
  const d=new Date(ev.date+'T00:00:00');
...
function prefillPrivateEventType(type){
  eventBuilderState.eventName=I18n.t(String(type||''));
...
function renderCatalogFilterOptions(){
  ...
  if(cats){
    const buttons=[I18n.html`<button class="catalog-pill active" data-category="all" onclick="setCatalogFilter('category','all')">Tous <span>${allKits.length}</span></button>`]
      .concat(allCategories.map(c=>{
        const count=allKits.filter(k=>String(k.categoryId)===String(c.id)).length;
        return `<button class="catalog-pill" data-category="${safeAttr(c.id)}" onclick="setCatalogFilter('category','${safeAttr(c.id)}')">${safeText(I18n.field(c,'name'))} <span>${count}</span></button>`;
      }));
...
function getFilteredKits(){
  ...
  kits.sort((a,b)=>{
    ...
    if(sort==='name') return String(a.name||'').localeCompare(String(b.name||''),I18n.locale());
    if(sort==='newest') return Number(b.id||0)-Number(a.id||0);
    return Number(!!b.featured)-Number(!!a.featured) || String(a.name||'').localeCompare(String(b.name||''),I18n.locale());
  });
  return kits;
}
...
function renderActiveFilters(filtered){
  ...
  if(result) result.textContent=I18n.msg`${filtered.length} produit${filtered.length!==1?'s':''} trouvé${filtered.length!==1?'s':''} sur ${allKits.length}`;
  if(!wrap) return;
  const chips=[];
  if(catalogFilters.category!=='all'){
    const cat=allCategories.find(c=>String(c.id)===String(catalogFilters.category));
    chips.push(I18n.html`<button onclick="removeCatalogFilter('category')">Catégorie: ${safeText(cat?.name||catalogFilters.category)} ×</button>`);
  }
  if(catalogFilters.stock==='in') chips.push(I18n.html`<button onclick="removeCatalogFilter('stock')">En stock seulement ×</button>`);
  if(catalogFilters.search) chips.push(I18n.html`<button onclick="removeCatalogFilter('search')">Recherche: ${safeText(catalogFilters.search)} ×</button>`);
  ...
  wrap.innerHTML=chips.length?chips.join(''):I18n.html('<span class="active-filter-placeholder">Aucun filtre actif</span>');
}
...
function renderKitsGrid(){
  ...
  g.innerHTML=filtered.map(k=>{
    ...
    return `<div class="kit-card catalog-kit-card" onclick="navigate('#/product/${k.id}')">
      <div class="kit-card-img"><img src="${safeAttr(img)}" alt="${safeAttr(k.name)}" loading="lazy">${k.featured?I18n.html('<span class="kit-card-badge">Populaire</span>'):''}${!isInStock?I18n.html('<span class="kit-stock-badge">Épuisé</span>'):''}</div>
      <div class="kit-card-body">
        <div class="kit-card-category">${safeText(cat?cat.name:I18n.t('Sans catégorie'))}</div>
        <h3 class="kit-card-title">${safeText(I18n.field(k,'name'))}</h3>
        <p class="kit-card-desc">${safeText(k.shortDesc||k.description||'')}</p>
        <div class="kit-card-footer"><span class="kit-card-price">${I18n.currency(toMoney(k.price))}</span></div>
      </div>
    </div>`;
  }).join('');
  if(!filtered.length) g.innerHTML=I18n.html('<div class="empty-state catalog-empty"><h3>Aucun produit trouvé</h3><p>Essayez de retirer un filtre ou de chercher un mot plus simple.</p><button class="btn btn-orange btn-sm" onclick="resetCatalogFilters()">Réinitialiser les filtres</button></div>');
...
function toggleCatalogFilters(){
  ...
  syncMobileFilters();
  if(document.getElementById('catalogSidebar')?.classList.contains('open'))document.getElementById('catalogSidebar').querySelector('button,input')?.focus();
}
  ...
  c.innerHTML=I18n.html`
    ...
    <div class="event-detail-modern">
      ...
      <div class="event-detail-info event-detail-modern-info">
        ...
        <h1>${safeText(I18n.field(ev,'title'))}</h1>
        <div class="product-price">${I18n.currency(toMoney(ev.price))} <span>/personne</span></div>
        <p class="product-desc">${safeText(ev.description||I18n.t('Une activité créative Arty avec tout le matériel inclus.'))}</p>
...
function renderPartyCalendarAgenda(eventsByDate){
  ...
  const heading=selectedDate?selectedDate.toLocaleDateString(I18n.locale(),{weekday:'long',day:'numeric',month:'long',year:'numeric'}):I18n.t('Sélectionnez une date');
  ...
  agenda.innerHTML=I18n.html`<div class="party-calendar-agenda-head"><span>Votre sélection</span><h3>${safeText(heading)}</h3><p>${events.length?I18n.msg`${events.length} événement${events.length>1?'s':''} à cette date`:I18n.t('Aucun événement public à cette date')}</p></div>
    <div class="party-calendar-agenda-list">${cards||I18n.html`<div class="party-calendar-no-event"><span>${eventExperienceIcon('calendar')}</span><h4>Cette date est libre</h4><p>Vous pouvez demander un événement privé et nous préparerons une proposition adaptée à votre groupe.</p><button class="btn btn-orange btn-sm" type="button" onclick="navigate('#/event-builder')">Créer mon événement</button></div>`}</div>`;
}
function renderPartyCalendar(){
  ...
  title.textContent=monthStart.toLocaleDateString(I18n.locale(),{month:'long',year:'numeric'});
  ...
  for(let index=0;index<42;index++){
    ...
    const eventLabel=events.length?I18n.msg`${events.length} événement${events.length>1?'s':''}`:I18n.t('Aucun événement');
    cells.push(`<button type="button" class="${classes}" onclick="selectPartyCalendarDate('${key}')" aria-pressed="${key===partyCalendarSelectedDate?'true':'false'}" aria-label="${safeAttr(date.toLocaleDateString(I18n.locale(),{day:'numeric',month:'long',year:'numeric'}))}, ${eventLabel}"><span class="party-calendar-day-number">${date.getDate()}</span>${events.length?`<span class="party-calendar-day-event"><i></i><small>${events.length>1?events.length+I18n.t(' événements'):I18n.t('Événement')}</small></span>`:''}</button>`);
  }
...
function renderTutorialsPage(){
  ...
  if(!kitsWithVideo.length){grid.innerHTML=I18n.html('<div class="empty-state"><div class="empty-state-icon">🎬</div><p>Aucun tutoriel disponible pour le moment.</p></div>');initScrollEffects();return}
  grid.innerHTML = kitsWithVideo.map(k=>`<div class="tutorial-card">
    ...
    <div class="tutorial-body">
      <div class="tutorial-kit-name">Kit: ${safeText(I18n.field(k,'name'))}</div>
      <h3 class="tutorial-title">${k.videoTitle||I18n.t('Tutoriel ')+k.name}</h3>
    </div>
  </div>`).join('');
  initScrollEffects();
}
...
async function renderProfilePage(){
  ...
  const av=currentUser.picture?`<img src="${currentUser.picture}">`:`${currentUser.name.charAt(0).toUpperCase()}`;
  const badge=currentUser.role==='admin'?'<span class="profile-badge admin">Admin</span>':currentUser.role==='staff'?I18n.html('<span class="profile-badge admin">Équipe ARTY</span>'):I18n.html('<span class="profile-badge user">Membre</span>');
  const prov=currentUser.provider==='google'?'Google':I18n.t('Courriel & mot de passe');
  c.innerHTML=I18n.html`<div style="padding-top:20px"><div class="profile-header"><div class="profile-avatar">${av}</div><div class="profile-meta"><h2>${currentUser.name}</h2><p>${currentUser.email} · ${prov}</p>${badge}</div></div><div class="profile-tabs"><button class="profile-tab active" onclick="switchPTab('orders',this)">Commandes</button><button class="profile-tab" onclick="switchPTab('bookings',this)">Réservations</button><button class="profile-tab" onclick="switchPTab('settings',this)">Paramètres</button></div><div class="profile-panel active" id="panel-orders"><div class="profile-card"><h3>Historique des Commandes</h3><div id="ordersWrap"><p style="color:var(--text-faint)">Chargement...</p></div></div></div><div class="profile-panel" id="panel-bookings"><div class="profile-card"><h3>Mes Réservations</h3><div id="bookingsWrap"><p style="color:var(--text-faint)">Chargement...</p></div></div></div><div class="profile-panel" id="panel-settings"><div class="profile-card"><h3>Paramètres du Compte</h3><div class="form-group"><label>Nom</label><input type="text" id="profileName" value="${currentUser.name}"></div><div class="form-group"><label>Courriel</label><input type="email" value="${currentUser.email}" disabled style="opacity:.5"></div>${currentUser.provider==='local'?I18n.html`<div class="form-group"><label>Mot de passe actuel</label><input type="password" id="pCurPw" placeholder="Requis pour changer"></div><div class="form-group"><label>Nouveau mot de passe</label><input type="password" id="pNewPw" placeholder="Laisser vide pour garder"></div>`:I18n.html('<p style="font-size:.86rem;color:var(--text-light);margin:14px 0">Mot de passe géré par Google.</p>')}<button class="btn btn-teal" onclick="updateProfile()">Sauvegarder</button></div></div></div>`;
  try{const r=await artyFetch('/api/orders/mine',{headers:authH()});const orders=await r.json();document.getElementById('ordersWrap').innerHTML=orders.length?orders.map(o=>`<div class="order-item"><div><div class="order-id">${o.id}</div><div class="order-date">${new Date(o.createdAt).toLocaleDateString(I18n.locale())}</div><div class="order-items-list">${o.items.map(i=>i.name+' ×'+i.qty).join(', ')}</div></div><div style="text-align:right"><div class="order-total">${I18n.currency(o.total.toFixed(2))}</div><span class="order-status">${I18n.status(o.status)}</span></div></div>`).join(''):I18n.html('<div class="empty-state"><div class="empty-state-icon">📦</div><p>Aucune commande</p></div>')}catch{}
  try{const r=await artyFetch('/api/bookings/mine',{headers:authH()});const bks=await r.json();document.getElementById('bookingsWrap').innerHTML=bks.length?bks.map(b=>I18n.html`<div class="order-item"><div><div class="order-id">${b.event?.title||I18n.t('Événement')}</div><div class="order-date">${b.event?.date||''} · ${b.guests} personne(s)</div></div><div><span class="order-status">${I18n.status(b.status)}</span></div></div>`).join(''):I18n.html('<div class="empty-state"><div class="empty-state-icon">🎫</div><p>Aucune réservation</p></div>')}catch{}
}
...
async function updateProfile(){const body={name:document.getElementById('profileName').value};if(currentUser.provider==='local'){const cp=document.getElementById('pCurPw')?.value,np=document.getElementById('pNewPw')?.value;if(np){body.currentPassword=cp;body.newPassword=np}}try{const r=await artyFetch('/api/users/me',{method:'PUT',headers:authH(),body:JSON.stringify(body)});const d=await r.json();if(!r.ok)return showToast(d.error,'error');currentUser=d.user;localStorage.setItem('arty_user',JSON.stringify(currentUser));updateAuthUI();showToast(I18n.t('Profil mis à jour!'),'success')}catch{showToast(I18n.t('Erreur'),'error')}}
...
function addToCart(kitId){
  ...
  if(!kit)return;
  if(kit.inStock===false)return showToast(I18n.t('Ce produit est épuisé'),'error');
  ...
  saveCart();updateCartUI();showToast(I18n.msg`${kit.name} ajouté au panier!`,'success');
}
...
function addBundleToCart(bundleId){
  ...
  const ex=cart.find(i=>String(i.id)===id);
  if(ex)ex.qty+=1;
  else cart.push({id,name:b.name+I18n.t(' (Ensemble)'),price:Number(b.price)||0,image:b.image,qty:1,type:'bundle'});
  saveCart();updateCartUI();showToast(I18n.t('Ensemble ajouté au panier!'),'success')
}
...
function updateCartUI(){const n=cart.reduce((s,i)=>s+(parseInt(i.qty)||0),0);const b=document.getElementById('cartBtn'),c=document.getElementById('cartCount');if(b)b.style.display=n>0?'inline-flex':'none';if(c)c.textContent=n;syncMobileCart()}
function getSubtotal(){return cart.reduce((s,i)=>s+(Number(i.price)||0)*(parseInt(i.qty)||0),0)}
function getTotal(){return getSubtotal()}
function openCart(){closeMobileNavigation();renderCartItems();document.getElementById('cartOverlay').classList.add('open');document.getElementById('cartSidebar').classList.add('open');document.body.style.overflow='hidden'}
...
function goToCheckout(){if(!cart.length)return showToast(I18n.t('Panier vide'),'error');closeCart();navigate('#/checkout')}
function renderCartItems(){
  const c=document.getElementById('cartItems'),f=document.getElementById('cartFooter');
  if(!c||!f)return;
  if(!cart.length){c.innerHTML=I18n.html('<div class="cart-empty"><div class="cart-empty-icon">Panier</div><p>Panier vide</p></div>');f.style.display='none';return}
  f.style.display='block';
  c.innerHTML=cart.map(i=>I18n.html`<div class="cart-item">
    <img src="${safeAttr(i.image)}" class="cart-item-img" alt="${safeAttr(i.name)}">
    <div class="cart-item-info"><div class="cart-item-name">${safeText(i.name)}</div><div class="cart-item-price">${I18n.currency(toMoney(i.price))}</div>
  ...
  document.getElementById('cartTotal').textContent=`${I18n.currency(getTotal().toFixed(2))}`;
}
...
function renderCheckoutPage(){
  ...
  if(!c)return;
  if(!cart.length){c.innerHTML=I18n.html`<div class="checkout-empty"><div class="section-tag">Panier</div><h2 class="section-heading">Votre panier est <span class="accent">vide</span></h2><p class="section-sub">Ajoutez un kit ou un ensemble avant de continuer.</p><a href="#/paintings" class="btn btn-orange">Voir les kits →</a></div>`;return}
  ...
  c.innerHTML=I18n.html`
    ...
    <div class="checkout-layout">
      <section class="checkout-card-main">
        ...
        ${shippingStep}
        <div class="checkout-step-title"><span>3</span><div><h3>Paiement sécurisé</h3><p>${paymentUnavailable?I18n.t('Le paiement et sa confirmation automatique doivent être configurés avant la vente de billets.'):stripeConfigured?I18n.t('Paiement intégré avec Stripe. ARTY ne reçoit jamais le numéro complet de la carte.'):I18n.t('Le paiement en ligne n’est pas encore connecté.')}</p></div></div>
        <div class="payment-provider-box ${stripeConfigured&&!paymentUnavailable?'stripe-ready':''} ${paymentUnavailable?'payment-required':''}"><strong>${paymentUnavailable?I18n.t('Paiement des billets indisponible'):stripeConfigured?I18n.t('Paiement sécurisé par Stripe'):I18n.t('Paiement en ligne indisponible')}</strong><p>${paymentUnavailable?I18n.t('La commande ne peut pas être passée et aucun billet ne sera créé tant que la confirmation sécurisée du paiement n’est pas active.'):stripeConfigured?I18n.t('Cliquez sur Continuer au paiement pour afficher le formulaire sécurisé.'):I18n.t('La commande sera enregistrée en attente de paiement.')}</p></div>
        ...
        <button class="btn btn-orange checkout-submit" id="placeOrderBtn" onclick="placeOrder()" ${paymentUnavailable?'disabled':''}>${paymentUnavailable?I18n.t('Paiement requis pour acheter les billets'):stripeConfigured?I18n.t('Continuer au paiement sécurisé →'):I18n.t('Créer la commande →')}</button>
      </section>
      <aside class="checkout-summary-card">
        <h3>Résumé</h3>${summary}
        <div class="checkout-total-row"><span>Sous-total</span><strong>${I18n.currency(toMoney(getSubtotal()))}</strong></div>
        <div class="checkout-note-small">${hasEventTickets?(needsShipping?I18n.t('Les accès seront envoyés après paiement. Les produits physiques seront livrés à l’adresse indiquée.'):I18n.t('Un seul billet par événement indiquera le nombre total de personnes admises.')):I18n.t('Les taxes et frais de livraison applicables sont confirmés au paiement.')}</div>
      </aside>
    </div>`;
}
async function placeOrder(){
  if(!cart.length)return showToast(I18n.t('Panier vide'),'error');
  ...
  if(!name)return showToast(I18n.t('Entrez votre nom'),'error');
  if(!email||!validateEmail(email))return showToast(I18n.t('Entrez un courriel valide'),'error');
  if(needsShipping&&!address)return showToast(I18n.t('Entrez l’adresse de livraison'),'error');
  if(hasEventTickets&&!ticketPaymentsConfigured)return showToast(I18n.t('Le paiement sécurisé doit être entièrement configuré avant de vendre des billets'),'error');
  if(!document.getElementById('coPolicyAccept')?.checked)return showToast(I18n.t('Veuillez accepter les politiques avant de continuer'),'error');
  ...
  if(btn){btn.disabled=true;btn.textContent=I18n.t('Préparation du paiement...')}
  try{
    const r=await artyFetch('/api/orders',{method:'POST',headers:authH(),body:JSON.stringify(payload)});
    const d=await r.json();
    if(!r.ok){if(btn){btn.disabled=false;btn.textContent=stripeConfigured?I18n.t('Continuer au paiement sécurisé →'):I18n.t('Créer la commande →')};return showToast(d.error||I18n.t('Erreur'),'error')}
    ...
    showOrderSuccess(d.order,I18n.t('Paiement: en attente. Stripe n’est pas encore disponible pour cette commande.'));
    renderCheckoutPage();
  }catch(err){
    console.error(err);
    showToast(I18n.t('Erreur lors de la commande'),'error');
    if(btn){btn.disabled=false;btn.textContent=stripeConfigured?I18n.t('Continuer au paiement sécurisé →'):I18n.t('Créer la commande →')}
  }
}
function showOrderSuccess(order,paymentNote){
  document.getElementById('successTitle').textContent=I18n.t('Commande reçue!');
  document.getElementById('successSubtitle').textContent=I18n.t('Nous avons enregistré votre commande.');
  document.getElementById('successOrderId').textContent=I18n.msg`Commande #${order.id}`;
...
async function mountStripePayment(order,payment){
  if(!window.Stripe)return showToast(I18n.t('Stripe ne s’est pas chargé. Rechargez la page.'),'error');
  if(!payment.publishableKey)return showToast(I18n.t('Clé publishable Stripe manquante'),'error');
  ...
  stripeElements=stripeInstance.elements({clientSecret:payment.clientSecret,appearance,locale:I18n.language()});
  ...
  if(label)label.textContent=`Commande ${order.id} · ${I18n.currency(toMoney(order.total))}`;
  if(message)message.textContent='';
  if(panel)panel.style.display='block';
  paymentElement.mount('#payment-element');
  panel?.scrollIntoView({behavior:'smooth',block:'center'});
  showToast(I18n.t('Commande créée. Entrez la carte pour payer.'),'success');
}
async function confirmStripePayment(){
  if(!stripeInstance||!stripeElements||!currentStripeOrder||!currentStripePayment)return showToast(I18n.t('Paiement Stripe non prêt'),'error');
  const btn=document.getElementById('stripePayBtn');
  const msg=document.getElementById('stripePaymentMessage');
  if(btn){btn.disabled=true;btn.textContent=I18n.t('Paiement en cours...')}
  if(msg){msg.textContent=I18n.t('Traitement du paiement...');msg.className='stripe-payment-message'}
  try{
    ...
    if(result.error){
      if(msg){msg.textContent=result.error.message||I18n.t('Paiement refusé');msg.className='stripe-payment-message error'}
      if(btn){btn.disabled=false;btn.textContent=I18n.t('Réessayer le paiement →')}
      return;
    }
    const pi=result.paymentIntent;
    const r=await artyFetch('/api/stripe/confirm-order',{method:'POST',headers:authH(),body:JSON.stringify({orderId:currentStripeOrder.id,paymentIntentId:pi?.id||currentStripePayment.paymentIntentId})});
    const d=await r.json().catch(()=>({}));
    if(!r.ok)throw new Error(d.error||I18n.t('Paiement traité, mais confirmation serveur impossible'));
    cart=[];saveCart();updateCartUI();
    ...
    if(issuedTicket?.code){
      ...
      showToast(I18n.t('Paiement confirmé. Vos billets sont prêts.'),'success');
      navigate(`#/ticket/${encodeURIComponent(issuedTicket.code)}`);
      return;
    }
    if((d.order?.paymentStatus||'')!=='paid'){
      showToast(I18n.t('Paiement en traitement. Les billets seront envoyés après confirmation.'),'success');
      navigate(`#/payment-complete?order=${encodeURIComponent(currentStripeOrder.id)}`);
      return;
    }
    showOrderSuccess(d.order||currentStripeOrder,I18n.t('Paiement Stripe confirmé. La commande est maintenant marquée comme payée.'));
    renderCheckoutPage();
  }catch(err){
    console.error(err);
    if(msg){msg.textContent=err.message||I18n.t('Erreur de paiement');msg.className='stripe-payment-message error'}
    if(btn){btn.disabled=false;btn.textContent=I18n.t('Réessayer le paiement →')}
  }
}
...
function switchAuthTab(t){
  ...
  const copy={login:[I18n.t('Bienvenue'),I18n.t('Connectez-vous à votre compte.')],register:[I18n.t('Créer un compte'),I18n.t('Inscription rapide et sécurisée.')],forgot:[I18n.t('Mot de passe oublié'),I18n.t('Recevez un lien sécurisé par courriel.')],reset:[I18n.t('Nouveau mot de passe'),I18n.t('Choisissez votre nouveau mot de passe.')]}[active];
...
async function doLogin(){
  ...
  if(!e||!p)return showToast(I18n.t('Remplissez tous les champs'),'error');
  try{const r=await artyFetch('/api/users/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:e,password:p})});const d=await r.json();if(!r.ok)return showToast(d.error||I18n.t('Connexion impossible'),'error');authToken=d.token;currentUser=d.user;localStorage.setItem('arty_token',authToken);localStorage.setItem('arty_user',JSON.stringify(currentUser));updateAuthUI();closeModal('auth');refreshCheckoutIfOpen();showToast(I18n.msg`Bienvenue, ${currentUser.name}!`,'success')}catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
...
function togglePasswordField(id,btn){const input=document.getElementById(id);if(!input)return;const show=input.type==='password';input.type=show?'text':'password';if(btn)btn.textContent=show?I18n.t('Cacher'):I18n.t('Voir')}
function updatePasswordMatchUI(){
  ...
  if(!p&&!c){f.textContent=I18n.t('Utilisez au moins 6 caractères. Les deux mots de passe doivent être identiques.');return}
  if(p.length<6){f.textContent=I18n.t('Le mot de passe doit contenir au moins 6 caractères.');f.classList.add('bad');return}
  if(c&&p!==c){f.textContent=I18n.t('Les deux mots de passe ne sont pas identiques.');f.classList.add('bad');return}
  if(c&&p===c){f.textContent=I18n.t('Parfait, les mots de passe correspondent.');f.classList.add('ok');return}
  f.textContent=I18n.t('Confirmez le mot de passe pour éviter une erreur.');
}
async function doRegister(){
  ...
  if(!n||!e||!p||!pc)return showToast(I18n.t('Remplissez tous les champs'),'error');
  if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e))return showToast(I18n.t('Entrez un courriel valide'),'error');
  if(p.length<6)return showToast(I18n.t('Mot de passe: 6 caractères minimum'),'error');
  if(p!==pc){updatePasswordMatchUI();return showToast(I18n.t('Les mots de passe ne correspondent pas'),'error')}
  try{const r=await artyFetch('/api/users/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n,email:e,password:p,confirmPassword:pc})});const d=await r.json();if(!r.ok)return showToast(d.error||I18n.t('Inscription impossible'),'error');authToken=d.token;currentUser=d.user;localStorage.setItem('arty_token',authToken);localStorage.setItem('arty_user',JSON.stringify(currentUser));updateAuthUI();closeModal('auth');refreshCheckoutIfOpen();showToast(d.emailStatus==='sent'?I18n.t('Compte créé. Un courriel de bienvenue vous a été envoyé.'):I18n.msg`Bienvenue chez ARTY, ${currentUser.name}!`,'success')}catch{showToast(I18n.t('Erreur'),'error')}
}
async function requestPasswordReset(){
  ...
  if(!email||!validateEmail(email))return showToast(I18n.t('Entrez un courriel valide'),'error');
  if(button){button.disabled=true;button.textContent=I18n.t('Envoi en cours...')}
  try{const r=await artyFetch('/api/users/forgot-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Envoi impossible'),'error');showToast(d.message||I18n.t('Si le compte existe, le lien a été envoyé.'),'success');switchAuthTab('login');document.getElementById('loginEmail').value=email}catch{showToast(I18n.t('Erreur de connexion'),'error')}finally{if(button){button.disabled=false;button.textContent=I18n.t('Envoyer le lien')}}
}
async function submitPasswordReset(){
  ...
  if(!token)return showToast(I18n.t('Lien de réinitialisation invalide'),'error');
  if(password.length<6)return showToast(I18n.t('Le mot de passe doit contenir au moins 6 caractères'),'error');
  if(password!==confirmPassword)return showToast(I18n.t('Les mots de passe ne correspondent pas'),'error');
  if(button){button.disabled=true;button.textContent=I18n.t('Modification...')}
  try{const r=await artyFetch('/api/users/reset-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,password,confirmPassword})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Modification impossible'),'error');history.replaceState(null,'',window.location.pathname+'#/');switchAuthTab('login');document.getElementById('resetPassword').value='';document.getElementById('resetPasswordConfirm').value='';showToast(d.message||I18n.t('Mot de passe modifié'),'success')}catch{showToast(I18n.t('Erreur de connexion'),'error')}finally{if(button){button.disabled=false;button.textContent=I18n.t('Modifier le mot de passe')}}
}
function logout(s){artyFetch('/api/users/logout',{method:'POST',headers:authH()}).catch(()=>{});authToken=null;currentUser=null;localStorage.removeItem('arty_token');localStorage.removeItem('arty_user');updateAuthUI();navigate('#/');if(!s)showToast(I18n.t('Déconnecté'),'success')}
...
function initGoogleSignIn(retry=0){
  ...
  if(!googleClientId||googleClientId==='YOUR_GOOGLE_CLIENT_ID_HERE'){
    w.innerHTML=I18n.html`<button class="google-btn google-btn-disabled" onclick="showToast('Google non configuré. Ajoutez GOOGLE_CLIENT_ID dans Render ou googleClientId dans db.json','error')">${googleIcon()}Continuer avec Google</button><div class="google-help">Google sera actif dès que le Client ID est configuré.</div>`;
    return;
  }
  if(!window.google?.accounts?.id){
    w.innerHTML=I18n.html`<button class="google-btn google-btn-disabled" disabled>${googleIcon()}Chargement de Google...</button>`;
    if(retry<24)setTimeout(()=>initGoogleSignIn(retry+1),250);else w.innerHTML=I18n.html`<button class="google-btn google-btn-disabled" onclick="showToast('Google ne s’est pas chargé. Vérifiez le domaine autorisé et le script Google.','error')">${googleIcon()}Google non disponible</button>`;
    return;
  }
  try{
    ...
    w.innerHTML='';
    google.accounts.id.renderButton(w,{theme:'outline',size:'large',width:380,text:'continue_with',shape:'pill',locale:I18n.language()});
  }catch(err){
    console.error('Google init error',err);
    w.innerHTML=I18n.html`<button class="google-btn google-btn-disabled" onclick="showToast('Google non disponible pour ce domaine. Vérifiez Authorized JavaScript origins.','error')">${googleIcon()}Google non disponible</button>`;
  }
}
async function handleGoogle(r){try{const res=await artyFetch('/api/users/google',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({credential:r.credential})});const d=await res.json();if(!res.ok)return showToast(d.error||I18n.t('Connexion Google impossible'),'error');authToken=d.token;currentUser=d.user;localStorage.setItem('arty_token',authToken);localStorage.setItem('arty_user',JSON.stringify(currentUser));updateAuthUI();closeModal('auth');refreshCheckoutIfOpen();showToast(I18n.msg`Bienvenue, ${currentUser.name}!`,'success')}catch{showToast(I18n.t('Échec Google'),'error')}}
...
function openBooking(eventId){
  ...
  if(!ev)return;
  const left=spotsLeft(ev);
  if(left<=0)return showToast(I18n.t('Cet événement est complet'),'error');
...
function confirmBooking(){
  ...
  saveCart();updateCartUI();closeModal('booking');showToast(I18n.msg`Accès pour ${qty} personne${qty>1?'s':''} ajouté au panier`,'success');setTimeout(openCart,120);
  if(button){button.disabled=false;button.textContent=I18n.t('Ajouter au panier')}
}
async function submitPrivateEventRequest(){
  ...
  if(!payload.name||!payload.email||!payload.eventType)return showToast(I18n.t('Nom, courriel et type d’événement requis'),'error');
  try{
    const r=await artyFetch('/api/event-requests',{method:'POST',headers:{'Content-Type':'application/json',...(currentUser?authH():{})},body:JSON.stringify(payload)});
    const d=await r.json();
    if(!r.ok)return showToast(d.error,'error');
    showToast(I18n.t('Demande envoyée! Arty pourra vous répondre bientôt.'),'success');
...
function renderAdminOrders(){
  ...
  const rows=(adminOrders||[]).map(o=>{
    ...
    return I18n.html`<tr><td><strong>${safeText(o.id)}</strong><br><span class="admin-muted">${new Date(o.createdAt).toLocaleDateString(I18n.locale())}</span></td><td>${safeText(cust.name||'')}<br><span class="admin-muted">${safeText(cust.email||o.guestEmail||'')}</span></td><td>${itemText}</td><td>${I18n.currency(toMoney(o.total))}</td><td><span class="admin-status ${o.paymentStatus==='paid'?'ok':'out'}">${safeText(I18n.status(o.paymentStatus||'pending'))}</span></td><td><select class="admin-status-select" onchange="updateOrderStatus('${safeAttr(o.id)}',this.value)"><option value="en attente de paiement" ${o.status==='en attente de paiement'?'selected':''}>En attente paiement</option><option value="payée" ${o.status==='payée'?'selected':''}>Payée</option><option value="préparation" ${o.status==='préparation'?'selected':''}>Préparation</option><option value="expédiée" ${o.status==='expédiée'?'selected':''}>Expédiée</option><option value="annulée" ${o.status==='annulée'?'selected':''}>Annulée</option></select></td></tr>`;
  }).join('');
  panel.innerHTML=I18n.html`<div class="admin-form-card"><h3>Commandes clients</h3><p class="admin-help">Les commandes invitées et connectées restent sauvegardées dans la base persistante.</p></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Commande</th><th>Client</th><th>Articles</th><th>Total</th><th>Paiement</th><th>Statut</th></tr></thead><tbody>${rows||I18n.html('<tr><td colspan="6" class="admin-muted">Aucune commande pour le moment.</td></tr>')}</tbody></table></div>`;
}
async function updateOrderStatus(id,status){
  try{const r=await artyFetch(`/api/admin/orders/${encodeURIComponent(id)}/status`,{method:'PUT',headers:authH(),body:JSON.stringify({status})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');showToast(I18n.t('Statut mis à jour'),'success');await loadAdminOrders();renderAdminOrders()}catch{showToast(I18n.t('Erreur'),'error')}
}
...
function renderAdminEvents(){
  ...
  const requestRows=eventRequests.map(r=>I18n.html`<tr><td><strong>${safeText(r.name)}</strong><br><small>${safeText(r.email)} ${r.phone?'· '+safeText(r.phone):''}</small></td><td>${safeText(r.eventType)}<br><small>${r.preferredDate?safeText(r.preferredDate):I18n.t('Date flexible')} · ${r.guests||'?'} pers.</small></td><td>${safeText(r.location||I18n.t('À confirmer'))}</td><td><span class="admin-status-badge">${safeText(I18n.status(r.status||'nouvelle'))}</span></td><td><div class="admin-actions"><button class="admin-btn admin-btn-edit" onclick="updateEventRequestStatus(${r.id},'contactée')">Contactée</button><button class="admin-btn admin-btn-delete" onclick="deleteEventRequest(${r.id})">Supprimer</button></div></td></tr>`).join('');
  const bookingRows=adminBookings.slice(0,25).map(b=>I18n.html`<tr><td><strong>${safeText(b.name)}</strong><br><small>${safeText(b.email)} ${b.phone?'· '+safeText(b.phone):''}</small></td><td>${safeText(b.event?.title||I18n.t('Événement supprimé'))}</td><td>${b.guests} place${b.guests>1?'s':''}</td><td>${new Date(b.bookedAt).toLocaleDateString(I18n.locale())}</td><td>${safeText(I18n.status(b.status||'confirmée'))}</td></tr>`).join('');
  const eventRows=adminEvents.map(e=>I18n.html`<tr><td><strong>${safeText(I18n.field(e,'title'))}</strong><br><small>${safeText(I18n.t(e.eventType)||'atelier')} · ${safeText(e.location||I18n.t('Lieu à confirmer'))}</small></td><td>${safeText(e.date||'')} ${safeText(e.time||'')}</td><td><span class="admin-status-badge ${e.status==='draft'?'draft':e.status==='cancelled'?'cancelled':''}">${safeText(I18n.status(e.status||'published'))}</span></td><td>${I18n.currency(toMoney(e.price))}</td><td>${e.bookedSpots||0}/${e.maxSpots||0}</td><td><div class="admin-actions"><button class="admin-btn admin-btn-edit" onclick="editEv(${e.id})">Modifier</button><button class="admin-btn admin-btn-delete" onclick="deleteEv(${e.id})">Supprimer</button></div></td></tr>`).join('');
  panel.innerHTML=I18n.html`
    ...
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Événement</th><th>Date</th><th>Statut</th><th>Prix</th><th>Places</th><th>Actions</th></tr></thead><tbody>${eventRows||I18n.html('<tr><td colspan="6">Aucun événement.</td></tr>')}</tbody></table></div>
    <div class="admin-section-title"><h3>Réservations récentes</h3><p>Les 25 dernières réservations de places.</p></div>
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Client</th><th>Événement</th><th>Places</th><th>Date</th><th>Statut</th></tr></thead><tbody>${bookingRows||I18n.html('<tr><td colspan="5">Aucune réservation.</td></tr>')}</tbody></table></div>
    ...
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Client</th><th>Projet</th><th>Lieu</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${requestRows||I18n.html('<tr><td colspan="5">Aucune demande privée.</td></tr>')}</tbody></table></div>`;
}
async function saveEv(){
  ...
  showToast(eid?I18n.t('Événement modifié!'):I18n.t('Événement publié!'),'success');
  await loadEvents();
  await loadAdminData();
}
function editEv(id){
...
function kitPriceHTML(k,cls='kit-card-price'){
  ...
  if(sale!==null&&sale<original){return `<span class="${cls} price-sale"><span>${I18n.currency(toMoney(sale))}</span><small>${I18n.currency(toMoney(original))}</small></span>${label?`<span class="discount-pill">${safeText(label)}</span>`:''}`}
  return `<span class="${cls}">${I18n.currency(toMoney(original))}</span>${label?`<span class="discount-pill">${safeText(label)}</span>`:''}`;
}
function stockBadgeHTML(k){
  if(k?.inStock===false)return I18n.html('<span class="kit-stock-badge">Épuisé</span>');
  return '';
}
function stockTagText(k){
  if(k?.inStock===false)return I18n.t('Épuisé');
  return I18n.t('En stock');
}

async function loadAdminAnalytics(){try{adminAnalyticsPro=await(await artyFetch('/api/admin/analytics',{headers:authH()})).json()}catch{adminAnalyticsPro=null}}
async function loadAdminDiscounts(){try{adminDiscounts=await(await artyFetch('/api/admin/discounts',{headers:authH()})).json()}catch{adminDiscounts=[]}}
async function loadAdminBundleDeals(){try{adminBundleDealRules=await(await artyFetch('/api/admin/bundle-deals',{headers:authH()})).json();bundleDealRules=adminBundleDealRules}catch{adminBundleDealRules=[]}}
async function loadAdminRefunds(){try{adminRefunds=await(await artyFetch('/api/admin/refunds',{headers:authH()})).json()}catch{adminRefunds=[]}}
async function loadAdminAnnouncement(){try{siteAnnouncement=await(await artyFetch('/api/admin/announcement',{headers:authH()})).json();renderSiteAnnouncement()}catch{siteAnnouncement={enabled:false,message:''}}}
async function loadAdminKits(){try{const r=await artyFetch('/api/admin/kits',{headers:authH()});if(!r.ok)throw new Error();allKits=await r.json()}catch{await loadKits()}}
async function loadAdminProductTemplates(){try{const r=await artyFetch('/api/admin/product-templates',{headers:authH()});if(!r.ok)throw new Error();adminProductTemplates=await r.json()}catch{adminProductTemplates=[]}}
...
async function loadAdminData(){
  try{
    ...
    document.getElementById('statRevenue').textContent=`${I18n.currency(toMoney(adminAnalyticsPro?.revenue||0))}`;
...
function updateProductPrice(kitId){
  ...
  const priceEl=document.getElementById('productConfiguredPrice');if(priceEl)priceEl.textContent=`${I18n.currency(toMoney(price))}`;
  const compareEl=document.getElementById('productConfiguredCompare');
  if(compareEl){compareEl.textContent=`${I18n.currency(toMoney(regular))}`;compareEl.style.display=regular>price?'inline':'none'}
  const summary=document.getElementById('productSelectionSummary');
  if(summary){const labels=[selection.size?.label,...selection.addOns.map(option=>option.label)].filter(Boolean);summary.textContent=labels.length?labels.join(' · '):I18n.t('Configuration standard')}
}
function productUiText(){
  const en=I18n.language?.()==='en';
  return en?{
    home:'Home',kits:'Painting kits',back:'Back to kits',popular:'Popular',inStock:'In stock',soldOut:'Sold out',
    chooseSize:'Choose your size',required:'Required',customize:'Customize your kit',optional:'Optional',
    yourPrice:'Your price',selection:'Your selection',quantity:'Quantity',addCart:'Add to cart',buyNow:'Buy now',
    bundle:'Build a bundle with this kit',shipping:'Free shipping',shippingSub:'On orders $75+',secure:'Secure payment',
    secureSub:'Protected checkout',guided:'Guided creation',guidedSub:'Instructions included',checkoutNote:'Shipping and taxes are calculated at checkout.',
    complete:'Complete kit',included:'What’s included',includedIntro:'Everything included with this product is prepared together so you can start creating with confidence.',
    how:'How it works',howIntro:'A simple ARTY experience from selection to creation.',step1:'Choose your options',step1Sub:'Select your preferred size and any extras.',
    step2:'We prepare your kit',step2Sub:'Your order is carefully prepared for delivery.',step3:'Create at your pace',step3Sub:'Follow the included instructions and tutorial.',
    zoom:'Enlarge image',previous:'Previous image',next:'Next image',close:'Close',photo:'Photo'
  }:{
...
function closeProductLightbox(){
  document.getElementById('productImageLightbox')?.classList.remove('open');
...
async function renderProductPage(id,{force=false}={}){
  ...
  if(!kit||force){
    c.innerHTML=productLoadingHTML();
    if(force)allKits=allKits.filter(k=>String(k.id)!==String(id));
  ...
  const thumbs=images.length>1?`<div class="product-thumbs" aria-label="${safeAttr(t.photo)}">${images.map((img,index)=>`<button type="button" class="product-thumb${index===0?' active':''}" data-image="${safeAttr(img)}" data-index="${index}" data-alt="${safeAttr(localizedName)} — ${safeAttr(t.photo)} ${index+1}" onclick="switchProductImage(this)" aria-label="${safeAttr(t.photo)} ${index+1}" aria-pressed="${index===0?'true':'false'}"><img src="${safeAttr(img)}" alt="" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='logoarty.png'"></button>`).join('')}</div>`:'';
  const sizeOptions=sizes.length?`<div class="product-option-group"><div class="product-option-heading"><div><span>${safeText(t.chooseSize)}</span><small>${safeText(t.required)}</small></div></div><div class="product-size-grid">${sizes.map((option,index)=>`<label class="product-size-option"><input type="radio" name="productSize" value="${safeAttr(option.id)}" ${index===0?'checked':''} onchange="updateProductPrice(${kit.id})"><span><strong>${safeText(I18n.field(option,'label')||option.label)}</strong><small>${productChoicePrice(option.priceDelta)}</small></span></label>`).join('')}</div></div>`:'';
  const addOnOptions=addOns.length?`<div class="product-option-group"><div class="product-option-heading"><div><span>${safeText(t.customize)}</span><small>${safeText(t.optional)}</small></div></div><div class="product-addon-list">${addOns.map(option=>`<label class="product-addon-option"><input class="product-addon-input" type="checkbox" value="${safeAttr(option.id)}" onchange="updateProductPrice(${kit.id})"><span class="product-addon-check" aria-hidden="true">✓</span><span class="product-addon-copy"><strong>${safeText(I18n.field(option,'label')||option.label)}</strong>${option.description?`<small>${safeText(I18n.field(option,'description')||option.description)}</small>`:''}</span><b>${productChoicePrice(option.priceDelta)}</b></label>`).join('')}</div></div>`:'';
  const regular=Number(kit.originalPrice??kit.price)||0,price=getKitDisplayPrice(kit),inStock=kit.inStock!==false;
  const includedCard=included.length?`<section class="product-detail-card product-included-card"><div class="product-detail-card-head"><div class="product-detail-icon" aria-hidden="true">✓</div><div><span class="product-detail-kicker">${safeText(t.complete)}</span><h2>${safeText(t.included)}</h2><p>${safeText(t.includedIntro)}</p></div></div><ul>${included.map(item=>`<li>${safeText(item)}</li>`).join('')}</ul></section>`:'';
  const processCard=`<section class="product-detail-card product-process-card"><div class="product-detail-card-head"><div class="product-detail-icon teal" aria-hidden="true">3</div><div><span class="product-detail-kicker">ARTY</span><h2>${safeText(t.how)}</h2><p>${safeText(t.howIntro)}</p></div></div><div class="product-process-steps"><div><b>1</b><span><strong>${safeText(t.step1)}</strong><small>${safeText(t.step1Sub)}</small></span></div><div><b>2</b><span><strong>${safeText(t.step2)}</strong><small>${safeText(t.step2Sub)}</small></span></div><div><b>3</b><span><strong>${safeText(t.step3)}</strong><small>${safeText(t.step3Sub)}</small></span></div></div></section>`;
  c.innerHTML=`
    <nav class="product-breadcrumb" aria-label="Breadcrumb">
      <button type="button" onclick="navigate('#/')">${safeText(t.home)}</button><span>›</span>
      <button type="button" onclick="navigate('#/paintings')">${safeText(t.kits)}</button><span>›</span>
      <span aria-current="page">${safeText(localizedName)}</span>
    </nav>
    <button class="product-back" onclick="navigate('#/paintings')">← ${safeText(t.back)}</button>
    <div class="product-layout product-layout-pro">
      <section class="product-gallery product-gallery-pro">
        <div class="product-main-media">
          <img src="${safeAttr(images[0])}" class="product-main-img" id="pMainImg" data-index="0" alt="${safeAttr(localizedName)} — ${safeAttr(t.photo)} 1" decoding="async" fetchpriority="high" onclick="openProductLightbox()" onerror="this.onerror=null;this.src='logoarty.png'">
          ${images.length>1?`<button class="product-image-nav product-image-prev" type="button" onclick="stepProductImage(-1)" aria-label="${safeAttr(t.previous)}">‹</button><button class="product-image-nav product-image-next" type="button" onclick="stepProductImage(1)" aria-label="${safeAttr(t.next)}">›</button><span class="product-photo-count" id="productPhotoCount">${safeText(t.photo)} 1 / ${images.length}</span>`:''}
          <button class="product-image-zoom" type="button" onclick="openProductLightbox()" aria-label="${safeAttr(t.zoom)}"><span aria-hidden="true">⌕</span><small>${safeText(t.zoom)}</small></button>
        </div>
        ${thumbs}
      </section>
      <section class="product-info product-info-pro">
        <div class="product-meta-line"><div class="product-cat">${safeText(localizedCategory)}</div><span class="product-stock-status ${inStock?'is-in':'is-out'}"><i></i>${safeText(inStock?t.inStock:t.soldOut)}</span></div>
        <div class="product-title-row"><h1>${safeText(localizedName)}</h1>${kit.featured?`<span class="product-popular-mark">${safeText(t.popular)}</span>`:''}</div>
        ${localizedDescription?`<p class="product-desc">${safeText(localizedDescription)}</p>`:''}
        <div class="product-purchase-card">
          <div class="product-price-line"><div><span>${safeText(t.yourPrice)}</span><div class="product-configured-price"><strong id="productConfiguredPrice">${I18n.currency(toMoney(price))}</strong><small id="productConfiguredCompare" style="${regular>price?'':'display:none'}">${I18n.currency(toMoney(regular))}</small></div>${kit.discountLabel?`<em>${safeText(kit.discountLabel)}</em>`:''}</div><div class="product-selection-total"><span>${safeText(t.selection)}</span><strong id="productSelectionSummary">${sizes[0]?safeText(I18n.field(sizes[0],'label')||sizes[0].label):I18n.t('Configuration standard')}</strong></div></div>
          ${sizeOptions}${addOnOptions}
          <div class="product-buy-row"><div class="product-qty-row"><label>${safeText(t.quantity)}</label><div class="qty-ctrl"><button class="qty-btn" onclick="chgQty(-1)" aria-label="-">−</button><input class="qty-val" id="pQty" value="1" readonly aria-label="${safeAttr(t.quantity)}"><button class="qty-btn" onclick="chgQty(1)" aria-label="+">+</button></div></div><button class="btn btn-orange product-add-button" onclick="addToCart(${kit.id})" ${!inStock?'disabled':''}>${safeText(inStock?t.addCart:t.soldOut)}</button></div>
          <button class="btn btn-teal product-buy-now" onclick="buyNow(${kit.id})" ${!inStock?'disabled':''}>${safeText(t.buyNow)} →</button>
          <button class="btn btn-ghost product-bundle-button" onclick="startBundleWithKit(${kit.id})">${safeText(t.bundle)}</button>
          <p class="product-checkout-note">${safeText(t.checkoutNote)}</p>
        </div>
        <div class="product-service-strip"><div>${productServiceIcon('delivery')}<strong>${safeText(t.shipping)}</strong><small>${safeText(t.shippingSub)}</small></div><div>${productServiceIcon('secure')}<strong>${safeText(t.secure)}</strong><small>${safeText(t.secureSub)}</small></div><div>${productServiceIcon('guide')}<strong>${safeText(t.guided)}</strong><small>${safeText(t.guidedSub)}</small></div></div>
      </section>
    </div>
...
function addToCart(kitId){
  ...
  if(sizes.length&&!selection.size){showToast(I18n.t('Choisissez un format'),'error');return false}
  ...
  saveCart();updateCartUI();showToast(I18n.msg`${kit.name} ajouté au panier!`,'success');return true;
}
function configuredKitDetailsHTML(item,className=''){
  const data=item?.customData;
  if(data?.kind==='event-ticket'){
    const date=data.eventDate?new Date(`${data.eventDate}T00:00:00`).toLocaleDateString(I18n.locale(),{day:'numeric',month:'long',year:'numeric'}):I18n.t('Date à confirmer');
    const admissions=Number(item.qty)||1;
    return I18n.html`<div class="configured-kit-details event-ticket-details ${safeAttr(className)}"><span>${safeText(date)} · ${safeText(data.eventTime||I18n.t('Heure à confirmer'))}</span><span>Accès pour ${admissions} personne${admissions>1?'s':''}</span></div>`;
  }
  if(data?.kind==='design-studio')return `<div class="configured-kit-details ${safeAttr(className)}"><span>${safeText(data.productLabel||I18n.t('Création Studio ARTY'))}</span><span>${safeText(data.orientation||'')}</span></div>`;
...
function renderAdminDashboard(){
  ...
  const top=(a.topProducts||[]).map(p=>`<tr><td><strong>${safeText(p.name)}</strong></td><td>${p.qty}</td><td>${I18n.currency(toMoney(p.revenue))}</td></tr>`).join('');
  const low=(a.lowInventory||[]).slice(0,6).map(k=>`<div class="admin-alert-line"><strong>${safeText(I18n.field(k,'name'))}</strong><span>${safeText(I18n.t(k.stockLabel))}</span></div>`).join('')||I18n.html('<p class="admin-muted">Aucune alerte stock.</p>');
  const latest=(a.latestOrders||[]).map(o=>`<div class="admin-order-mini"><strong>${safeText(o.id)}</strong><span>${I18n.currency(toMoney(o.total))} · ${safeText(I18n.status(o.status||''))}</span></div>`).join('')||I18n.html('<p class="admin-muted">Aucune commande.</p>');
  panel.innerHTML=I18n.html`<div class="admin-dashboard-grid"><div class="admin-pro-card big"><div class="admin-card-head"><h3>Ventes des 14 derniers jours</h3><span>${I18n.currency(toMoney(a.revenue||0))} total</span></div><div class="admin-sales-chart">${bars}</div></div><div class="admin-pro-card"><h3>Résumé</h3><div class="admin-kpi-list"><div><span>Aujourd’hui</span><strong>${I18n.currency(toMoney(a.todayRevenue||0))}</strong></div><div><span>Ce mois</span><strong>${I18n.currency(toMoney(a.monthRevenue||0))}</strong></div><div><span>Panier moyen</span><strong>${I18n.currency(toMoney(a.averageOrder||0))}</strong></div><div><span>Rabais donnés</span><strong>${I18n.currency(toMoney(a.discountTotal||0))}</strong></div><div><span>Remboursements</span><strong>${I18n.currency(toMoney(a.refundTotal||0))}</strong></div><div><span>Rabais actifs</span><strong>${a.activeDiscounts||0}</strong></div></div></div><div class="admin-pro-card"><h3>Alertes inventaire</h3>${low}</div><div class="admin-pro-card"><h3>Commandes récentes</h3>${latest}</div><div class="admin-pro-card big"><h3>Meilleurs produits</h3><div class="admin-table-wrap compact"><table class="admin-table"><thead><tr><th>Produit</th><th>Qté</th><th>Ventes</th></tr></thead><tbody>${top||I18n.html('<tr><td colspan="3" class="admin-muted">Aucune vente.</td></tr>')}</tbody></table></div></div></div>`;
}
...
function adminProductRowHTML(kind,item={}){
  if(kind==='images'){const url=item.url||item.value||'';return I18n.html`<div class="admin-repeat-row admin-image-row" data-kind="images"><div class="admin-image-row-preview">${url?I18n.html`<img src="${safeAttr(url)}" alt="Aperçu">`:'<span>Photo</span>'}</div><input class="aKitImageUrl" type="text" value="${safeAttr(url)}" placeholder="Adresse de l’image" oninput="refreshAdminImagePreview(this)"><button type="button" onclick="removeAdminProductRow(this)">Retirer</button></div>`}
  if(kind==='includes')return I18n.html`<div class="admin-repeat-row admin-repeat-simple" data-kind="includes"><input class="aKitIncludeItem" type="text" value="${safeAttr(item.label||item.value||'')}" placeholder="Ex: Toile pré-tracée 11 × 14"><button type="button" onclick="removeAdminProductRow(this)">Retirer</button></div>`;
  const id=item.id||newAdminProductChoiceId(kind==='sizes'?'size':'addon');
  if(kind==='sizes')return I18n.html`<div class="admin-repeat-row admin-repeat-choice" data-kind="sizes" data-choice-id="${safeAttr(id)}"><input class="aKitSizeLabel" type="text" value="${safeAttr(item.label||'')}" placeholder="Format, ex: 11 × 14"><div class="admin-price-input"><span>+$</span><input class="aKitSizePrice" type="number" min="0" step="0.01" value="${Number(item.priceDelta)||0}" aria-label="Supplément du format"></div><button type="button" onclick="removeAdminProductRow(this)">Retirer</button></div>`;
  return I18n.html`<div class="admin-repeat-row admin-repeat-addon" data-kind="addons" data-choice-id="${safeAttr(id)}"><input class="aKitAddonLabel" type="text" value="${safeAttr(item.label||'')}" placeholder="Option, ex: Ajouter un chevalet"><input class="aKitAddonDesc" type="text" value="${safeAttr(item.description||'')}" placeholder="Courte explication optionnelle"><div class="admin-price-input"><span>+$</span><input class="aKitAddonPrice" type="number" min="0" step="0.01" value="${Number(item.priceDelta)||0}" aria-label="Prix de l'option"></div><button type="button" onclick="removeAdminProductRow(this)">Retirer</button></div>`;
}
...
function refreshAdminImagePreview(input){
  ...
  const url=input.value.trim();preview.innerHTML=url?I18n.html`<img src="${safeAttr(url)}" alt="Aperçu">`:'<span>Photo</span>';
}
function readAdminImageFile(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(new Error(I18n.t('Lecture impossible')));reader.readAsDataURL(file)})}
async function uploadAdminProductImages(input){
  ...
  if(files.some(file=>file.size>10*1024*1024)){showToast(I18n.t('Chaque image doit faire moins de 10 Mo'),'error');input.value='';return}
  input.disabled=true;if(status)status.textContent=I18n.msg`Téléversement de ${files.length} image${files.length>1?'s':''}...`;
  try{
    for(const file of files){
      const dataUrl=await readAdminImageFile(file);
      const r=await artyFetch('/api/admin/product-images',{method:'POST',headers:authH(),body:JSON.stringify({fileName:file.name,dataUrl})});
      const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||I18n.t('Téléversement impossible'));
    ...
    if(status)status.textContent=I18n.t('Images ajoutées à la galerie.');showToast(I18n.t('Images téléversées'),'success');
  }catch(err){if(status)status.textContent='';showToast(err.message||I18n.t('Erreur de téléversement'),'error')}
  finally{input.disabled=false;input.value=''}
}
function productTemplateOptionsHTML(selected=''){return I18n.html`<option value="">Choisir un modèle</option>${adminProductTemplates.map(template=>`<option value="${safeAttr(template.id)}" ${String(template.id)===String(selected)?'selected':''}>${safeText(template.name)}</option>`).join('')}`}
...
function applyProductTemplate(){
  ...
  if(!template)return showToast(I18n.t('Choisissez un modèle'),'error');
  setAdminProductRows('includes',template.includes||[]);setAdminProductRows('sizes',template.sizeOptions||[]);setAdminProductRows('addons',template.addOns||[]);showToast(I18n.t('Modèle appliqué'),'success');
}
async function saveProductTemplate(){
  ...
  if(!name)return showToast(I18n.t('Donnez un nom au modèle'),'error');
  if(!details.includes.length&&!details.sizeOptions.length&&!details.addOns.length)return showToast(I18n.t('Ajoutez du contenu avant de créer le modèle'),'error');
  try{const r=await artyFetch('/api/admin/product-templates',{method:'POST',headers:authH(),body:JSON.stringify({name,includes:details.includes,sizeOptions:details.sizeOptions,addOns:details.addOns})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');adminProductTemplates.push(d.template);adminProductTemplates.sort((a,b)=>String(a.name).localeCompare(String(b.name),I18n.locale()));refreshProductTemplateSelect(d.template.id);document.getElementById('aKitTemplateName').value='';showToast(I18n.t('Modèle créé'),'success')}catch{showToast(I18n.t('Erreur'),'error')}
}
...
function renderAdminKits(){
  ...
  const canInventory=currentUser?.role==='admin'||(Array.isArray(currentUser?.permissions)&&currentUser.permissions.includes('inventory'));
  ...
  panel.innerHTML=I18n.html`
    ...
    <div class="admin-form-card admin-product-editor">
      ...
      <section class="admin-product-section"><div class="admin-product-section-title"><span>6</span><div><h4>${canInventory?I18n.t('Inventaire et visibilité'):I18n.t('Visibilité')}</h4><p>${canInventory?I18n.t('Contrôlez la disponibilité dans la boutique.'):I18n.t('Choisissez si ce produit est mis en avant.')}</p></div></div>${canInventory?`<div class="form-row"><div class="form-group"><label>${I18n.t('Inventaire actuel')}</label><input type="number" id="aKitStockQty" min="0" placeholder="${I18n.t('ex: 12')}"></div><div class="form-group"><label>${I18n.t('Seuil stock bas')}</label><input type="number" id="aKitLowStock" min="0" value="3"></div></div><div class="admin-check-row"><label><input type="checkbox" id="aKitStock" checked> ${I18n.t('Visible / vendable')}</label></div>`:''}<div class="admin-check-row"><label><input type="checkbox" id="aKitFeatured"> ${I18n.t('Produit populaire')}</label></div></section>
    ...
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Produit</th><th>Catégorie</th><th>Stock</th><th>Prix</th><th>Actions</th></tr></thead><tbody>${rows||I18n.html('<tr><td colspan="5">Aucun produit.</td></tr>')}</tbody></table></div>`;
...
async function saveKit(){
  ...
  const canInventory=currentUser?.role==='admin'||(Array.isArray(currentUser?.permissions)&&currentUser.permissions.includes('inventory'));
  const p={name:document.getElementById('aKitName').value.trim(),price:document.getElementById('aKitPrice').value,compareAtPrice:document.getElementById('aKitCompare').value,description:document.getElementById('aKitDesc').value,shortDesc:document.getElementById('aKitShortDesc').value,categoryId:parseInt(document.getElementById('aKitCat').value),images:details.images,includes:details.includes,sizeOptions:details.sizeOptions,addOns:details.addOns,featured:document.getElementById('aKitFeatured').checked};
  if(canInventory)Object.assign(p,{stockQty:document.getElementById('aKitStockQty')?.value??'',lowStockThreshold:document.getElementById('aKitLowStock')?.value??'',inStock:document.getElementById('aKitStock')?.checked!==false});
  if(!p.name||!p.price)return showToast(I18n.t('Nom et prix requis'),'error');
  try{const r=await artyFetch(eid?`/api/admin/kits/${eid}`:'/api/admin/kits',{method:eid?'PUT':'POST',headers:authH(),body:JSON.stringify(p)});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');showToast(eid?I18n.t('Produit modifié!'):I18n.t('Produit ajouté!'),'success');await loadAdminData()}catch{showToast(I18n.t('Erreur'),'error')}
}
function editKit(id){
  const k=allKits.find(x=>String(x.id)===String(id));if(!k)return;
  document.getElementById('editKitId').value=k.id;document.getElementById('aKitName').value=k.name||'';document.getElementById('aKitPrice').value=k.originalPrice||k.price||'';document.getElementById('aKitCompare').value=k.compareAtPrice||'';document.getElementById('aKitDesc').value=k.description||'';document.getElementById('aKitShortDesc').value=k.shortDesc||'';document.getElementById('aKitCat').value=k.categoryId||'';const stockQty=document.getElementById('aKitStockQty');if(stockQty)stockQty.value=k.stockQty??'';const lowStock=document.getElementById('aKitLowStock');if(lowStock)lowStock.value=k.lowStockThreshold??3;const stock=document.getElementById('aKitStock');if(stock)stock.checked=k.inStock!==false;document.getElementById('aKitFeatured').checked=!!k.featured;
...
async function saveAnnouncement(){
  ...
  if(enabled&&!message)return showToast(I18n.t('Écrivez un message avant de l’afficher'),'error');
  try{
    const r=await artyFetch('/api/admin/announcement',{method:'PUT',headers:authH(),body:JSON.stringify({message,enabled})});
    const d=await r.json().catch(()=>({}));
    if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');
    siteAnnouncement=d.announcement||{message,enabled};
    renderSiteAnnouncement();renderAdminAnnouncement();showToast(I18n.t('Annonce mise à jour'),'success');
  }catch{showToast(I18n.t('Erreur'),'error')}
}

function renderAdminOrders(){
  const panel=document.getElementById('adminOrdersPanel');if(!panel)return;
  const rows=(adminOrders||[]).map(o=>{const cust=o.customer||{};const itemText=(o.items||[]).map(i=>`${safeText(i.name)} ×${i.qty}${i.discountAmount?` <span class="admin-muted">(-${I18n.currency(toMoney(i.discountAmount))})</span>`:''}`).join('<br>');return I18n.html`<tr><td><strong>${safeText(o.id)}</strong><br><span class="admin-muted">${new Date(o.createdAt).toLocaleDateString(I18n.locale())}</span></td><td>${safeText(cust.name||'')}<br><span class="admin-muted">${safeText(cust.email||o.guestEmail||'')}</span></td><td>${itemText}</td><td><strong>${I18n.currency(toMoney(o.total))}</strong><br>${o.discountTotal?I18n.html`<span class="admin-muted">Rabais: ${I18n.currency(toMoney(o.discountTotal))}</span>`:''}${o.refundedTotal?I18n.html`<span class="admin-muted">Remb.: ${I18n.currency(toMoney(o.refundedTotal))}</span>`:''}</td><td><span class="admin-status ${o.paymentStatus==='paid'?'ok':o.paymentStatus==='cancelled'?'out':'pending'}">${safeText(I18n.status(o.paymentStatus||'pending'))}</span></td><td><select class="admin-status-select" onchange="updateOrderStatus('${safeAttr(o.id)}',this.value)"><option value="en attente de paiement" ${o.status==='en attente de paiement'?'selected':''}>En attente paiement</option><option value="payée" ${o.status==='payée'?'selected':''}>Payée</option><option value="préparation" ${o.status==='préparation'?'selected':''}>Préparation</option><option value="expédiée" ${o.status==='expédiée'?'selected':''}>Expédiée</option><option value="annulée" ${o.status==='annulée'?'selected':''}>Annulée</option><option value="remboursée" ${o.status==='remboursée'?'selected':''}>Remboursée</option></select><div class="admin-actions" style="margin-top:8px"><button class="admin-btn admin-btn-edit" onclick="createRefund('${safeAttr(o.id)}')">Rembourser</button></div></td></tr>`}).join('');
  const refundRows=(adminRefunds||[]).map(r=>`<tr><td><strong>${safeText(r.id)}</strong><br><span class="admin-muted">${new Date(r.createdAt).toLocaleDateString(I18n.locale())}</span></td><td>${safeText(r.orderId)}</td><td>${I18n.currency(toMoney(r.amount))}</td><td>${safeText(r.reason||'')}</td><td><span class="admin-status ${r.status==='manual_refund_logged'?'pending':'ok'}">${safeText(I18n.status(r.status))}</span></td></tr>`).join('');
  panel.innerHTML=I18n.html`<div class="admin-form-card"><h3>Commandes & remboursements</h3><p class="admin-help">Les commandes réduisent l’inventaire automatiquement. Si vous annulez ou remboursez avec retour stock, l’inventaire est remis.</p></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Commande</th><th>Client</th><th>Articles</th><th>Total</th><th>Paiement</th><th>Statut / action</th></tr></thead><tbody>${rows||I18n.html('<tr><td colspan="6" class="admin-muted">Aucune commande pour le moment.</td></tr>')}</tbody></table></div><div class="admin-section-title"><h3>Historique des remboursements</h3><p>Pour l’instant c’est un registre interne. Quand Stripe/Square/Moneris sera branché, l’action pourra déclencher le vrai remboursement.</p></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Remboursement</th><th>Commande</th><th>Montant</th><th>Raison</th><th>Statut</th></tr></thead><tbody>${refundRows||I18n.html('<tr><td colspan="5" class="admin-muted">Aucun remboursement.</td></tr>')}</tbody></table></div>`;
}
async function updateOrderStatus(id,status){try{const r=await artyFetch(`/api/admin/orders/${encodeURIComponent(id)}/status`,{method:'PUT',headers:authH(),body:JSON.stringify({status})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');showToast(I18n.t('Statut mis à jour'),'success');await loadAdminData()}catch{showToast(I18n.t('Erreur'),'error')}}
async function createRefund(orderId){
  const o=adminOrders.find(x=>String(x.id)===String(orderId));if(!o)return;
  if(o.isTest||o.paymentProvider!=='stripe'||!o.paymentReference||o.paymentStatus!=='paid'||Number(o.refundPendingTotal||0)>0)return showToast(I18n.t('Aucun paiement Stripe confirmé à rembourser pour cette commande'),'error');
  const remaining=Math.max(0,Number(o.total||0)-Number(o.refundedTotal||0)-Number(o.refundPendingTotal||0));
  if(remaining<=0)return showToast(I18n.t('Aucun montant remboursable'),'error');
  const raw=prompt(`Montant à rembourser (max ${I18n.currency(toMoney(remaining))})`,toMoney(remaining));if(raw===null)return;
  const amount=Number(raw);if(!Number.isFinite(amount)||amount<=0||amount>remaining)return showToast(I18n.t('Montant de remboursement invalide'),'error');
  const reason=prompt(I18n.t('Raison du remboursement'),I18n.t('Demande client'))||I18n.t('Demande client');
  const full=Math.abs(amount-remaining)<0.01;
  const restock=full?confirm(I18n.t('Remettre les produits physiques en inventaire après confirmation du remboursement Stripe?')):false;
  if(!confirm(I18n.msg`Confirmer le remboursement de ${I18n.currency(toMoney(amount))}? ARTY enverra maintenant le remboursement directement à Stripe et l’argent retournera au mode de paiement original.`))return;
  try{
    const r=await artyFetch(`/api/admin/orders/${encodeURIComponent(orderId)}/refund`,{method:'POST',headers:authH(),body:JSON.stringify({amount,reason,restock})}),d=await r.json().catch(()=>({}));
    if(!r.ok)return showToast(d.error||I18n.t('Remboursement impossible'),'error');
    const pending=['pending','requires_action'].includes(String(d.stripeStatus||''));
    showToast(pending?I18n.t('Remboursement accepté par Stripe et en traitement'):I18n.t('Remboursement effectué dans Stripe'),'success');
    await loadAdminData();
  }catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
...
const customPhotoSizes={petit:{get label(){return I18n.t('Petit 11 x 14')},price:49.99},moyen:{get label(){return I18n.t('Moyen 16 x 20')},price:69.99},grand:{get label(){return I18n.t('Grand 18 x 24')},price:89.99}};

function renderCustomPhotoPage(){
  const c=document.getElementById('customPhotoPageContent'); if(!c) return;
  const sizeCards=Object.entries(customPhotoSizes).map(([key,val])=>`<button class="custom-option-card ${customPhotoState.size===key?'active':''}" onclick="selectCustomPhotoSize('${key}')"><strong>${safeText(val.label)}</strong><span>${I18n.currency(toMoney(val.price))}</span></button>`).join('');
  c.innerHTML=I18n.html`<div class="custom-hero text-center fade-up"><div class="section-tag">Produit personnalisé</div><h2 class="section-heading">Peinture de ta <span class="accent">propre photo</span></h2><p class="section-sub">Téléversez votre photo, choisissez le format de toile et voyez un aperçu avant d’ajouter au panier.</p></div>
    <div class="custom-layout fade-up">
      <section class="custom-builder-card">
        ...
        <div class="custom-price-box"><span>Prix</span><strong>${I18n.currency(toMoney(getCustomPhotoPrice()))}</strong></div>
      ...
      <aside class="custom-preview-card">
        ...
        <div class="canvas-mockup ${customPhotoState.image?'has-image':''}">${customPhotoState.image?I18n.html`<img src="${safeAttr(customPhotoState.image)}" alt="Aperçu">`:I18n.html('<div class="canvas-placeholder"><strong>Ajoutez une photo</strong><span>Le rendu s’affichera ici.</span></div>')}</div>
        <div class="custom-summary-box"><div><span>Produit</span><strong>Tableau personnalisé</strong></div><div><span>Format</span><strong>${safeText(customPhotoSizes[customPhotoState.size].label)}</strong></div><div><span>Prix</span><strong>${I18n.currency(toMoney(getCustomPhotoPrice()))}</strong></div></div>
      </aside>
    </div>`;
  initScrollEffects();
}
...
function addCustomPhotoToCart(goCheckout=false){
  if(!customPhotoState.image) return showToast(I18n.t('Ajoutez une photo avant de continuer'),'error');
  ...
  cart.push({id,name:I18n.msg`Tableau personnalisé (${sizeInfo.label})`,price:sizeInfo.price,image:preview,qty:1,type:'custom-photo',customData:{kind:'photo-canvas',size:customPhotoState.size,sizeLabel:sizeInfo.label,notes:customPhotoState.notes||'',sourceImage:customPhotoState.image}});
  saveCart(); updateCartUI(); showToast(I18n.t('Tableau personnalisé ajouté au panier'),'success');
...
function renderCustomBagPage(){
  ...
  c.innerHTML=I18n.html`<div class="custom-hero text-center fade-up"><div class="section-tag">Produit personnalisé</div><h2 class="section-heading">Créez votre <span class="accent">sac personnalisé</span></h2><p class="section-sub">Ajoutez vos images sur le gabarit du sac, placez-les, redimensionnez-les et voyez le prix final avant de commander.</p></div>
    <div class="custom-layout fade-up custom-layout-bag">
      <section class="custom-builder-card">
        <div class="custom-block">
          ...
          <p class="custom-helper">Prix de base: ${I18n.currency(toMoney(customBagState.basePrice))}. Chaque image ajoutée après la première: +${I18n.currency(toMoney(customBagState.extraImagePrice))}.</p>
        </div>
        <div class="custom-block">
          <label class="custom-label">2. Contrôles</label>
          <div class="custom-controls-panel">
            <div class="custom-control-row"><span>Image sélectionnée</span><strong>${selected?safeText(selected.name||'Image'): I18n.t('Aucune')}</strong></div>
        ...
        <div class="custom-price-box"><span>Prix du sac</span><strong>${I18n.currency(toMoney(getCustomBagPrice()))}</strong></div>
      ...
      <aside class="custom-preview-card bag-preview-card">
        ...
        <div class="custom-summary-box"><div><span>Produit</span><strong>Sac personnalisé</strong></div><div><span>Images</span><strong>${customBagState.items.length}</strong></div><div><span>Prix</span><strong>${I18n.currency(toMoney(getCustomBagPrice()))}</strong></div></div>
      </aside>
    </div>`;
  initScrollEffects();
}
function renderBagLayersHTML(){
  return customBagState.items.map(item=>`<div class="bag-layer ${customBagState.selectedId===item.id?'selected':''}" data-bag-id="${safeAttr(item.id)}" onmousedown="startBagDrag(event,'${safeAttr(item.id)}')" onclick="selectBagItem('${safeAttr(item.id)}')" style="left:${item.x}%;top:${item.y}%;width:${item.size}px;height:${item.size}px;"><img src="${safeAttr(item.src)}" alt="${safeAttr(item.name||I18n.t('Image personnalisée'))}"></div>`).join('') || I18n.html('<div class="bag-placeholder"><strong>Ajoutez vos images</strong><span>Vous pourrez les déplacer et les agrandir sur le sac.</span></div>');
}
...
function buildBagPreviewSvg(){
  ...
  const svg=I18n.html`<svg xmlns="http://www.w3.org/2000/svg" width="900" height="1100" viewBox="0 0 900 1100"><rect width="100%" height="100%" fill="#f8f4ee"/><path d="M250 220c0-90 60-150 200-150s200 60 200 150" fill="none" stroke="#d8b38a" stroke-width="28" stroke-linecap="round"/><path d="M310 220c0-55 38-98 140-98s140 43 140 98" fill="none" stroke="#efdfc9" stroke-width="18" stroke-linecap="round"/><rect x="160" y="220" width="580" height="680" rx="44" fill="#efe3d0" stroke="#d8b38a" stroke-width="8"/><rect x="240" y="320" width="420" height="420" rx="22" fill="#fffdfb" stroke="#eadfce" stroke-dasharray="10 10"/>${images}<text x="450" y="1010" font-family="Outfit, Arial" font-size="40" text-anchor="middle" fill="#5C4F3D">Sac personnalisé Arty</text></svg>`;
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`;
}
function addCustomBagToCart(goCheckout=false){
  if(!customBagState.items.length) return showToast(I18n.t('Ajoutez au moins une image sur le sac'),'error');
  const id=`custom-bag-${Date.now()}`;
  const preview=buildBagPreviewSvg();
  cart.push({id,name:I18n.t('Sac personnalisé'),price:getCustomBagPrice(),image:preview,qty:1,type:'custom-bag',customData:{kind:'bag-design',notes:customBagState.notes||'',imageCount:customBagState.items.length,placements:customBagState.items.map(({name,src,x,y,size})=>({name,src,x,y,size}))}});
  saveCart(); updateCartUI(); showToast(I18n.t('Sac personnalisé ajouté au panier'),'success');
  if(goCheckout) setTimeout(()=>goToCheckout(),200);
}
...
function renderAdminOrders(){
  ...
  const rows=(adminOrders||[]).map(o=>{
    ...
    const itemText=(o.items||[]).map(i=>{
      ...
      return `${safeText(i.name)} ×${i.qty}${custom}${i.discountAmount?` <span class="admin-muted">(-${I18n.currency(toMoney(i.discountAmount))})</span>`:''}`
    }).join('<br>');
  ...
  panel.innerHTML=I18n.html`<div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Commande</th><th>Client</th><th>Articles</th><th>Total</th><th>Paiement</th><th>Gestion</th></tr></thead><tbody>${rows||I18n.html('<tr><td colspan="6" class="admin-muted">Aucune commande.</td></tr>')}</tbody></table></div>`;
}
...
function renderCustomPhotoPage(){
  ...
  const sizeCards=Object.entries(customPhotoSizes).map(([key,val])=>`<button type="button" class="custom-option-card ${customPhotoState.size===key?'active':''}" onclick="selectCustomPhotoSizePro('${key}')"><strong>${safeText(val.label)}</strong><span>${I18n.currency(toMoney(val.price))}</span></button>`).join('');
  c.innerHTML=I18n.html`<div class="custom-hero text-center fade-up"><div class="section-tag">Produit personnalisé</div><h2 class="section-heading">Peinture de ta <span class="accent">propre photo</span></h2><p class="section-sub">La photo est transformée en tracé noir et blanc, sans couleur, pour que le client puisse peindre par-dessus.</p></div>
    <div class="custom-layout fade-up">
      <section class="custom-builder-card pro-custom-panel">
        ...
        <div class="custom-price-box"><span>Prix final</span><strong id="customPhotoPrice">${I18n.currency(toMoney(currentSize.price))}</strong></div>
      ...
      <aside class="custom-preview-card custom-preview-pro">
        ...
        <div class="custom-summary-box"><div><span>Produit</span><strong>Tableau photo à peinturer</strong></div><div><span>Rendu</span><strong>Tracé noir et blanc</strong></div><div><span>Prix</span><strong>${I18n.currency(toMoney(currentSize.price))}</strong></div></div>
      </aside>
    </div>`;
...
function makePhotoTracePreview(dataUrl){
  traceImageToLineArt(dataUrl,700,900,{threshold:traceProOptions.photoThreshold,detail:traceProOptions.photoDetail,transparent:false}).then(url=>{
    ...
    initCustomPhotoCanvas();
  }).catch(()=>showToast(I18n.t('Impossible de lire cette image'),'error'));
}
function addCustomPhotoToCartPro(goCheckout=false){
  if(!customPhotoTraceData)return showToast(I18n.t('Ajoutez une photo avant de continuer'),'error');
  const sizeInfo=customPhotoSizes[customPhotoState.size]||customPhotoSizes.moyen;
  const id=`custom-photo-${Date.now()}`;
  cart.push({id,name:I18n.msg`Tableau personnalisé à peindre (${sizeInfo.label})`,price:sizeInfo.price,image:customPhotoTraceData,qty:1,type:'custom-photo',customData:{kind:'photo-canvas-trace',size:customPhotoState.size,sizeLabel:sizeInfo.label,notes:customPhotoState.notes||'',traceImage:customPhotoTraceData,sourceImage:customPhotoSourceData||customPhotoState.image||''}});
  saveCart(); updateCartUI(); showToast(I18n.t('Tableau personnalisé ajouté au panier'),'success');
  if(goCheckout)setTimeout(()=>goToCheckout(),200);
}
...
function renderCustomBagPage(){
  const c=document.getElementById('customBagPageContent'); if(!c) return;
  c.innerHTML=I18n.html`<div class="custom-hero text-center fade-up"><div class="section-tag">Produit personnalisé</div><h2 class="section-heading">Designer un <span class="accent">sac à peinturer</span></h2><p class="section-sub">Les images sont converties en tracés seulement. Le client reçoit un sac avec lignes à peindre, pas une impression couleur.</p></div>
    <div class="custom-layout fade-up custom-layout-bag">
      <section class="custom-builder-card pro-custom-panel">
        ...
        <div class="custom-price-box"><span>Prix final</span><strong id="customBagPrice">${I18n.currency(toMoney(getCustomBagPrice()))}</strong></div>
      ...
      <aside class="custom-preview-card bag-preview-card custom-preview-pro">
        ...
        <div class="custom-summary-box"><div><span>Produit</span><strong>Sac personnalisé à peindre</strong></div><div><span>Images</span><strong id="bagImageCount">${customBagState.items.length}</strong></div><div><span>Prix</span><strong id="bagSummaryPrice">${I18n.currency(toMoney(getCustomBagPrice()))}</strong></div></div>
      </aside>
    </div>`;
  initBagCanvasEditor();
  initScrollEffects();
...
function drawBagEditor(){
  ...
  ctx.font='700 18px Outfit, Arial';ctx.fillStyle='#b5a997';ctx.textAlign='center';ctx.fillText(I18n.t('ZONE À PEINDRE'),360,295);
...
function syncBagControls(updateSlider=true){
  ...
  if(label)label.textContent=item?(item.name||'Image'):I18n.t('Aucune');
...
function updateBagTraceSetting(key,val){traceProOptions[key]=Number(val); showToast(I18n.t('Le nouveau réglage sera appliqué aux prochaines images'),'success')}
function handleCustomBagUploadPro(event){
  ...
  files.forEach((file,index)=>{
    ...
    reader.onload=async()=>{
      ...
      }catch{showToast(I18n.t('Impossible de convertir une image'),'error')}
    };
    reader.readAsDataURL(file);
  });
  event.target.value='';
...
function renderBundleBuilderPage(){
  const c=document.getElementById('bundleBuilderPageContent');if(!c)return;const t=builderTotals(bundleBuilderState,bundleBuilderState.purpose);const selected=t.items.map(i=>`<div class="builder-summary-line"><span>${safeText(i.kit.name)} ×${i.qty}</span><strong>${I18n.currency(toMoney((getKitDisplayPrice?getKitDisplayPrice(i.kit):i.kit.price)*i.qty))}</strong></div>`).join('')||I18n.html('<p class="builder-muted">Sélectionnez au moins un produit.</p>');
  c.innerHTML=I18n.html`<div class="builder-hero"><div><div class="section-tag">Forfait sur mesure</div><h2 class="section-heading">Créez votre <span class="accent">forfait Arty</span></h2></div><button class="btn btn-ghost" onclick="navigate('#/paintings')">Voir tous les produits</button></div><div class="client-builder-layout"><section class="builder-main-card"><div class="builder-purpose-row"><button class="builder-purpose ${bundleBuilderState.purpose==='group'?'active':''}" onclick="setBundlePurpose('group')">Groupe / amis</button><button class="builder-purpose ${bundleBuilderState.purpose==='event'?'active':''}" onclick="setBundlePurpose('event')">Événement</button><button class="builder-purpose ${bundleBuilderState.purpose==='wedding'?'active':''}" onclick="setBundlePurpose('wedding')">Mariage</button></div><div class="builder-form-row"><label>Nombre de personnes prévu</label><input type="number" min="1" value="${bundleBuilderState.people}" oninput="bundleBuilderState.people=Math.max(1,parseInt(this.value)||1)"></div><div class="builder-section-title"><h3>Choisir les produits</h3><span>${t.qty} kit${t.qty>1?'s':''} sélectionné${t.qty>1?'s':''}</span></div>${kitPickerHTML(bundleBuilderState,'bundle')}<div class="builder-form-row"><label>Texte personnalisé</label><input type="text" maxlength="90" value="${safeAttr(bundleBuilderState.customText||'')}" placeholder="Ex: Mariage Anna & David · 12 juillet 2026" oninput="bundleBuilderState.customText=this.value;renderBundleBuilderPage()"><small>Optionnel. Parfait pour mariages, anniversaires et événements corporatifs.</small></div></section><aside class="builder-summary-card"><div class="summary-art-badge">Arty</div><h3>Résumé du forfait</h3>${selected}<hr><div class="builder-summary-line"><span>Sous-total</span><strong>${I18n.currency(toMoney(t.subtotal))}</strong></div><div class="builder-summary-line discount"><span>${t.rule?safeText(t.rule.label||(I18n.t('Rabais ')+t.percent+'%')):I18n.t('Rabais quantité')}</span><strong>${t.rule?'- $'+toMoney(t.discount):I18n.t('Aucun')}</strong></div>${t.customTextFee?I18n.html`<div class="builder-summary-line"><span>Inscription personnalisée</span><strong>${I18n.currency(toMoney(t.customTextFee))}</strong></div>`:''}<div class="builder-total"><span>Total estimé</span><strong>${I18n.currency(toMoney(t.total))}</strong></div><button class="btn btn-orange" onclick="addClientBundleToCart()" ${!t.qty?'disabled style="opacity:.5"':''}>Ajouter au panier →</button><p class="builder-footnote">Le stock et le paiement sont vérifiés au checkout Stripe.</p></aside></div>`;initScrollEffects();
}
function addClientBundleToCart(goCheckout=false){const t=builderTotals(bundleBuilderState,bundleBuilderState.purpose);if(!t.qty)return showToast(I18n.t('Sélectionnez au moins un produit'),'error');const title=t.qty>=10?I18n.t('Forfait groupe personnalisé'):I18n.t('Forfait personnalisé');const id=`client-bundle-${Date.now()}`;const img=t.items[0]?.kit?.image||'logoarty.png';cart.push({id,name:`${title} (${t.qty} kits)`,price:t.total,image:img,qty:1,type:'custom-bundle',customData:{kind:'client-bundle',purpose:bundleBuilderState.purpose,people:bundleBuilderState.people,customText:bundleBuilderState.customText||'',subtotal:t.subtotal,discount:t.discount,discountRule:t.rule||null,items:t.items.map(i=>({kitId:i.kit.id,name:i.kit.name,qty:i.qty,unitPrice:getKitDisplayPrice?getKitDisplayPrice(i.kit):Number(i.kit.price||0)}))}});saveCart();updateCartUI();showToast(I18n.t('Forfait ajouté au panier'),'success');if(goCheckout)setTimeout(()=>goToCheckout(),250)}
function setEventType(v){eventBuilderState.eventType=v;eventBuilderState.step=2;renderEventBuilderPage()}
function eventTypeLabel(t){return {wedding:I18n.t('Mariage'),birthday:I18n.t('Anniversaire'),corporate:I18n.t('Entreprise / équipe'),family:I18n.t('Famille'),friends:I18n.t('Soirée entre amis'),kids:I18n.t('Enfants / école')}[t]||I18n.t('Événement')}
...
async function saveBundleDeal(){const id=document.getElementById('editBundleDealId')?.value;const payload={label:document.getElementById('bdLabel')?.value,appliesTo:document.getElementById('bdApplies')?.value,minQty:document.getElementById('bdMinQty')?.value,percent:document.getElementById('bdPercent')?.value,customTextFee:document.getElementById('bdTextFee')?.value,active:document.getElementById('bdActive')?.value==='true'};if(!payload.label)return showToast(I18n.t('Nom de règle requis'),'error');try{const r=await artyFetch(id?`/api/admin/bundle-deals/${id}`:'/api/admin/bundle-deals',{method:id?'PUT':'POST',headers:authH(),body:JSON.stringify(payload)});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');showToast(I18n.t('Règle sauvegardée'),'success');await loadAdminBundleDeals();renderAdminBundleDeals()}catch{showToast(I18n.t('Erreur'),'error')}}
async function deleteBundleDeal(id){if(!confirm(I18n.t('Supprimer cette règle?')))return;await artyFetch(`/api/admin/bundle-deals/${id}`,{method:'DELETE',headers:authH()});await loadAdminBundleDeals();renderAdminBundleDeals();showToast(I18n.t('Règle supprimée'),'success')}

// Improve admin order item display for client-created bundles/events
function renderAdminOrders(){
  const panel=document.getElementById('adminOrdersPanel');if(!panel)return;const refundRows=(adminRefunds||[]).map(r=>`<tr><td>${safeText(r.id)}</td><td>${safeText(r.orderId)}</td><td>${I18n.currency(toMoney(r.amount))}</td><td>${safeText(r.reason||'')}</td><td>${safeText(I18n.status(r.status||'noté'))}</td></tr>`).join('');
  const rows=(adminOrders||[]).map(o=>{const cust=o.customer||{};const itemText=(o.items||[]).map(i=>{let details='';if(i.customData?.kind==='configured-kit')details=`<div class="admin-muted">${[i.customData.sizeLabel,...(i.customData.addOnLabels||[])].filter(Boolean).map(safeText).join(' · ')}</div>`;if(i.customData?.kind==='client-bundle')details=I18n.html`<div class="admin-muted">Forfait client · ${(i.customData.items||[]).map(x=>safeText(x.name)+' x'+x.qty).join(', ')}${i.customData.customText?I18n.html('<br>Texte: ')+safeText(i.customData.customText):''}</div>`;if(i.customData?.kind==='event-package')details=I18n.html`<div class="admin-muted">${safeText(i.customData.eventLabel||I18n.t('Événement'))} · ${safeText(i.customData.guests||'')} invités · ${safeText(i.customData.date||I18n.t('date flexible'))}<br>${(i.customData.items||[]).map(x=>safeText(x.name)+' x'+x.qty).join(', ')}${i.customData.customText?I18n.html('<br>Texte souvenir: ')+safeText(i.customData.customText):''}</div>`;if(i.customData?.kind==='photo-canvas-trace')details=`<div class="admin-muted">Format: ${safeText(i.customData.sizeLabel||'')}</div>`;if(i.customData?.kind==='bag-trace-design')details=`<div class="admin-muted">Images: ${safeText(i.customData.imageCount||0)}</div>`;return `${safeText(i.name)} ×${i.qty}${details}${i.discountAmount?` <span class="admin-muted">(-${I18n.currency(toMoney(i.discountAmount))})</span>`:''}`}).join('<br>');return I18n.html`<tr><td><strong>${safeText(o.id)}</strong><br><span class="admin-muted">${new Date(o.createdAt).toLocaleDateString(I18n.locale())}</span></td><td>${safeText(cust.name||'')}<br><span class="admin-muted">${safeText(cust.email||o.guestEmail||'')}</span></td><td>${itemText}</td><td><strong>${I18n.currency(toMoney(o.total))}</strong><br>${o.discountTotal?I18n.html`<span class="admin-muted">Rabais: ${I18n.currency(toMoney(o.discountTotal))}</span>`:''}${o.refundedTotal?I18n.html`<span class="admin-muted">Remb.: ${I18n.currency(toMoney(o.refundedTotal))}</span>`:''}</td><td><span class="admin-status ${o.paymentStatus==='paid'?'ok':o.paymentStatus==='cancelled'?'out':'pending'}">${safeText(I18n.status(o.paymentStatus||'pending'))}</span></td><td><select class="admin-status-select" onchange="updateOrderStatus('${safeAttr(o.id)}',this.value)"><option value="en attente de paiement" ${o.status==='en attente de paiement'?'selected':''}>En attente paiement</option><option value="payée" ${o.status==='payée'?'selected':''}>Payée</option><option value="préparation" ${o.status==='préparation'?'selected':''}>Préparation</option><option value="expédiée" ${o.status==='expédiée'?'selected':''}>Expédiée</option><option value="annulée" ${o.status==='annulée'?'selected':''}>Annulée</option><option value="remboursée" ${o.status==='remboursée'?'selected':''}>Remboursée</option></select><div class="admin-actions" style="margin-top:8px"><button class="admin-btn admin-btn-edit" onclick="createRefund('${safeAttr(o.id)}')">Rembourser</button></div></td></tr>`}).join('');panel.innerHTML=I18n.html`<div class="admin-form-card"><h3>Commandes & remboursements</h3><p class="admin-help">Les forfaits et événements personnalisés apparaissent ici avec les quantités, textes et détails.</p></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Commande</th><th>Client</th><th>Articles</th><th>Total</th><th>Paiement</th><th>Statut / action</th></tr></thead><tbody>${rows||I18n.html('<tr><td colspan="6" class="admin-muted">Aucune commande pour le moment.</td></tr>')}</tbody></table></div><div class="admin-section-title"><h3>Historique des remboursements</h3></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Remboursement</th><th>Commande</th><th>Montant</th><th>Raison</th><th>Statut</th></tr></thead><tbody>${refundRows||I18n.html('<tr><td colspan="5" class="admin-muted">Aucun remboursement.</td></tr>')}</tbody></table></div>`;
}
...
function setBundleCustomText(v){
  ...
  if(p)p.textContent=bundleBuilderState.customText||I18n.t('Votre texte apparaîtra ici');
  updateBuilderLive('bundle');
}
...
function setEventCustomText(v){
  ...
  if(p)p.textContent=eventBuilderState.customText||I18n.t('Ex: Mariage Anna & David · 12 juillet 2026');
  updateBuilderLive('event');
}
...
function kitPickerHTMLPro(state,context){
  ...
  if(!kits.length)return I18n.html('<div class="builder-empty-modern">Aucun kit disponible pour le moment.</div>');
  return `<div class="builder-kit-grid builder-kit-grid-pro">${kits.map(k=>{
    const qty=Number(state.selected[String(k.id)]||0);
    return I18n.html`<article class="builder-kit-card builder-kit-card-pro ${qty?'selected':''}" data-builder-card="${context}-${k.id}">
      ...
      <div class="builder-kit-info"><strong>${safeText(I18n.field(k,'name'))}</strong><span>${kitPriceHTML ? kitPriceHTML(k) : ('$'+toMoney(k.price))}</span></div>
...
function builderSummaryHTML(t,context){
  const selected=t.items.map(i=>`<div class="builder-summary-line"><span>${safeText(i.kit.name)} ×${i.qty}</span><strong>${I18n.currency(toMoney((getKitDisplayPrice?getKitDisplayPrice(i.kit):i.kit.price)*i.qty))}</strong></div>`).join('')||I18n.html('<p class="builder-muted">Aucun produit sélectionné.</p>');
  const label=context==='event'?I18n.t('Rabais événement'):I18n.t('Rabais quantité');
  return I18n.html`${selected}<hr><div class="builder-summary-line"><span>Sous-total</span><strong>${I18n.currency(toMoney(t.subtotal))}</strong></div><div class="builder-summary-line discount"><span>${t.rule?safeText(t.rule.label||label):label}</span><strong>${t.rule?'- $'+toMoney(t.discount):I18n.t('Aucun')}</strong></div>${t.customTextFee?I18n.html`<div class="builder-summary-line"><span>Texte personnalisé</span><strong>${I18n.currency(toMoney(t.customTextFee))}</strong></div>`:''}<div class="builder-total"><span>Total instantané</span><strong>${I18n.currency(toMoney(t.total))}</strong></div>`;
}
...
function renderBundleBuilderPage(){
  ...
  c.innerHTML=I18n.html`<div class="builder-hero builder-hero-polished"><div><div class="section-tag">Forfait sur mesure</div><h2 class="section-heading">Créez votre <span class="accent">forfait Arty</span></h2></div><button type="button" class="btn btn-ghost" onclick="navigate('#/paintings')">Voir les produits</button></div>
...
function builderSummaryHTML(t,context){
  const selected=t.items.map(i=>`<div class="builder-summary-line"><span>${safeText(i.kit.name)} ×${i.qty}</span><strong>${I18n.currency(toMoney(i.lineTotal))}</strong></div>`).join('')||I18n.html('<p class="builder-muted">Aucun produit sélectionné.</p>');
  const label=context==='event'?I18n.t('Rabais événement'):I18n.t('Rabais quantité');
  return I18n.html`${selected}<hr><div class="builder-summary-line"><span>Sous-total</span><strong>${I18n.currency(toMoney(t.subtotal))}</strong></div><div class="builder-summary-line discount"><span>${t.rule?safeText(t.rule.label||label):label}</span><strong>${t.rule?'- $'+toMoney(t.discount):I18n.t('Aucun')}</strong></div>${t.customTextFee?I18n.html`<div class="builder-summary-line"><span>Texte personnalisé</span><strong>${I18n.currency(toMoney(t.customTextFee))}</strong></div>`:''}<div class="builder-total"><span>Total instantané</span><strong>${I18n.currency(toMoney(t.total))}</strong></div>`;
}
function eventNextAction(){
...
function addEventPackageToCart(){
  const purpose=eventBuilderState.eventType==='wedding'?'wedding':'event';
  const t=builderTotals(eventBuilderState,purpose);
  if(!t.qty)return showToast(I18n.t('Choisissez au moins un produit pour l’événement'),'error');
  cart.push({id:`event-package-${Date.now()}`,name:I18n.msg`Événement ${eventTypeLabel(eventBuilderState.eventType)} (${t.qty} kits)`,price:t.total,image:t.items[0]?.kit?.image||'photoacceuil.jpg',qty:1,type:'custom-event-package',customData:{kind:'event-package',eventType:eventBuilderState.eventType,eventLabel:eventTypeLabel(eventBuilderState.eventType),guests:eventBuilderState.guests,date:eventBuilderState.date,location:eventBuilderState.location,hostName:eventBuilderState.hostName||currentUser?.name||'',customText:eventBuilderState.customText||'',notes:eventBuilderState.notes||'',subtotal:t.subtotal,discount:t.discount,customTextFee:t.customTextFee,discountRule:t.rule||null,items:t.items.map(i=>({kitId:i.kit.id,name:i.kit.name,qty:i.qty,unitPrice:i.unitPrice,lineTotal:i.lineTotal}))}});
  saveCart();updateCartUI();showToast(I18n.t('Événement ajouté au panier'),'success');setTimeout(()=>goToCheckout(),250);
}


// ===== COMPLETE CUSTOMER ACCOUNT EXPERIENCE =====
let profileOrders=[],profilePrivateEvents=[],profileBookings=[],profileSupportRequests=[],profileActiveTab='orders',adminSupportRequests=[];
...
function profileDate(value,withTime=false){
  ...
  return date.toLocaleDateString(I18n.locale(),withTime?{day:'numeric',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'}:{day:'numeric',month:'long',year:'numeric'});
}
function profileStatusMeta(status){
  const key=String(status||'').toLowerCase();
  const map={
    'en attente de paiement':{label:I18n.t('En attente de paiement'),className:'pending',step:0},
    'payée':{label:I18n.t('Paiement confirmé'),className:'paid',step:1},
    'préparation':{label:I18n.t('En préparation'),className:'preparing',step:2},
    'expédiée':{label:I18n.t('Expédiée'),className:'shipped',step:3},
    'livrée':{label:I18n.t('Livrée'),className:'delivered',step:4},
    'annulée':{label:I18n.t('Annulée'),className:'cancelled',step:0},
    'remboursée':{label:I18n.t('Remboursée'),className:'refunded',step:0}
  };
  return map[key]||{label:status||I18n.t('Commande reçue'),className:'pending',step:0};
}
function profileOrderProgress(order,compact=false){
  ...
  const steps=[I18n.t('Commande reçue'),I18n.t('Paiement'),I18n.t('Préparation'),I18n.t('Expédition'),I18n.t('Livraison')];
...
async function loadProfileAccount(){
  const r=await artyFetch('/api/users/me',{headers:authH()});if(!r.ok)throw new Error(I18n.t('Compte indisponible'));
  currentUser=await r.json();localStorage.setItem('arty_user',JSON.stringify(currentUser));updateAuthUI();
}
async function loadProfileOrders(){try{const r=await artyFetch('/api/orders/mine',{headers:authH()});if(!r.ok)throw new Error();profileOrders=await r.json()}catch{profileOrders=[]}}
async function loadProfilePrivateEvents(){try{const r=await artyFetch('/api/event-requests/mine',{headers:authH()});if(!r.ok)throw new Error();profilePrivateEvents=await r.json()}catch{profilePrivateEvents=[]}}
async function loadProfileBookings(){try{const r=await artyFetch('/api/bookings/mine',{headers:authH()});if(!r.ok)throw new Error();profileBookings=await r.json()}catch{profileBookings=[]}}
async function loadProfileSupport(){try{const r=await artyFetch('/api/support-requests/mine',{headers:authH()});if(!r.ok)throw new Error();profileSupportRequests=await r.json()}catch{profileSupportRequests=[]}}
...
async function renderProfilePage(){
  ...
  const provider=currentUser.provider==='google'?'Google':I18n.t('Courriel et mot de passe');
  ...
  c.innerHTML=I18n.html`
    ...
    <div class="account-layout">
      <aside class="account-sidebar">
        <div class="account-identity"><div class="account-avatar">${av}</div><strong>${safeText(currentUser.name)}</strong><span>${safeText(currentUser.email)}</span>${memberSince?I18n.html`<small>Membre depuis ${memberSince}</small>`:''}</div>
      ...
      <main class="account-main">
        <section class="account-panel" id="panel-orders" data-account-panel="orders"><div class="account-panel-head"><div><span>Commandes</span><h2>Suivi de vos achats</h2><p>Consultez vos commandes, événements privés, paiements et détails de livraison.</p></div><button type="button" class="btn btn-orange btn-sm" onclick="navigate('#/paintings')">Magasiner</button></div><div class="account-summary-grid" id="profileOrderSummary"></div><div id="ordersWrap" class="account-order-list"><div class="account-loading"></div><div class="account-loading"></div></div><div class="account-private-event-section"><div class="account-card-title">${profileIcon('calendar')}<div><h3>Événements privés & devis</h3><p>Retrouvez vos demandes, créations, devis et paiements d’événements privés.</p></div></div><div id="privateEventPurchasesWrap" class="account-private-event-list"><div class="account-loading"></div></div></div></section>
  ...
  await Promise.all([loadProfileOrders(),loadProfilePrivateEvents(),loadProfileBookings(),loadProfileSupport()]);
...
function profilePrivateEventPathLabel(path){return {inventory:I18n.t('Kits du catalogue'),custom:I18n.t('Création Studio personnalisée'),expert:I18n.t('Accompagnement expert')}[path]||I18n.t('Événement privé')}
function renderProfilePrivateEvents(){
  const wrap=document.getElementById('privateEventPurchasesWrap');if(!wrap)return;
  if(!profilePrivateEvents.length){wrap.innerHTML=I18n.html`<div class="account-empty compact">${profileIcon('calendar')}<h3>Aucun événement privé</h3><p>Vos demandes et devis privés apparaîtront ici.</p></div>`;return}
  wrap.innerHTML=profilePrivateEvents.map(request=>{
    const paymentMeta=privateEventPaymentMeta(request),preview=request.customKit?.paintedPreview||(request.inventoryItems||[])[0]?.image||'logoarty.png';
    return I18n.html`<article class="account-order-card account-private-event-row"><div class="account-order-card-main"><img src="${safeAttr(preview)}" alt=""><div class="account-order-copy"><div class="account-order-topline"><span>${safeText(request.reference||I18n.t('Événement privé'))}</span><span class="account-status ${paymentMeta.className}">${safeText(paymentMeta.label)}</span></div><h3>${safeText(request.eventType||I18n.t('Événement privé'))}</h3><p>${safeText(request.preferredDate||I18n.t('Date à confirmer'))} · ${Number(request.guests)||1} personne${Number(request.guests)>1?'s':''} · ${safeText(profilePrivateEventPathLabel(request.servicePath))}</p></div><div class="account-order-actions">${Number(request.quoteAmount)>0?`<strong>${I18n.currency(toMoney(Number(request.quoteRefundedTotal)>0?request.quoteNetPaid:request.quoteAmount))}</strong>${Number(request.quoteRefundedTotal)>0?`<small class="account-refund-summary">${I18n.t('Remboursé')}: ${I18n.currency(toMoney(request.quoteRefundedTotal))}</small>`:''}`:''}<button type="button" onclick="viewProfilePrivateEvent('${safeAttr(request.id)}')">${safeText(I18n.t('Voir les détails'))}</button></div></div></article>`;
  }).join('');
}
function viewProfilePrivateEvent(eventId){
  const request=profilePrivateEvents.find(item=>String(item.id)===String(eventId)),modal=document.getElementById('profileOrderModal'),detail=document.getElementById('profileOrderDetail');if(!request||!modal||!detail)return;
  const paymentMeta=privateEventPaymentMeta(request),paid=['paid','refund_pending','refunded'].includes(String(request.paymentStatus||'')),address=request.address||{};
  const items=(request.inventoryItems||[]).map(item=>I18n.html`<div class="account-detail-item"><img src="${safeAttr(item.image||'logoarty.png')}" alt=""><div><strong>${safeText(item.name)}</strong><span>Quantité : ${Number(item.quantity)||1}</span></div></div>`).join('');
  const custom=request.customKit?I18n.html`<div class="account-detail-item">${request.customKit.paintedPreview?`<img src="${safeAttr(request.customKit.paintedPreview)}" alt="">`:`<img src="logoarty.png" alt="">`}<div><strong>${safeText(request.customKit.productLabel||I18n.t('Création personnalisée'))}</strong><small>${safeText([request.customKit.sizeLabel,request.customKit.orientation].filter(Boolean).join(' · '))}</small><span>Quantité : ${Number(request.customKit.quantity)||1}</span></div></div>`:'';
  const taxLines=(request.quoteTaxLines||[]).map(line=>I18n.html`<div><span>${safeText(line.label)} ${Number(line.rate)||0}%</span><strong>${I18n.currency(toMoney(line.amount))}</strong></div>`).join('');
  detail.innerHTML=I18n.html`<div class="account-detail-head"><span>${safeText(request.reference||I18n.t('Événement privé'))}</span><h2 id="profileOrderTitle">${safeText(request.eventType||I18n.t('Événement privé'))}</h2><div><small>Demande créée le ${profileDate(request.createdAt)}</small><b class="account-status ${paymentMeta.className}">${safeText(paymentMeta.label)}</b></div></div>
    <div class="account-private-event-facts account-private-event-modal-facts"><div><small>Date</small><strong>${safeText([request.preferredDate,request.eventTime].filter(Boolean).join(' · ')||I18n.t('À confirmer'))}</strong></div><div><small>Participants</small><strong>${Number(request.guests)||1}</strong></div><div><small>Solution</small><strong>${safeText(profilePrivateEventPathLabel(request.servicePath))}</strong></div></div>
    <div class="account-detail-grid"><section><h3>${I18n.t('Détails de l’événement')}</h3><div class="account-detail-items">${items}${custom}${!items&&!custom&&request.expertBrief?`<div class="account-private-event-brief">${safeText(request.expertBrief)}</div>`:''}</div>
    ${request.quoteDescription?`<div class="account-private-event-brief"><strong>${I18n.t('Détails du devis')}</strong><p>${safeText(request.quoteDescription)}</p></div>`:''}
    ${Number(request.quoteAmount)>0?`<div class="account-detail-totals"><div><span>${I18n.t('Sous-total')}</span><strong>${I18n.currency(toMoney(request.quoteSubtotal))}</strong></div><div><span>${I18n.t('Livraison / déplacement')}</span><strong>${I18n.currency(toMoney(request.quoteShipping))}</strong></div>${taxLines}<div class="total"><span>${I18n.t('Total payé')}</span><strong>${I18n.currency(toMoney(request.quoteAmount))}</strong></div>${Number(request.quoteRefundedTotal)>0?`<div class="discount"><span>${I18n.t('Remboursé')}</span><strong>− ${I18n.currency(toMoney(request.quoteRefundedTotal))}</strong></div>`:''}${Number(request.quoteRefundPendingTotal)>0?`<div><span>${I18n.t('Remboursement en cours')}</span><strong>${I18n.currency(toMoney(request.quoteRefundPendingTotal))}</strong></div>`:''}${Number(request.quoteRefundedTotal)>0?`<div class="total"><span>${I18n.t('Montant net')}</span><strong>${I18n.currency(toMoney(request.quoteNetPaid))}</strong></div>`:''}</div>${(request.refunds||[]).length?`<div class="account-private-event-refunds"><strong>${I18n.t('Historique des remboursements')}</strong>${request.refunds.map(refund=>`<div><span>${profileDate(refund.completedAt||refund.createdAt,true)} · ${safeText(refund.reason||I18n.t('Remboursement'))}</span><strong>− ${I18n.currency(toMoney(refund.amount))}</strong></div>`).join('')}</div>`:''}`:''}</section>
    <aside><div class="account-detail-address">${profileIcon('location')}<div><span>${I18n.t('Lieu de l’événement')}</span><strong>${safeText(address.line1||request.location||I18n.t('À confirmer'))}</strong><p>${safeText([address.city,address.province,address.postal].filter(Boolean).join(', '))}<br>${safeText(address.country||'')}</p></div></div>${paid&&request.paidAt?`<div class="account-detail-help"><h3>${safeText(paymentMeta.label)}</h3><p>${I18n.t('Payé le')} ${profileDate(request.paidAt,true)}${Number(request.quoteRefundedTotal)>0?` · ${I18n.t('Remboursé')}: ${I18n.currency(toMoney(request.quoteRefundedTotal))}`:''}</p></div>`:''}</aside></div>`;
...
function renderProfileOrders(){
  ...
  if(!profileOrders.length){wrap.innerHTML=I18n.html`<div class="account-empty compact">${profileIcon('orders')}<h3>Aucune commande de boutique</h3><p>Vos commandes de kits apparaîtront ici.</p></div>`}
  else wrap.innerHTML=profileOrders.map(order=>{const meta=profileStatusMeta(order.status),items=Array.isArray(order.items)?order.items:[],first=items[0]||{},more=Math.max(0,items.length-1);return I18n.html`<article class="account-order-card"><div class="account-order-card-main"><img src="${safeAttr(first.image||'logoarty.png')}" alt=""><div class="account-order-copy"><div class="account-order-topline"><span>Commande ${safeText(order.id)}</span><span class="account-status ${meta.className}">${safeText(meta.label)}</span></div><h3>${safeText(first.name||I18n.t('Commande ARTY'))}${more?I18n.html` <small>+ ${more} autre${more>1?'s':''}</small>`:''}</h3><p>${profileDate(order.createdAt)} · ${items.reduce((sum,item)=>sum+(Number(item.qty)||1),0)} article${items.length>1?'s':''}</p>${profileOrderProgress(order,true)}</div><div class="account-order-actions"><strong>${I18n.currency(toMoney(order.total))}</strong><button type="button" onclick="viewProfileOrder('${safeAttr(order.id)}')">Voir la commande</button></div></div></article>`}).join('');
  renderProfilePrivateEvents();
}
function renderProfileBookings(){
  const wrap=document.getElementById('bookingsWrap');if(!wrap)return;
  if(!profileBookings.length){wrap.innerHTML=I18n.html`<div class="account-empty">${profileIcon('calendar')}<h3>Aucune réservation</h3><p>Réservez une activité ARTY et retrouvez-la ensuite dans votre compte.</p><button class="btn btn-orange" onclick="navigate('#/party')">Voir les événements</button></div>`;return}
  wrap.innerHTML=profileBookings.map(booking=>{const event=booking.event||{};return I18n.html`<article class="account-booking-card"><div class="account-booking-date"><strong>${event.date?new Date(event.date+'T00:00:00').toLocaleDateString(I18n.locale(),{day:'2-digit'}):'—'}</strong><span>${event.date?new Date(event.date+'T00:00:00').toLocaleDateString(I18n.locale(),{month:'short'}):''}</span></div><div><span class="account-booking-label">Réservation ${safeText(I18n.status(booking.status||'confirmée'))}</span><h3>${safeText(event.title||I18n.t('Événement ARTY'))}</h3><p>${safeText(event.time||I18n.t('Heure à confirmer'))} · ${Number(booking.guests)||1} personne${Number(booking.guests)>1?'s':''}</p></div><button type="button" onclick="navigate('#/party')">Voir les événements</button></article>`}).join('');
}
function supportStatusMeta(status){const map={nouvelle:[I18n.t('Nouvelle'),'new'],'en cours':[I18n.t('En cours'),'progress'],'répondue':[I18n.t('Répondue'),'answered'],'fermée':[I18n.t('Fermée'),'closed']};const value=map[String(status||'').toLowerCase()]||[status||I18n.t('Nouvelle'),'new'];return{label:value[0],className:value[1]}}
...
async function submitProfileSupport(event){
  ...
  if(!subject||subject.length<4)return showToast(I18n.t('Ajoutez un objet clair'),'error');if(!message||message.length<10)return showToast(I18n.t('Ajoutez quelques détails'),'error');
  if(button){button.disabled=true;button.textContent=I18n.t('Envoi en cours...')}
  try{const r=await artyFetch('/api/support-requests',{method:'POST',headers:authH(),body:JSON.stringify({topic,orderId,subject,message})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Demande impossible'),'error');document.getElementById('supportSubject').value='';document.getElementById('supportMessage').value='';await loadProfileSupport();renderProfileSupport();showToast(I18n.t('Votre demande a été envoyée'),'success')}catch{showToast(I18n.t('Erreur de connexion'),'error')}finally{if(button){button.disabled=false;button.textContent=I18n.t('Envoyer la demande')}}
}
function openSupportForOrder(orderId){closeProfileOrder();switchProfileTab('support');const select=document.getElementById('supportOrderId');if(select)select.value=orderId;const topic=document.getElementById('supportTopic');if(topic)topic.value='commande';const subject=document.getElementById('supportSubject');if(subject&&!subject.value)subject.value=I18n.msg`Question concernant la commande ${orderId}`;document.getElementById('panel-support')?.scrollIntoView({behavior:'smooth',block:'start'})}
...
function viewProfileOrder(orderId){
  ...
  const itemRows=items.map(item=>I18n.html`<div class="account-detail-item"><img src="${safeAttr(item.image||'logoarty.png')}" alt=""><div><strong>${safeText(item.name)}</strong>${item.customData?.selectionLabel?`<small>${safeText(item.customData.selectionLabel)}</small>`:''}<span>Quantité : ${Number(item.qty)||1}</span></div><b>${I18n.currency(toMoney(item.lineTotal??((Number(item.price)||0)*(Number(item.qty)||1))))}</b></div>`).join('');
  detail.innerHTML=I18n.html`<div class="account-detail-head"><span>Commande ${safeText(order.id)}</span><h2 id="profileOrderTitle">Détails et suivi</h2><div><small>Passée le ${profileDate(order.createdAt)}</small><b class="account-status ${meta.className}">${safeText(meta.label)}</b></div></div><div class="account-detail-progress">${profileOrderProgress(order)}</div>${tracking.number||tracking.carrier||tracking.estimatedDelivery?I18n.html`<section class="account-tracking-card">${profileIcon('truck')}<div><span>Informations de livraison</span><h3>${safeText(tracking.carrier||I18n.t('Transporteur à confirmer'))}</h3>${tracking.number?I18n.html`<p>Numéro de suivi : <strong>${safeText(tracking.number)}</strong></p>`:''}${tracking.estimatedDelivery?I18n.html`<p>Livraison estimée : <strong>${profileDate(tracking.estimatedDelivery)}</strong></p>`:''}</div>${trackingUrl?I18n.html`<a href="${safeAttr(trackingUrl)}" target="_blank" rel="noopener">Suivre le colis</a>`:''}</section>`:''}<div class="account-detail-grid"><section><h3>Articles</h3><div class="account-detail-items">${itemRows}</div><div class="account-detail-totals"><div><span>Sous-total</span><strong>${I18n.currency(toMoney(order.subtotal??order.total))}</strong></div>${Number(order.discountTotal)>0?I18n.html`<div class="discount"><span>Rabais</span><strong>− ${I18n.currency(toMoney(order.discountTotal))}</strong></div>`:''}<div class="total"><span>Total</span><strong>${I18n.currency(toMoney(order.total))}</strong></div></div></section><aside><div class="account-detail-address">${profileIcon('location')}<div><span>Adresse de livraison</span><strong>${safeText(address.line1||I18n.t('Adresse non disponible'))}</strong><p>${safeText([address.city,address.province,address.postal].filter(Boolean).join(', '))}<br>${safeText(address.country||'')}</p></div></div><div class="account-detail-help"><h3>Besoin d’aide?</h3><p>Communiquez avec notre équipe en indiquant cette commande.</p><button type="button" class="btn btn-ghost" onclick="openSupportForOrder('${safeAttr(order.id)}')">Contacter le service à la clientèle</button></div></aside></div>`;
  modal.hidden=false;document.body.style.overflow='hidden';
}
...
async function updateProfile(event){
  ...
  if(!name||name.length<2)return showToast(I18n.t('Entrez votre nom'),'error');if(newPassword&&newPassword!==confirmPassword)return showToast(I18n.t('Les nouveaux mots de passe ne correspondent pas'),'error');if(newPassword&&newPassword.length<6)return showToast(I18n.t('Le mot de passe doit contenir au moins 6 caractères'),'error');
  ...
  if(button){button.disabled=true;button.textContent=I18n.t('Enregistrement...')}
  try{const r=await artyFetch('/api/users/me',{method:'PUT',headers:authH(),body:JSON.stringify(body)});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Modification impossible'),'error');currentUser=d.user;localStorage.setItem('arty_user',JSON.stringify(currentUser));updateAuthUI();profileActiveTab='settings';await renderProfilePage();showToast(I18n.t('Vos informations ont été enregistrées'),'success')}catch{showToast(I18n.t('Erreur de connexion'),'error')}finally{if(button){button.disabled=false;button.textContent=I18n.t('Enregistrer les modifications')}}
}
...
async function loadAdminData(){
  try{await Promise.all([loadAdminEvents(),loadAdminEventOptions(),loadAdminBookings(),loadEventRequests(),loadAdminOrders(),loadAdminAnalytics(),loadAdminDiscounts(),loadAdminRefunds(),loadAdminAnnouncement(),loadAdminKits(),loadAdminProductTemplates(),loadCategories()]);document.getElementById('statRevenue').textContent=`${I18n.currency(toMoney(adminAnalyticsPro?.revenue||0))}`;document.getElementById('statOrders').textContent=adminAnalyticsPro?.ordersCount??(adminOrders||[]).length;document.getElementById('statKits').textContent=allKits.length;document.getElementById('statLowInventory').textContent=adminAnalyticsPro?.lowInventoryCount??0}catch(e){console.warn(e)}
  renderAdminDashboard();renderAdminKits();renderAdminInventory();renderAdminDiscounts();renderAdminOrders();renderAdminCategories();renderAdminEvents();renderAdminAnnouncement();renderAdminEventOptions();
}
...
function renderAdminSupportRequests(){
  ...
  const cards=(adminSupportRequests||[]).map(request=>{const meta=supportStatusMeta(request.status);return I18n.html`<article class="admin-support-card"><div class="admin-support-head"><div><span>${safeText(request.id)}</span><h3>${safeText(request.subject)}</h3><p>${safeText(request.customer?.name||'')} · ${safeText(request.customer?.email||'')}${request.orderId?I18n.msg` · Commande ${safeText(request.orderId)}`:''}</p></div><b class="account-ticket-status ${meta.className}">${safeText(meta.label)}</b></div><div class="admin-support-message"><span>${safeText(I18n.t(request.topic||'autre'))}</span><p>${safeText(request.message)}</p></div><div class="admin-support-response"><div class="form-group"><label>Statut</label><select id="supportStatus-${safeAttr(request.id)}"><option value="nouvelle" ${request.status==='nouvelle'?'selected':''}>Nouvelle</option><option value="en cours" ${request.status==='en cours'?'selected':''}>En cours</option><option value="répondue" ${request.status==='répondue'?'selected':''}>Répondue</option><option value="fermée" ${request.status==='fermée'?'selected':''}>Fermée</option></select></div><div class="form-group"><label>Réponse au client</label><textarea id="supportReply-${safeAttr(request.id)}" placeholder="Écrivez la réponse visible dans le compte client">${safeText(request.adminReply||'')}</textarea></div><button class="btn btn-teal btn-sm" onclick="saveAdminSupportRequest('${safeAttr(request.id)}')">Enregistrer la réponse</button></div></article>`}).join('');
  panel.innerHTML=I18n.html`<div class="admin-form-card"><h3>Service à la clientèle</h3><p class="admin-help">Gérez les demandes envoyées depuis les comptes clients et publiez vos réponses.</p></div><div class="admin-support-list">${cards||I18n.html('<div class="admin-form-card"><p class="admin-muted">Aucune demande pour le moment.</p></div>')}</div>`;
}
async function saveAdminSupportRequest(id){const status=document.getElementById(`supportStatus-${id}`)?.value,adminReply=document.getElementById(`supportReply-${id}`)?.value.trim()||'';try{const r=await artyFetch(`/api/admin/support-requests/${encodeURIComponent(id)}`,{method:'PATCH',headers:authH(),body:JSON.stringify({status,adminReply})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');await loadAdminSupportRequests();renderAdminSupportRequests();showToast(I18n.t('Demande mise à jour'),'success')}catch{showToast(I18n.t('Erreur'),'error')}}
function adminOrderDomId(id){return String(id||'').replace(/[^a-z0-9_-]/gi,'-')}
function adminOrderStatusOptions(current){return [['en attente de paiement',I18n.t('En attente de paiement')],['payée',I18n.t('Payée')],['préparation',I18n.t('En préparation')],['expédiée',I18n.t('Expédiée')],['livrée',I18n.t('Livrée')],['annulée',I18n.t('Annulée')],['remboursée',I18n.t('Remboursée')]].map(([value,label])=>`<option value="${value}" ${current===value?'selected':''}>${label}</option>`).join('')}
function adminPaymentMeta(status){const key=String(status||'pending').toLowerCase(),map={paid:[I18n.t('Paiement confirmé'),'ok'],pending:[I18n.t('En attente'),'pending'],cancelled:[I18n.t('Annulé'),'out'],refund_needed:[I18n.t('Remboursement requis'),'pending'],refund_pending:[I18n.t('Remboursement en cours'),'pending'],refunded:[I18n.t('Remboursé'),'out'],provider_not_connected:[I18n.t('Paiement à confirmer'),'pending']};const value=map[key]||[status||I18n.t('À confirmer'),'pending'];return{label:value[0],className:value[1]}}
function adminOrderItemDetails(item){
  const custom=item.customData||{},details=[];
  if(custom.kind==='event-ticket'){
    if(custom.eventDate)details.push(I18n.msg`Date : ${custom.eventDate}${custom.eventTime?I18n.msg` à ${custom.eventTime}`:''}`);
    if(custom.eventLocation)details.push(I18n.msg`Lieu : ${custom.eventLocation}`);
    (custom.guestNames||[]).forEach((name,index)=>details.push(I18n.msg`Participant ${index+1} : ${name}`));
  }
  if(custom.selectionLabel)details.push(custom.selectionLabel);
  if(custom.sizeLabel&&!details.includes(custom.sizeLabel))details.push(I18n.msg`Format : ${custom.sizeLabel}`);
  (custom.addOnLabels||[]).forEach(label=>details.push(I18n.msg`Option : ${label}`));
  if(custom.kind==='client-bundle'){
    (custom.items||[]).forEach(product=>details.push(`${product.name||I18n.t('Produit')} × ${Number(product.qty)||1}`));
    if(custom.customText)details.push(I18n.msg`Texte personnalisé : ${custom.customText}`);
  }
  if(custom.kind==='event-package'){
    if(custom.eventLabel)details.push(custom.eventLabel);
    if(custom.guests)details.push(I18n.msg`${custom.guests} invités`);
    if(custom.date)details.push(I18n.msg`Date : ${custom.date}`);
    if(custom.location)details.push(I18n.msg`Lieu : ${custom.location}`);
    (custom.items||[]).forEach(product=>details.push(`${product.name||I18n.t('Produit')} × ${Number(product.qty)||1}`));
    if(custom.customText)details.push(I18n.msg`Texte personnalisé : ${custom.customText}`);
  }
  if(custom.kind==='bag-trace-design'&&custom.imageCount)details.push(I18n.msg`${custom.imageCount} image${Number(custom.imageCount)>1?'s':''} personnalisée${Number(custom.imageCount)>1?'s':''}`);
  if(custom.kind==='design-studio'){
    details.push(I18n.msg`Création Studio ARTY · ${custom.productLabel||I18n.t('Produit personnalisé')}`);
    if(custom.orientation)details.push(I18n.msg`Orientation : ${custom.orientation}`);
    if(custom.elementCount)details.push(I18n.msg`${custom.elementCount} élément${Number(custom.elementCount)>1?'s':''} · ${Number(custom.imageCount)||0} image${Number(custom.imageCount)>1?'s':''} · ${Number(custom.textCount)||0} texte${Number(custom.textCount)>1?'s':''}`);
  }
  if(custom.notes)details.push(I18n.msg`Note : ${custom.notes}`);
  if(item.discountLabel)details.push(I18n.msg`Rabais : ${item.discountLabel}`);
  return details;
}
function adminDesignStudioAssets(item){
  const custom=item?.customData||{};if(custom.kind!=='design-studio')return'';
  return `<div class="admin-order-design-previews">${custom.paintedPreview?I18n.html`<a href="${safeAttr(custom.paintedPreview)}" download="apercu-peint-arty.jpg"><img src="${safeAttr(custom.paintedPreview)}" alt="Aperçu peint"><span>Télécharger l’aperçu peint</span></a>`:''}${custom.productionTrace?I18n.html`<a href="${safeAttr(custom.productionTrace)}" download="trace-production-haute-resolution-arty.png"><img src="${safeAttr(custom.productionTrace)}" alt="Tracé de production haute résolution"><span>Télécharger le tracé haute résolution</span></a>`:custom.tracePreview?I18n.html`<a href="${safeAttr(custom.tracePreview)}" download="trace-production-arty.png"><img src="${safeAttr(custom.tracePreview)}" alt="Tracé de production"><span>Télécharger le tracé de production</span></a>`:''}${custom.colorArtwork?I18n.html`<a href="${safeAttr(custom.colorArtwork)}" download="reference-couleur-arty.jpg"><img src="${safeAttr(custom.colorArtwork)}" alt="Référence couleur"><span>Télécharger la référence couleur</span></a>`:''}</div>`;
}
function renderAdminOrders(){
  const panel=document.getElementById('adminOrdersPanel');if(!panel)return;
  ...
  const refundRows=(adminRefunds||[]).map(refund=>{const status=String(refund.providerStatus||refund.status||'').toLowerCase(),cls=status==='succeeded'?'ok':status==='failed'||status==='canceled'||status==='legacy_not_sent'?'out':'pending',label=status==='succeeded'?I18n.t('Remboursement effectué'):status==='pending'||status==='requires_action'?I18n.t('En traitement Stripe'):status==='legacy_not_sent'?I18n.t('Ancien registre — non envoyé à Stripe'):status==='failed'?I18n.t('Échec du remboursement'):I18n.status(status||'noté');return `<tr><td>${safeText(refund.id)}</td><td>${safeText(refund.orderId)}</td><td>${I18n.currency(toMoney(refund.amount))}</td><td>${safeText(refund.reason||'')}</td><td><span class="admin-status ${cls}">${safeText(label)}</span>${refund.stripeRefundId?`<small class="admin-order-cell-note">${safeText(refund.stripeRefundId)}</small>`:''}</td></tr>`}).join('');
  const realOrders=(adminOrders||[]).filter(order=>!order.isTest),testOrders=(adminOrders||[]).filter(order=>!!order.isTest);
  const awaiting=realOrders.filter(order=>['payée','préparation'].includes(order.status)).length,shipping=realOrders.filter(order=>order.status==='expédiée').length;
  const rows=(adminOrders||[]).map(order=>{
    ...
    return I18n.html`<tr class="admin-order-row ${order.isTest?'is-test-order':''}"><td><button type="button" class="admin-order-number" onclick="openAdminOrderDetail('${safeAttr(order.id)}')"><span class="admin-order-id-line"><strong>${safeText(order.id)}</strong>${order.isTest?'<em class="admin-test-badge">TEST</em>':''}</span><span>${profileDate(order.createdAt)}</span></button></td><td><strong>${safeText(customer.name||I18n.t('Client non indiqué'))}</strong><span class="admin-order-cell-note">${safeText(customer.email||order.guestEmail||I18n.t('Courriel non indiqué'))}</span></td><td><strong>${safeText(ticketOnly?I18n.t('Billet électronique'):address.city||I18n.t('Adresse à consulter'))}</strong><span class="admin-order-cell-note">${safeText(ticketOnly?I18n.t('Envoi après paiement'):[address.province,address.postal].filter(Boolean).join(' · '))}</span></td><td><strong>${quantity} ${ticketOnly?`personne${quantity>1?'s':''}`:`article${quantity>1?'s':''}`}</strong><span class="admin-order-cell-note">${safeText(items[0]?.name||I18n.t('Aucun article'))}${items.length>1?` + ${items.length-1}`:''}</span></td><td><strong>${I18n.currency(toMoney(order.total))}</strong><span class="admin-status ${payment.className}">${safeText(payment.label)}</span></td><td><span class="account-status ${meta.className}">${safeText(meta.label)}</span></td><td><button type="button" class="admin-order-view" onclick="openAdminOrderDetail('${safeAttr(order.id)}')">Voir les détails</button></td></tr>`;
  }).join('');
  panel.innerHTML=I18n.html`<div class="admin-orders-heading"><div><span>Gestion des commandes</span><h3>Commandes et livraisons</h3><p>Les commandes marquées TEST restent visibles ici, mais elles sont exclues du chiffre d’affaires, du panier moyen, des meilleures ventes et des statistiques marketing. Les remboursements des paiements Stripe sont envoyés directement à Stripe depuis ARTY.</p><div class="admin-test-order-actions">${realOrders.length?`<button type="button" class="btn btn-ghost btn-sm" onclick="markAllCurrentOrdersAsTest()">Réinitialiser les statistiques de test</button>`:''}${testOrders.length?`<span>${testOrders.length} commande${testOrders.length>1?'s':''} test exclue${testOrders.length>1?'s':''} du tableau de bord</span>`:''}</div></div><div class="admin-orders-overview"><div><strong>${realOrders.length}</strong><span>Commandes réelles</span></div><div><strong>${testOrders.length}</strong><span>Tests</span></div><div><strong>${awaiting}</strong><span>À préparer</span></div><div><strong>${shipping}</strong><span>En livraison</span></div></div></div><div class="admin-table-wrap admin-orders-table"><table class="admin-table"><thead><tr><th>Commande</th><th>Client</th><th>Destination</th><th>Produits</th><th>Total</th><th>Statut</th><th></th></tr></thead><tbody>${rows||I18n.html('<tr><td colspan="7" class="admin-muted">Aucune commande pour le moment.</td></tr>')}</tbody></table></div><div class="admin-section-title"><h3>Historique des remboursements</h3></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Remboursement</th><th>Commande</th><th>Montant</th><th>Raison</th><th>Statut</th></tr></thead><tbody>${refundRows||I18n.html('<tr><td colspan="5" class="admin-muted">Aucun remboursement.</td></tr>')}</tbody></table></div><div class="admin-order-detail-modal" id="adminOrderDetailModal" hidden><button class="admin-order-detail-backdrop" type="button" onclick="closeAdminOrderDetail()" aria-label="Fermer les détails"></button><section class="admin-order-detail-sheet" role="dialog" aria-modal="true" aria-labelledby="adminOrderDetailTitle"><button class="admin-order-detail-close" type="button" onclick="closeAdminOrderDetail()" aria-label="Fermer">×</button><div id="adminOrderDetailContent"></div></section></div>`;
}
async function refreshAdminSalesAfterTestChange(){
  await Promise.all([loadAdminOrders(),loadAdminAnalytics()]);
  const revenue=document.getElementById('statRevenue'),orders=document.getElementById('statOrders');
  if(revenue)revenue.textContent=I18n.currency(toMoney(adminAnalyticsPro?.revenue||0));
  if(orders)orders.textContent=adminAnalyticsPro?.ordersCount??0;
  renderAdminDashboard();renderAdminOrders();
}
async function setAdminOrderTestFlag(id,isTest){
  const order=(adminOrders||[]).find(item=>String(item.id)===String(id));if(!order)return;
  const message=isTest
    ? I18n.t('Marquer cette commande comme TEST? Elle restera enregistrée mais ne comptera plus dans les statistiques.')
    : I18n.t('Compter cette commande comme une vente réelle? Elle sera réintégrée aux statistiques.');
  if(!confirm(message))return;
  try{
    const response=await artyFetch(`/api/admin/orders/${encodeURIComponent(id)}/test`,{method:'PATCH',headers:authH(),body:JSON.stringify({isTest})});
    const data=await response.json().catch(()=>({}));if(!response.ok)return showToast(data.error||I18n.t('Modification impossible'),'error');
    closeAdminOrderDetail();await refreshAdminSalesAfterTestChange();showToast(isTest?I18n.t('Commande exclue des statistiques'):I18n.t('Commande réintégrée aux statistiques'),'success');
  }catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
async function markAllCurrentOrdersAsTest(){
  const realCount=(adminOrders||[]).filter(order=>!order.isTest).length;if(!realCount)return;
  if(!confirm(I18n.msg`Marquer les ${realCount} commandes actuelles comme TEST et réinitialiser les statistiques de ventes? Utilisez cette option seulement si ces commandes sont vos essais.`))return;
  try{
    const response=await artyFetch('/api/admin/orders/mark-all-test',{method:'POST',headers:authH(),body:JSON.stringify({scope:'all'})});
    const data=await response.json().catch(()=>({}));if(!response.ok)return showToast(data.error||I18n.t('Modification impossible'),'error');
    await refreshAdminSalesAfterTestChange();showToast(I18n.msg`${data.count||realCount} commandes marquées comme TEST`,'success');
  }catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
function openAdminOrderDetail(orderId){
  ...
  const customer=order.customer||{},address=order.address||{},items=Array.isArray(order.items)?order.items:[],ticketOnly=items.length>0&&items.every(item=>item.type==='event-ticket'),tracking=order.tracking||{},statusMeta=profileStatusMeta(order.status),payment=adminPaymentMeta(order.paymentStatus),domId=adminOrderDomId(order.id),email=customer.email||order.guestEmail||'',phone=customer.phone||'',phoneHref=String(phone).replace(/[^\d+]/g,''),remaining=Math.max(0,Number(order.total||0)-Number(order.refundedTotal||0)-Number(order.refundPendingTotal||0));
  const itemRows=items.map(item=>{const qty=Number(item.qty)||1,details=adminOrderItemDetails(item),unit=Number(item.originalUnitPrice??item.price??0),line=Number(item.lineTotal??(unit*qty)),eventTicket=item.type==='event-ticket',studioItem=item.customData?.kind==='design-studio',typeLabel=eventTicket?I18n.t('Billet'):studioItem?I18n.t('Création personnalisée'):item.type==='bundle'?I18n.t('Forfait'):I18n.t('Produit');return `<article class="admin-order-detail-item ${studioItem?'admin-order-detail-studio-item':''}"><img src="${safeAttr(item.image||'logoarty.png')}" alt=""><div><span>${safeText(typeLabel)}</span><h4>${safeText(item.name||I18n.t('Produit ARTY'))}</h4><p>${eventTicket?`Accès pour ${qty} personne${qty>1?'s':''} · Prix par personne : ${I18n.currency(toMoney(unit))}`:`Quantité : ${qty} · Prix unitaire : ${I18n.currency(toMoney(unit))}`}</p>${details.length?`<ul>${details.map(detail=>`<li>${safeText(detail)}</li>`).join('')}</ul>`:''}${adminDesignStudioAssets(item)}</div><strong>${I18n.currency(toMoney(line))}</strong></article>`}).join('');
  const quantityLabel=ticketOnly?I18n.t('Personnes admises'):I18n.t('Nombre d’articles');
  ...
  const paymentProviderLabel=order.paymentProvider==='stripe'?'Stripe':order.paymentProvider&&order.paymentProvider!=='not_connected'?order.paymentProvider:I18n.t('À confirmer');
  content.innerHTML=I18n.html`<header class="admin-order-detail-head"><div><span>Commande ${safeText(order.id)}</span><h2 id="adminOrderDetailTitle">Détails de la commande</h2><p>Reçue le ${profileDate(order.createdAt,true)}</p></div><div>${order.isTest?'<b class="admin-test-badge admin-test-badge-large">TEST</b>':''}<b class="account-status ${statusMeta.className}">${safeText(statusMeta.label)}</b><b class="admin-status ${payment.className}">${safeText(payment.label)}</b></div></header><div class="admin-order-detail-summary"><div>${profileIcon('receipt')}<span>Total de la commande<strong>${I18n.currency(toMoney(order.total))}</strong></span></div><div>${profileIcon('orders')}<span>${quantityLabel}<strong>${items.reduce((total,item)=>total+(Number(item.qty)||1),0)}</strong></span></div><div>${profileIcon('user')}<span>Type de commande<strong>${order.checkoutMode==='account'?I18n.t('Compte client'):I18n.t('Commande invité')}</strong></span></div></div><div class="admin-order-detail-columns"><main><section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${profileIcon('user')}<h3>Informations du client</h3></div><span>Pour communiquer au besoin</span></div><div class="admin-customer-info"><div><span>Nom complet</span><strong>${safeText(customer.name||I18n.t('Non indiqué'))}</strong></div><div><span>Courriel</span><strong>${safeText(email||I18n.t('Non indiqué'))}</strong></div><div><span>Téléphone</span><strong>${safeText(phone||I18n.t('Non indiqué'))}</strong></div></div><div class="admin-contact-actions">${email?I18n.html`<a href="mailto:${safeAttr(email)}">Envoyer un courriel</a>`:''}${phoneHref?I18n.html`<a href="tel:${safeAttr(phoneHref)}">Appeler le client</a>`:''}</div></section><section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${profileIcon('location')}<h3>Adresse de livraison</h3></div><button type="button" onclick="copyAdminShippingAddress('${safeAttr(order.id)}')">Copier l’adresse</button></div><address><strong>${safeText(customer.name||'')}</strong><span>${safeText(address.line1||I18n.t('Adresse non indiquée'))}</span><span>${safeText([address.city,address.province,address.postal].filter(Boolean).join(', '))}</span><span>${safeText(address.country||'')}</span></address>${address.notes?I18n.html`<div class="admin-shipping-note"><strong>Instructions de livraison</strong><p>${safeText(address.notes)}</p></div>`:''}</section><section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${profileIcon('orders')}<h3>Produits commandés</h3></div><span>${items.length} produit${items.length>1?'s':''}</span></div><div class="admin-order-detail-items">${itemRows||I18n.html('<p class="admin-muted">Aucun produit enregistré.</p>')}</div><div class="admin-order-detail-totals"><div><span>Sous-total</span><strong>${I18n.currency(toMoney(order.subtotal??order.total))}</strong></div>${Number(order.discountTotal)>0?I18n.html`<div class="discount"><span>Rabais</span><strong>− ${I18n.currency(toMoney(order.discountTotal))}</strong></div>`:''}${Number(order.refundedTotal)>0?I18n.html`<div><span>Montant remboursé</span><strong>− ${I18n.currency(toMoney(order.refundedTotal))}</strong></div>`:''}${Number(order.refundPendingTotal)>0?I18n.html`<div><span>Remboursement en cours</span><strong>${I18n.currency(toMoney(order.refundPendingTotal))}</strong></div>`:''}<div class="total"><span>Total</span><strong>${I18n.currency(toMoney(order.total))}</strong></div></div></section></main><aside><section class="admin-order-detail-card admin-order-management"><div class="admin-order-detail-title"><div>${profileIcon('truck')}<h3>Statut et suivi</h3></div></div><div class="form-group"><label>Statut de la commande</label><select id="detailStatus-${domId}">${adminOrderStatusOptions(order.status)}</select></div><div class="form-group"><label>Transporteur</label><input id="detailCarrier-${domId}" value="${safeAttr(tracking.carrier||'')}" placeholder="Ex: Postes Canada"></div><div class="form-group"><label>Numéro de suivi</label><input id="detailNumber-${domId}" value="${safeAttr(tracking.number||'')}" placeholder="Numéro de suivi"></div><div class="form-group"><label>Livraison estimée</label><input id="detailDate-${domId}" type="date" value="${safeAttr(tracking.estimatedDelivery||'')}"></div><div class="form-group"><label>Lien de suivi</label><input id="detailUrl-${domId}" value="${safeAttr(tracking.url||'')}" placeholder="https://..."></div><button class="btn btn-teal" type="button" id="detailSave-${domId}" onclick="saveAdminOrderDetail('${safeAttr(order.id)}','${safeAttr(domId)}')">Enregistrer les changements</button></section><section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${profileIcon('receipt')}<h3>Paiement</h3></div></div><div class="admin-payment-details"><div><span>Statut</span><strong>${safeText(payment.label)}</strong></div><div><span>Mode</span><strong>${safeText(paymentProviderLabel)}</strong></div><div><span>Environnement</span><strong>${safeText(order.isTest?I18n.t('Test / Sandbox'):I18n.t('Réel'))}</strong></div>${order.paymentReference?I18n.html`<div><span>Référence</span><strong>${safeText(order.paymentReference)}</strong></div>`:''}</div><div class="admin-test-order-control"><strong>${order.isTest?I18n.t('Commande de test'):I18n.t('Vente réelle')}</strong><p>${order.isTest?I18n.t('Cette commande est conservée pour référence, mais elle ne compte pas dans les statistiques.'):I18n.t('Cette commande compte dans le chiffre d’affaires et les statistiques du tableau de bord.')}</p><button type="button" class="btn btn-ghost btn-sm" onclick="setAdminOrderTestFlag('${safeAttr(order.id)}',${order.isTest?'false':'true'})">${order.isTest?I18n.t('Compter comme vente réelle'):I18n.t('Marquer comme commande TEST')}</button></div>${remaining>0&&!order.isTest&&order.paymentProvider==='stripe'&&!!order.paymentReference&&order.paymentStatus==='paid'&&Number(order.refundPendingTotal||0)<=0?I18n.html`<button class="admin-refund-button" type="button" onclick="closeAdminOrderDetail();createRefund('${safeAttr(order.id)}')">Effectuer un remboursement</button>`:''}${order.paymentStatus==='pending'?I18n.html`<p class="admin-muted" style="margin-top:12px">Aucun paiement n’a encore été encaissé. Cette commande ne peut pas être remboursée. Annulez-la si le client a abandonné le paiement.</p>`:''}</section>${history?I18n.html`<section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${profileIcon('receipt')}<h3>Historique</h3></div></div><div class="admin-order-history">${history}</div></section>`:''}</aside></div>`;
...
function ticketStatusMeta(status){
  ...
  if(value==='cancelled')return {label:I18n.t('Billet annulé'),className:'cancelled'};
  return value==='checked_in'?{label:I18n.t('Entrée confirmée'),className:'checked'}:{label:I18n.t('Billet valide'),className:'valid'};
}

async function renderPublicTicket(code){
  const container=document.getElementById('ticketPageContent');if(!container)return;
  container.innerHTML=I18n.html('<div class="ticket-loading"><span></span><p>Chargement du billet...</p></div>');
  try{
    const response=await artyFetch(`/api/tickets/${encodeURIComponent(code)}`),data=await response.json().catch(()=>({}));
    if(!response.ok)throw new Error(data.error||I18n.t('Billet non trouvé'));
    const ticket=data.ticket||{},event=data.event||{},booking=data.booking||{},admissions=Math.max(1,Number(ticket.admissions)||1),eventCancelled=event.status==='cancelled',ticketCancelled=ticket.status==='cancelled',meta=eventCancelled?{label:I18n.t('Événement annulé'),className:'cancelled'}:ticketStatusMeta(ticket.status);
    ...
    const emailNotice=lastTicketEmailStatus==='sent'
      ? I18n.html('<div class="ticket-delivery-note success"><strong>Billets envoyés par courriel</strong><span>Une copie a été envoyée à l’adresse utilisée pour la réservation.</span></div>')
      : lastTicketEmailStatus
        ? I18n.html('<div class="ticket-delivery-note warning"><strong>Votre billet est prêt</strong><span>Gardez cette page accessible. L’envoi par courriel n’a pas pu être complété.</span></div>')
        : '';
    container.innerHTML=I18n.html`<button type="button" class="product-back ticket-back" onclick="navigate('#/party')">← Retour aux événements</button>
      ${eventCancelled?I18n.html('<div class="ticket-delivery-note cancelled"><strong>Événement annulé</strong><span>Communiquez avec l’équipe ARTY pour les prochaines étapes.</span></div>'):ticketCancelled?I18n.html('<div class="ticket-delivery-note cancelled"><strong>Billet annulé</strong><span>Ce billet ne peut plus être utilisé à l’entrée.</span></div>'):emailNotice}
      <div class="public-ticket-layout">
        <section class="public-ticket-card ${meta.className}">
          ...
          <div class="public-ticket-event"><span>${safeText(eventDate)} · ${safeText(event.time||I18n.t('Heure à confirmer'))}</span><h1>${safeText(event.title||I18n.t('Événement ARTY'))}</h1><p>${safeText(event.location||I18n.t('Lieu à confirmer'))}</p></div>
          <div class="public-ticket-holder"><span>Acheteur</span><strong>${safeText(ticket.holderName||booking.name||I18n.t('Client ARTY'))}</strong><small>Accès pour ${admissions} personne${admissions>1?'s':''}</small></div>
          ...
          ${ticket.checkedInAt?I18n.html`<footer class="public-ticket-checkin">Entrée confirmée le ${profileDate(ticket.checkedInAt,true)}</footer>`:''}
        </section>
        <aside class="public-ticket-info"><span>Votre arrivée</span><h2>Tout est prêt pour l’événement</h2><div><strong>Date et heure</strong><p>${safeText(eventDate)}<br>${safeText(event.time||I18n.t('Heure à confirmer'))}</p></div><div><strong>Lieu</strong><p>${safeText(event.location||I18n.t('Lieu à confirmer'))}</p></div>${event.hostNote?I18n.html`<div><strong>Information importante</strong><p>${safeText(event.hostNote)}</p></div>`:''}<p class="public-ticket-help">Vous pouvez aussi retrouver tous vos billets dans votre profil si vous avez réservé en étant connecté.</p><button class="btn btn-ghost" type="button" onclick="window.print()">Imprimer le billet</button></aside>
      </div>`;
    lastTicketEmailStatus='';
  }catch(error){
    container.innerHTML=I18n.html`<div class="ticket-not-found"><span>Billet</span><h1>Ce billet est introuvable</h1><p>${safeText(error.message||I18n.t('Vérifiez le lien reçu par courriel.'))}</p><button class="btn btn-orange" onclick="navigate('#/party')">Voir les événements</button></div>`;
  }
}

function renderProfileBookings(){
  const wrap=document.getElementById('bookingsWrap');if(!wrap)return;
  if(!profileBookings.length){wrap.innerHTML=I18n.html`<div class="account-empty">${profileIcon('calendar')}<h3>Aucune réservation</h3><p>Réservez une activité ARTY et retrouvez vos billets ici.</p><button class="btn btn-orange" onclick="navigate('#/party')">Voir les événements</button></div>`;return}
  wrap.innerHTML=profileBookings.map(booking=>{
    ...
    const day=event.date?new Date(`${event.date}T00:00:00`).toLocaleDateString(I18n.locale(),{day:'2-digit'}):'—',month=event.date?new Date(`${event.date}T00:00:00`).toLocaleDateString(I18n.locale(),{month:'short'}):'';
    return I18n.html`<article class="account-booking-card account-ticket-booking"><div class="account-booking-date"><strong>${safeText(day)}</strong><span>${safeText(month)}</span></div><div class="account-booking-copy"><span class="account-booking-label">Accès pour ${admissions||Number(booking.guests)||1} personne${(admissions||Number(booking.guests)||1)>1?'s':''} · ${emailSent?I18n.t('Courriel envoyé'):I18n.t('Disponible dans votre compte')}</span><h3>${safeText(event.title||I18n.t('Événement ARTY'))}</h3><p>${safeText(event.time||I18n.t('Heure à confirmer'))} · ${safeText(event.location||I18n.t('Lieu à confirmer'))}</p><div class="account-ticket-progress"><span style="width:${admissions?Math.round((checked/admissions)*100):0}%"></span></div><small>${checked?I18n.msg`Entrée confirmée pour ${checked} personne${checked>1?'s':''}`:I18n.t('Billet prêt à être présenté')}</small></div><button type="button" onclick="openProfileTickets('${safeAttr(booking.id)}')">Voir le billet</button></article>`;
  }).join('');
}
...
async function checkInAdminTicket(ticketId,checkedIn){
  try{const response=await artyFetch(`/api/admin/tickets/${encodeURIComponent(ticketId)}`,{method:'PATCH',headers:authH(),body:JSON.stringify({checkedIn})}),data=await response.json().catch(()=>({}));if(!response.ok)return showToast(data.error||I18n.t('Validation impossible'),'error');await loadAdminBookings();renderAdminEvents();showToast(checkedIn?I18n.t('Entrée confirmée'):I18n.t('Validation annulée'),'success')}catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
async function submitTicketScan(event){
  event?.preventDefault();const input=document.getElementById('adminTicketScan'),code=input?.value.trim();if(!code)return showToast(I18n.t('Scannez ou entrez un code de billet'),'error');
  try{const response=await artyFetch('/api/admin/tickets/check-in',{method:'POST',headers:authH(),body:JSON.stringify({code})}),data=await response.json().catch(()=>({}));if(!response.ok)return showToast(data.error||I18n.t('Billet invalide'),'error');adminGuestEventId=String(data.record?.event?.id||adminGuestEventId);await loadAdminBookings();renderAdminEvents();showToast(I18n.msg`${data.record?.ticket?.holderName||I18n.t('Billet')} : entrée confirmée`,'success');setTimeout(()=>document.getElementById('adminTicketScan')?.focus(),30)}catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
async function resendAdminTickets(bookingId){
  const button=Array.from(document.querySelectorAll('[data-resend-booking]')).find(element=>element.dataset.resendBooking===String(bookingId));if(button){button.disabled=true;button.textContent=I18n.t('Envoi...')}
  try{const response=await artyFetch(`/api/admin/bookings/${encodeURIComponent(bookingId)}/resend-ticket`,{method:'POST',headers:authH()}),data=await response.json().catch(()=>({}));if(!response.ok)return showToast(data.error||I18n.t('Courriel non envoyé'),'error');await loadAdminBookings();renderAdminEvents();showToast(I18n.t('Billets renvoyés par courriel'),'success')}catch{showToast(I18n.t('Erreur de connexion'),'error')}finally{if(button){button.disabled=false;button.textContent=I18n.t('Renvoyer le courriel')}}
}
...
function renderAdminEvents(){
  ...
  const eventCards=(adminEvents||[]).map(event=>{const counts=adminEventTicketCounts(event.id),capacity=Number(event.maxSpots)||0,pct=capacity?Math.min(100,Math.round((counts.total/capacity)*100)):0;return I18n.html`<article class="admin-managed-event ${String(event.id)===String(adminGuestEventId)?'selected':''}"><img src="${safeAttr(event.image||'photoacceuil.jpg')}" alt=""><div><span>${safeText(event.date?formatEventDate(event,true):I18n.t('Date à confirmer'))} · ${safeText(event.time||'')}</span><h4>${safeText(I18n.field(event,'title'))}</h4><p>${safeText(event.location||I18n.t('Lieu à confirmer'))}</p><div class="admin-event-capacity"><span style="width:${pct}%"></span></div><small>${counts.total} personne${counts.total>1?'s':''} · ${counts.checked} présence${counts.checked>1?'s':''} · ${safeText(I18n.status(event.status||'published'))}</small></div><div class="admin-managed-actions"><button type="button" onclick="selectAdminGuestEvent('${safeAttr(event.id)}')">Liste d’invités</button><button type="button" onclick="openAdminEventEditor(${Number(event.id)})">Modifier</button><button type="button" class="danger" onclick="deleteEv(${Number(event.id)})">Supprimer</button></div></article>`}).join('');
  const guestRows=selectedBookings.flatMap(booking=>(booking.tickets||[]).map(ticket=>{const checked=ticket.status==='checked_in',cancelled=ticket.status==='cancelled',admissions=Math.max(1,Number(ticket.admissions)||1),search=[ticket.holderName,booking.name,booking.email,booking.phone,ticket.code].join(' ').toLowerCase();return I18n.html`<tr data-guest-search="${safeAttr(search)}"><td><strong>${safeText(ticket.holderName)}</strong><span>${safeText(ticket.code)}</span><small>Accès pour ${admissions} personne${admissions>1?'s':''}</small></td><td><strong>${safeText(booking.name)}</strong><span>${safeText(booking.email)}${booking.phone?` · ${safeText(booking.phone)}`:''}</span></td><td><span class="admin-ticket-state ${cancelled?'cancelled':checked?'checked':'valid'}">${cancelled?I18n.t('Annulé'):checked?I18n.t('Présent'):I18n.t('À venir')}</span>${checked&&ticket.checkedInAt?`<small>${profileDate(ticket.checkedInAt,true)}</small>`:''}</td><td>${cancelled?I18n.html('<span class="admin-ticket-unavailable">Aucune entrée</span>'):`<button class="admin-checkin-button ${checked?'undo':''}" type="button" onclick="checkInAdminTicket('${safeAttr(ticket.id)}',${checked?'false':'true'})">${checked?I18n.msg`Annuler (${admissions})`:I18n.msg`Confirmer ${admissions} entrée${admissions>1?'s':''}`}</button>`}</td></tr>`})).join('');
  const selectedCounts=selectedEvent?adminEventTicketCounts(selectedEvent.id):{total:0,checked:0};
  const bookingContacts=selectedBookings.map(booking=>I18n.html`<article class="admin-booking-contact"><div><strong>${safeText(booking.name)}</strong><span>${safeText(booking.email)}${booking.phone?` · ${safeText(booking.phone)}`:''}</span><small>Accès pour ${booking.guests} personne${booking.guests>1?'s':''} · Total ${I18n.currency(toMoney(booking.total))}</small></div><div><span class="admin-email-state ${booking.emailDelivery?.status==='sent'?'sent':'attention'}">${booking.emailDelivery?.status==='sent'?I18n.t('Courriel envoyé'):I18n.t('Courriel à vérifier')}</span><button type="button" data-resend-booking="${safeAttr(booking.id)}" onclick="resendAdminTickets('${safeAttr(booking.id)}')">Renvoyer le courriel</button></div></article>`).join('');
  const requestRows=(eventRequests||[]).map(request=>I18n.html`<tr><td><strong>${safeText(request.name)}</strong><br><small>${safeText(request.email)}${request.phone?` · ${safeText(request.phone)}`:''}</small></td><td>${safeText(request.eventType)}<br><small>${request.preferredDate?safeText(request.preferredDate):I18n.t('Date flexible')} · ${Number(request.guests)||'?'} pers.</small></td><td>${safeText(request.location||I18n.t('À confirmer'))}</td><td><span class="admin-status-badge">${safeText(I18n.status(request.status||'nouvelle'))}</span></td><td><div class="admin-actions"><button class="admin-btn admin-btn-edit" onclick="updateEventRequestStatus(${Number(request.id)},'contactée')">Contactée</button><button class="admin-btn admin-btn-delete" onclick="deleteEventRequest(${Number(request.id)})">Supprimer</button></div></td></tr>`).join('');
  panel.innerHTML=I18n.html`<div class="admin-events-heading"><div><span>Billetterie et événements</span><h3>Gestion des invités</h3><p>Gérez les événements, retrouvez chaque participant et confirmez les entrées à partir de la même section.</p></div><button class="btn btn-orange" type="button" onclick="document.getElementById('adminEventEditor').open=true;document.getElementById('adminEventEditor').scrollIntoView({behavior:'smooth'})">Créer un événement</button></div>
    ${!transactionalEmailConfigured?I18n.html('<div class="admin-ticket-config"><strong>Courriels automatiques à configurer</strong><p>Ajoutez RESEND_API_KEY, EMAIL_FROM et ARTY_PUBLIC_URL dans Render pour activer les courriels de compte, commandes, suivi et billets.</p></div>'):''}
    ...
    <section class="admin-event-list"><div class="admin-section-title"><h3>Événements</h3><p>Sélectionnez un événement pour ouvrir sa liste d’invités.</p></div><div class="admin-managed-events">${eventCards||I18n.html('<div class="admin-empty-panel">Aucun événement créé.</div>')}</div></section>
    <section class="admin-guest-manager" id="adminGuestManager"><div class="admin-guest-head"><div><span>Contrôle des entrées</span><h3>${safeText(I18n.field(selectedEvent,'title')||I18n.t('Liste d’invités'))}</h3><p>${selectedEvent?`${safeText(formatEventDate(selectedEvent,true))} · ${safeText(selectedEvent.time||'')} · ${safeText(selectedEvent.location||I18n.t('Lieu à confirmer'))}`:I18n.t('Créez un événement pour commencer.')}</p></div><div class="admin-guest-totals"><strong>${selectedCounts.checked}<small>/ ${selectedCounts.total}</small></strong><span>présences</span></div></div>
      <div class="admin-guest-tools"><select aria-label="Événement" onchange="selectAdminGuestEvent(this.value)">${adminEvents.map(event=>`<option value="${safeAttr(event.id)}" ${String(event.id)===String(adminGuestEventId)?'selected':''}>${safeText(I18n.field(event,'title'))} · ${safeText(event.date||'')}</option>`).join('')}</select><input type="search" placeholder="Rechercher un nom, courriel ou billet" oninput="filterAdminGuestList(this.value)"><form onsubmit="submitTicketScan(event)"><input id="adminTicketScan" autocomplete="off" placeholder="Scanner ou entrer le code"><button type="submit">Valider</button></form></div>
...
function eventPathLabel(path){return {inventory:I18n.t('Choisir nos kits'),custom:I18n.t('Créer un kit personnalisé'),expert:I18n.t('Concevoir avec un expert')}[path]||I18n.t('À confirmer')}
function eventPathDescription(path){return {inventory:I18n.t('Sélectionnez les modèles et les quantités désirées dans le catalogue actuel.'),custom:I18n.t('Créez librement votre toile ou votre sac dans le Studio ARTY, puis joignez le design à votre demande.'),expert:I18n.t('Expliquez votre idée et notre équipe vous aidera à concevoir la bonne expérience.')}[path]||''}
...
function setEventGuests(value){eventBuilderState.guests=Math.max(1,Math.min(1000,parseInt(value)||1));if(eventBuilderState.customKit)eventBuilderState.customKit.quantity=eventBuilderState.guests;if(eventStudioDraftState)eventStudioDraftState.quantity=eventBuilderState.guests;if(designStudioContext==='event')designStudioState.quantity=eventBuilderState.guests;const input=document.getElementById('eventGuestsValue');if(input)input.value=eventBuilderState.guests;updateEventQuoteSummary()}
function adjustEventGuests(delta){setEventGuests((Number(eventBuilderState.guests)||1)+delta)}
...
function setEventCustomQuantity(value){eventBuilderState.customKit=eventBuilderState.customKit||{};eventBuilderState.customKit.quantity=Math.max(1,Math.min(1000,parseInt(value)||1));if(eventStudioDraftState)eventStudioDraftState.quantity=eventBuilderState.customKit.quantity;if(designStudioContext==='event')designStudioState.quantity=eventBuilderState.customKit.quantity;const input=document.getElementById('eventCustomQuantity');if(input)input.value=eventBuilderState.customKit.quantity;updateEventQuoteSummary()}
function setEventCustomNotes(value){eventBuilderState.customKit=eventBuilderState.customKit||{};eventBuilderState.customKit.notes=String(value||'');if(eventStudioDraftState)eventStudioDraftState.notes=eventBuilderState.customKit.notes;if(designStudioContext==='event')designStudioState.notes=eventBuilderState.customKit.notes}
...
function eventCustomSizeInfo(){return eventStudioDesign?.sizeLabel?{label:eventStudioDesign.sizeLabel}:customPhotoSizes[eventBuilderState.customKit?.size]||customPhotoSizes.moyen}
...
function eventQuoteSolutionSummary(){
  if(eventBuilderState.servicePath==='inventory'){
    ...
    return items.length?items.map(item=>`<div><span>${safeText(item.kit.name)}</span><strong>${item.qty}</strong></div>`).join(''):I18n.html('<p>Aucun kit sélectionné pour le moment.</p>');
  }
  if(eventBuilderState.servicePath==='custom')return I18n.html`<div><span>Produit</span><strong>${safeText(eventStudioDesign?.productLabel||I18n.t('À créer dans le Studio'))}</strong></div><div><span>Quantité</span><strong>${Number(eventBuilderState.customKit?.quantity)||eventBuilderState.guests}</strong></div><div><span>Design</span><strong>${eventStudioDesign?I18n.t('Joint à la demande'):eventStudioDraftState?.elements?.length?I18n.t('Brouillon en cours'):I18n.t('À créer')}</strong></div>`;
  return `<p>${eventBuilderState.expertBrief?safeText(eventBuilderState.expertBrief).slice(0,180):I18n.t('Décrivez votre concept pour guider notre expert.')}</p>`;
}
function eventQuoteSummaryHTML(){
  const address=eventBuilderState.address||{};
  return I18n.html`<div class="event-quote-summary-head"><span>Votre projet</span><h3>${safeText(eventBuilderState.eventName||I18n.t('Événement à créer'))}</h3></div><div class="event-quote-summary-facts"><div>${eventExperienceIcon('people')}<span>Invités<strong>${Number(eventBuilderState.guests)||1}</strong></span></div><div>${eventExperienceIcon('calendar')}<span>Date<strong>${eventBuilderState.date?safeText(eventBuilderState.date):I18n.t('À confirmer')}</strong></span></div><div>${eventExperienceIcon('location')}<span>Lieu<strong>${safeText(address.city||address.line1||I18n.t('À confirmer'))}</strong></span></div></div><div class="event-quote-summary-choice"><span>Solution choisie</span><strong>${safeText(eventPathLabel(eventBuilderState.servicePath))}</strong>${eventQuoteSolutionSummary()}</div><div class="event-quote-no-charge"><strong>Aucun paiement maintenant</strong><p>ARTY vérifiera votre demande, confirmera le prix avec vous et créera un lien de paiement seulement après votre accord.</p></div>`;
}
...
function eventQuoteKitPickerHTML(){
  ...
  if(!kits.length)return I18n.html('<div class="builder-empty-modern">Aucun kit disponible pour le moment.</div>');
  return `<div class="event-quote-kit-grid">${kits.map(kit=>{const qty=Number(eventBuilderState.selected[String(kit.id)]||0);return I18n.html`<article class="event-quote-kit ${qty?'selected':''}" data-builder-card="event-${kit.id}"><button type="button" class="event-quote-kit-select" onclick="changeBuilderQty('event',${kit.id},1)" aria-label="Ajouter ${safeAttr(kit.name)}"></button><img src="${safeAttr(kit.image||'logoarty.png')}" alt="${safeAttr(kit.name)}"><div><strong>${safeText(I18n.field(kit,'name'))}</strong><small>${safeText(kit.shortDesc||I18n.t('Kit ARTY'))}</small></div><div class="builder-qty builder-qty-modern"><button type="button" onclick="event.stopPropagation();changeBuilderQty('event',${kit.id},-1)">−</button><b data-builder-qty="event-${kit.id}">${qty}</b><button type="button" onclick="event.stopPropagation();changeBuilderQty('event',${kit.id},1)">+</button></div></article>`}).join('')}</div>`;
}
function updateBuilderLive(context){
...
function validateEventQuoteStep(step){
  const address=eventBuilderState.address||{};
  if(step===1){
    if(!String(eventBuilderState.eventName||'').trim())return showToast(I18n.t('Décrivez le type d’événement souhaité'),'error'),false;
    if(!String(eventBuilderState.hostName||'').trim())return showToast(I18n.t('Entrez le nom du contact'),'error'),false;
    if(!validateEmail(eventBuilderState.email))return showToast(I18n.t('Entrez un courriel valide'),'error'),false;
    if(!String(address.line1||'').trim())return showToast(I18n.t('Entrez l’adresse ou le lieu de l’événement'),'error'),false;
  }
  if(step===3){
    if(eventBuilderState.servicePath==='inventory'&&!eventQuoteSelectedItems().length)return showToast(I18n.t('Choisissez au moins un kit'),'error'),false;
    if(eventBuilderState.servicePath==='custom'&&(!eventStudioDesign||!eventCustomTraceData))return showToast(I18n.t('Terminez votre création dans le Studio ARTY'),'error'),false;
    if(eventBuilderState.servicePath==='expert'&&String(eventBuilderState.expertBrief||'').trim().length<10)return showToast(I18n.t('Décrivez votre idée pour notre expert'),'error'),false;
  }
  return true;
}
...
async function submitEventQuoteRequest(){
  const items=eventQuoteSelectedItems(),sizeInfo=eventCustomSizeInfo(),payload={name:eventBuilderState.hostName,email:eventBuilderState.email,phone:eventBuilderState.phone,eventName:eventBuilderState.eventName,preferredDate:eventBuilderState.date,eventTime:eventBuilderState.eventTime,guests:eventBuilderState.guests,address:eventBuilderState.address,servicePath:eventBuilderState.servicePath,contactPreference:eventBuilderState.contactPreference,notes:eventBuilderState.notes,inventoryItems:items.map(item=>({kitId:item.kit.id,quantity:item.qty})),customKit:eventBuilderState.servicePath==='custom'?{size:eventStudioDesign?.size||eventBuilderState.customKit?.size,sizeLabel:sizeInfo.label,quantity:Number(eventBuilderState.customKit?.quantity)||eventBuilderState.guests,notes:eventBuilderState.customKit?.notes||'',productType:eventStudioDesign?.productType||'',productLabel:eventStudioDesign?.productLabel||'',orientation:eventStudioDesign?.orientation||'',imageCount:eventStudioDesign?.imageCount||0,textCount:eventStudioDesign?.textCount||0,elementCount:eventStudioDesign?.elementCount||0,paintedPreview:eventStudioDesign?.paintedPreview||'',sourceImage:eventCustomSourceData,traceImage:eventCustomTraceData}:null,expertBrief:eventBuilderState.expertBrief};
  const button=document.getElementById('eventFinalActionBtn');if(button){button.disabled=true;button.textContent=I18n.t('Envoi en cours...')}
  try{const response=await artyFetch('/api/event-requests',{method:'POST',headers:{'Content-Type':'application/json',...(currentUser?authH():{})},body:JSON.stringify(payload)}),data=await response.json().catch(()=>({}));if(!response.ok)return showToast(data.error||I18n.t('Impossible d’envoyer la demande'),'error');eventQuoteConfirmation={reference:data.reference,emailStatus:data.emailStatus};renderEventBuilderPage();window.scrollTo({top:0,behavior:'smooth'})}catch(error){console.error(error);showToast(I18n.t('Erreur lors de l’envoi de la demande'),'error')}finally{if(button){button.disabled=false;button.textContent=I18n.t('Demander mon devis')}}
}
function submitBuiltEventRequest(){if(validateEventQuoteStep(3))submitEventQuoteRequest()}
function resetEventQuoteBuilder(){eventBuilderState={step:1,eventName:'',guests:10,date:'',eventTime:'',address:{line1:'',city:'',province:'QC',postal:'',country:'Canada'},hostName:currentUser?.name||'',email:currentUser?.email||'',phone:currentUser?.phone||'',notes:'',contactPreference:'email',servicePath:'inventory',selected:{},customKit:{size:'moyen',quantity:10,notes:''},expertBrief:''};eventCustomSourceData='';eventCustomTraceData='';eventStudioDesign=null;eventStudioDraftState=null;eventQuoteConfirmation=null;renderEventBuilderPage();window.scrollTo({top:0,behavior:'smooth'})}

/* ===== EVENT QUOTE PAYMENT PAGE ===== */
async function renderEventQuotePaymentPage(token){
  const container=document.getElementById('eventQuotePageContent');if(!container)return;currentEventQuoteToken=String(token||'');container.innerHTML=I18n.html('<div class="event-quote-loading"><span></span><p>Chargement de votre devis sécurisé...</p></div>');
  try{const response=await artyFetch(`/api/event-quotes/${encodeURIComponent(currentEventQuoteToken)}`),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||I18n.t('Devis introuvable'));currentEventQuote=data;renderEventQuotePaymentContent()}catch(error){container.innerHTML=I18n.html`<div class="event-quote-error"><h2>Lien indisponible</h2><p>${safeText(error.message)}</p><a class="btn btn-orange" href="#/party">Voir les événements</a></div>`}
}
function eventQuotePriceBreakdownHTML(quote){
  const taxLines=(quote.quoteTaxLines||[]).map(line=>`<div><span>${safeText(line.label)} ${Number(line.rate)||0}%</span><strong>${I18n.currency(toMoney(line.amount))}</strong></div>`).join('');
  const refunded=Number(quote.quoteRefundedTotal||0),pending=Number(quote.quoteRefundPendingTotal||0),net=Number(quote.quoteNetPaid??Math.max(0,Number(quote.quoteAmount||0)-refunded));
  return `<div class="event-quote-price-breakdown"><div><span>${I18n.t('Sous-total')}</span><strong>${I18n.currency(toMoney(quote.quoteSubtotal??quote.quoteAmount))}</strong></div><div><span>${I18n.t('Livraison / déplacement')}</span><strong>${I18n.currency(toMoney(quote.quoteShipping||0))}</strong></div>${taxLines}<div class="total"><span>${I18n.t('Total payé')}</span><strong>${I18n.currency(toMoney(quote.quoteAmount))} CAD</strong></div>${refunded>0?`<div><span>${I18n.t('Remboursé')}</span><strong>− ${I18n.currency(toMoney(refunded))}</strong></div>`:''}${pending>0?`<div><span>${I18n.t('Remboursement en cours')}</span><strong>${I18n.currency(toMoney(pending))}</strong></div>`:''}${refunded>0?`<div class="total"><span>${I18n.t('Montant net')}</span><strong>${I18n.currency(toMoney(net))} CAD</strong></div>`:''}</div>`;
}
function renderEventQuotePaymentContent(){
  ...
  if(['paid','refund_pending','refunded'].includes(String(quote.paymentStatus||''))){const refunded=quote.quoteRefundStatus==='refunded'||quote.paymentStatus==='refunded',partial=quote.quoteRefundStatus==='partial_refund'||Number(quote.quoteRefundedTotal)>0;container.innerHTML=I18n.html`<div class="event-payment-success"><span>${eventExperienceIcon('check')}</span><div class="section-tag">${refunded?I18n.t('Remboursement confirmé'):partial?I18n.t('Paiement ajusté'):I18n.t('Paiement confirmé')}</div><h1>${refunded?I18n.t('Votre paiement a été remboursé.'):I18n.t('Merci, votre événement est confirmé.')}</h1><p>${refunded?I18n.t('Le remboursement a été envoyé au mode de paiement original.'):I18n.msg`Nous avons reçu votre paiement pour la demande ${safeText(quote.reference)}.`}</p>${eventQuotePriceBreakdownHTML(quote)}<div>${refunded?I18n.t('Le délai d’apparition du remboursement dépend de votre institution financière.'):I18n.t('Notre équipe communiquera avec vous pour finaliser la production et les détails de l’événement.')}</div><a href="#/profile" class="btn btn-orange">Voir dans mon compte</a></div>`;return}
  const items=(quote.inventoryItems||[]).map(item=>I18n.html`<div class="event-payment-item">${item.image?`<img src="${safeAttr(item.image)}" alt="">`:''}<span><strong>${safeText(item.name)}</strong><small>Quantité ${Number(item.quantity)||1}</small></span></div>`).join('');
  const custom=quote.customKit?I18n.html`<div class="event-payment-item"><span><strong>${safeText(quote.customKit.productLabel||I18n.t('Kit personnalisé'))}</strong><small>${safeText([quote.customKit.sizeLabel,quote.customKit.orientation].filter(Boolean).join(' · ')||I18n.t('Format à confirmer'))} · ${Number(quote.customKit.quantity)||1} exemplaire(s)</small></span></div>`:'';
  container.innerHTML=I18n.html`<div class="event-payment-shell"><header><a href="#/" class="event-payment-brand"><img src="logoarty.png" alt="ARTY"></a><span>Paiement sécurisé</span></header><div class="event-payment-grid"><main><div class="section-tag">Devis ${safeText(quote.reference)}</div><h1>${safeText(quote.eventName)}</h1><p class="event-payment-intro">Vérifiez la proposition convenue avec l’équipe ARTY avant de procéder au paiement.</p><div class="event-payment-facts"><div><span>Invités</span><strong>${Number(quote.guests)||1}</strong></div><div><span>Date</span><strong>${safeText(quote.preferredDate||I18n.t('À confirmer'))}</strong></div><div><span>Lieu</span><strong>${safeText(quote.location||I18n.t('À confirmer'))}</strong></div></div><section><h3>Solution retenue</h3><p>${safeText(quote.servicePathLabel)}</p>${items}${custom}${quote.expertBrief?`<div class="event-payment-brief">${safeText(quote.expertBrief)}</div>`:''}</section>${quote.quoteDescription?I18n.html`<section><h3>Détails du devis</h3><div class="event-payment-brief">${safeText(quote.quoteDescription)}</div></section>`:''}</main><aside><span>Montant à payer</span>${eventQuotePriceBreakdownHTML(quote)}<p>Le paiement est traité par Stripe. ARTY ne reçoit pas votre numéro complet de carte.</p><label class="checkout-policy-check"><input type="checkbox" id="eventQuotePolicy"> J’accepte les <a href="#/policies">politiques d’achat</a> et la <a href="#/privacy">politique de confidentialité</a>.</label><button type="button" class="btn btn-orange" id="eventQuotePrepareButton" onclick="prepareEventQuotePayment()">Continuer au paiement</button><div class="event-quote-stripe" id="eventQuoteStripePanel" hidden><div id="event-quote-payment-element"></div><div id="eventQuotePaymentMessage"></div><button type="button" class="btn btn-orange" id="eventQuotePayButton" onclick="confirmEventQuotePayment()">Payer ${I18n.currency(toMoney(quote.quoteAmount))}</button></div></aside></div></div>`;
}
async function prepareEventQuotePayment(){
  if(!document.getElementById('eventQuotePolicy')?.checked)return showToast(I18n.t('Veuillez accepter les politiques avant de continuer'),'error');
  const button=document.getElementById('eventQuotePrepareButton');if(button){button.disabled=true;button.textContent=I18n.t('Préparation...')}
  try{const response=await artyFetch(`/api/event-quotes/${encodeURIComponent(currentEventQuoteToken)}/payment`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||I18n.t('Paiement indisponible'));if(data.paid){currentEventQuote=data.quote;renderEventQuotePaymentContent();return}if(!window.Stripe||!data.publishableKey||!data.clientSecret)throw new Error(I18n.t('Le formulaire Stripe ne peut pas être chargé'));eventQuoteStripe=Stripe(data.publishableKey);eventQuoteElements=eventQuoteStripe.elements({clientSecret:data.clientSecret,appearance:{theme:'flat',variables:{colorPrimary:'#1695a7',colorText:'#332b22',borderRadius:'14px',fontFamily:'Outfit, sans-serif'}},locale:I18n.language()});eventQuotePaymentIntentId=data.paymentIntentId;eventQuoteElements.create('payment',{layout:{type:'accordion',defaultCollapsed:false,radios:'always'},business:{name:'ARTY Création'}}).mount('#event-quote-payment-element');const panel=document.getElementById('eventQuoteStripePanel');if(panel)panel.hidden=false;if(button)button.hidden=true}catch(error){showToast(error.message||I18n.t('Paiement indisponible'),'error');if(button){button.disabled=false;button.textContent=I18n.t('Continuer au paiement')}}
}
async function confirmEventQuotePayment(){
  if(!eventQuoteStripe||!eventQuoteElements)return showToast(I18n.t('Le formulaire de paiement n’est pas prêt'),'error');const button=document.getElementById('eventQuotePayButton'),message=document.getElementById('eventQuotePaymentMessage');if(button){button.disabled=true;button.textContent=I18n.t('Paiement en cours...')}
  try{const result=await eventQuoteStripe.confirmPayment({elements:eventQuoteElements,confirmParams:{return_url:`${window.location.origin}${window.location.pathname}#/event-quote/${encodeURIComponent(currentEventQuoteToken)}`},redirect:'if_required'});if(result.error)throw new Error(result.error.message||I18n.t('Paiement refusé'));const response=await artyFetch(`/api/event-quotes/${encodeURIComponent(currentEventQuoteToken)}/confirm`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paymentIntentId:result.paymentIntent?.id||eventQuotePaymentIntentId})}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||I18n.t('Confirmation impossible'));currentEventQuote=data.quote;renderEventQuotePaymentContent()}catch(error){if(message)message.textContent=error.message||I18n.t('Le paiement n’a pas pu être traité');if(button){button.disabled=false;button.textContent=I18n.msg`Réessayer le paiement`}}
}

/* ===== ADMIN EVENT QUOTE MANAGEMENT ===== */
function adminEventPathLabel(path){return {inventory:I18n.t('Kits du catalogue'),custom:I18n.t('Kit personnalisé'),expert:I18n.t('Accompagnement expert')}[path]||I18n.t('Demande sur mesure')}
function eventQuoteProvinceOptions(selected='QC'){return [['AB','Alberta'],['BC','British Columbia'],['MB','Manitoba'],['NB','New Brunswick'],['NL','Newfoundland and Labrador'],['NS','Nova Scotia'],['NT','Northwest Territories'],['NU','Nunavut'],['ON','Ontario'],['PE','Prince Edward Island'],['QC','Quebec'],['SK','Saskatchewan'],['YT','Yukon']].map(([code,name])=>`<option value="${code}" ${String(selected||'').toUpperCase()===code?'selected':''}>${code} — ${name}</option>`).join('')}

function adminEventRequestStatusOptions(current){const rows=[['nouvelle',I18n.t('Nouvelle')],['en étude',I18n.t('En étude')],['contactée',I18n.t('Contactée')],['devis préparé',I18n.t('Devis préparé')],['paiement prêt',I18n.t('Paiement prêt')],['payée',I18n.t('Payée')],['fermée',I18n.t('Fermée')]];let html=rows.map(([value,label])=>`<option value="${value}" ${current===value?'selected':''}>${label}</option>`).join('');if(current==='remboursée')html+=`<option value="remboursée" selected disabled>${I18n.t('Remboursée')}</option>`;return html}
function eventRequestPaymentLabel(request){if(request.quoteRefundStatus==='refunded'||request.quotePaymentStatus==='refunded')return I18n.t('Remboursé');if(request.quoteRefundStatus==='partial_refund')return I18n.t('Partiellement remboursé');if(request.quoteRefundStatus==='refund_pending'||request.quotePaymentStatus==='refund_pending')return I18n.t('Remboursement en cours');return {not_created:I18n.t('Aucun lien'),ready:I18n.t('Lien prêt'),pending:I18n.t('Paiement commencé'),processing:I18n.t('En traitement'),paid:I18n.t('Payé'),failed:I18n.t('Échec'),cancelled:I18n.t('Annulé')}[request.quotePaymentStatus||'not_created']||I18n.t('À confirmer')}
function eventRefundSummaryHTML(request){
  if(!request.quotePaidAt)return'';
  const refunded=Number(request.quoteRefundedTotal||0),pending=Number(request.quoteRefundPendingTotal||0),paid=Number(request.paymentAmountReceived||request.quoteAmount||0),net=Math.max(0,Number(request.quoteNetPaid??(paid-refunded))),remaining=Math.max(0,paid-refunded-pending);
  const history=(request.quoteRefunds||[]).filter(refund=>refund.providerStatus!=='failed'||refund.stripeRefundId).slice().reverse().map(refund=>{const status=String(refund.providerStatus||refund.status||''),label=status==='succeeded'?I18n.t('Effectué'):status==='pending'||status==='requires_action'?I18n.t('En traitement'):I18n.t('Échec');return `<div class="admin-event-refund-row"><span><strong>${I18n.currency(toMoney(refund.amount))}</strong><small>${safeText(refund.reason||I18n.t('Remboursement'))} · ${profileDate(refund.completedAt||refund.createdAt,true)}</small></span><b class="admin-status ${status==='succeeded'?'ok':status==='failed'?'out':'pending'}">${safeText(label)}</b></div>`}).join('');
  return I18n.html`<section class="admin-order-detail-card admin-event-refund-card"><div class="admin-order-detail-title"><div>${profileIcon('receipt')}<h3>Remboursements</h3></div><span>Stripe</span></div><div class="admin-payment-details"><div><span>Montant payé</span><strong>${I18n.currency(toMoney(paid))}</strong></div>${refunded>0?`<div><span>Déjà remboursé</span><strong>− ${I18n.currency(toMoney(refunded))}</strong></div>`:''}${pending>0?`<div><span>En traitement</span><strong>${I18n.currency(toMoney(pending))}</strong></div>`:''}<div><span>Montant net</span><strong>${I18n.currency(toMoney(net))}</strong></div></div>${history?`<div class="admin-event-refund-history">${history}</div>`:''}${remaining>0&&pending<=0?`<button class="admin-refund-button" type="button" onclick="refundPrivateEvent('${safeAttr(request.id)}')">${I18n.t('Effectuer un remboursement')}</button>`:pending>0?`<p class="admin-quote-action-note">${I18n.t('Un remboursement Stripe est en cours. Attendez sa confirmation avant d’en créer un autre.')}</p>`:`<p class="admin-quote-action-note">${I18n.t('Ce paiement a été entièrement remboursé.')}</p>`}</section>`;
}
function privateEventPaymentMeta(request){if(request.quoteRefundStatus==='refunded'||request.paymentStatus==='refunded')return{label:I18n.t('Remboursé'),className:'refunded'};if(request.quoteRefundStatus==='partial_refund')return{label:I18n.t('Partiellement remboursé'),className:'pending'};if(request.quoteRefundStatus==='refund_pending'||request.paymentStatus==='refund_pending')return{label:I18n.t('Remboursement en cours'),className:'pending'};if(request.paymentStatus==='paid')return{label:I18n.t('Paiement confirmé'),className:'paid'};return{label:I18n.status(request.status||'nouvelle'),className:'pending'}}
function mountAdminEventQuoteWorkspace(panel){
  if(!panel)return;
  Array.from(panel.querySelectorAll('.admin-section-title')).forEach(title=>{if(title.querySelector('h3')?.textContent.includes(I18n.t('Demandes d’événements'))){const table=title.nextElementSibling;title.remove();if(table?.classList.contains('admin-table-wrap'))table.remove()}});
  const cards=(eventRequests||[]).map(request=>{const paymentMeta=privateEventPaymentMeta({...request,paymentStatus:request.quotePaymentStatus});return I18n.html`<article class="admin-event-quote-card" onclick="openAdminEventRequest(${Number(request.id)})"><div class="admin-event-quote-top"><span>${safeText(request.reference||`EVT-${request.id}`)}</span><b class="account-status ${paymentMeta.className}">${safeText(paymentMeta.label)}</b></div><h4>${safeText(request.eventType||I18n.t('Événement à préciser'))}</h4><p>${safeText(request.name||'')} · ${Number(request.guests)||'?'} personne(s)</p><div><span>${safeText(adminEventPathLabel(request.servicePath))}</span><strong>${request.quoteAmount?`${I18n.currency(toMoney(Number(request.quoteRefundedTotal)>0?request.quoteNetPaid:request.quoteAmount))}`:I18n.t('Prix à préparer')}</strong></div><button type="button">Ouvrir la demande</button></article>`}).join('');
  panel.insertAdjacentHTML('beforeend',I18n.html`<section class="admin-event-quotes"><div class="admin-section-title"><div><h3>Demandes de devis</h3><p>Consultez le projet, contactez le client et créez son lien de paiement après son accord.</p></div><span>${eventRequests.length} demande${eventRequests.length>1?'s':''}</span></div><div class="admin-event-quote-grid">${cards||I18n.html('<div class="admin-empty-panel">Aucune demande de devis pour le moment.</div>')}</div></section><div class="admin-order-detail-modal" id="adminEventRequestModal" hidden><button class="admin-order-detail-backdrop" type="button" onclick="closeAdminEventRequest()" aria-label="Fermer"></button><section class="admin-order-detail-sheet admin-event-request-sheet" role="dialog" aria-modal="true"><button class="admin-order-detail-close" type="button" onclick="closeAdminEventRequest()" aria-label="Fermer">×</button><div id="adminEventRequestContent"></div></section></div>`);
}
...
function openAdminEventRequest(id){
  const request=(eventRequests||[]).find(item=>String(item.id)===String(id)),modal=document.getElementById('adminEventRequestModal'),content=document.getElementById('adminEventRequestContent');if(!request||!modal||!content)return;const financialLocked=Boolean(request.quotePaidAt)||['paid','refund_pending','refunded'].includes(String(request.quotePaymentStatus||'')),lockAttr=financialLocked?'disabled':'',address=request.address||{},items=(request.inventoryItems||[]).map(item=>I18n.html`<article class="admin-event-request-product">${item.image?`<img src="${safeAttr(item.image)}" alt="">`:''}<div><strong>${safeText(item.name)}</strong><span>Quantité ${Number(item.quantity)||1}</span></div></article>`).join('');const custom=request.customKit||{},phoneHref=String(request.phone||'').replace(/[^\d+]/g,'');
  const project=request.servicePath==='inventory'?`<div class="admin-event-request-products">${items||I18n.html('<p>Aucun kit enregistré.</p>')}</div>`:request.servicePath==='custom'?I18n.html`<div class="admin-event-custom-review">${custom.paintedPreview?I18n.html`<div><span>Aperçu peint</span><img src="${safeAttr(custom.paintedPreview)}" alt="Aperçu peint de la création"></div>`:''}<div><span>Fichier couleur</span>${custom.sourceImage?I18n.html`<img src="${safeAttr(custom.sourceImage)}" alt="Composition couleur du client">`:I18n.html('<p>Non disponible</p>')}</div><div><span>Tracé de production</span>${custom.traceImage?I18n.html`<img src="${safeAttr(custom.traceImage)}" alt="Aperçu du tracé">`:I18n.html('<p>Non disponible</p>')}</div></div><div class="admin-event-custom-meta"><strong>${safeText(custom.productLabel||custom.sizeLabel||I18n.t('Création personnalisée'))}</strong><span>${safeText([custom.sizeLabel,custom.orientation].filter(Boolean).join(' · '))}</span><span>${Number(custom.quantity)||request.guests||1} kit(s)</span><span>${Number(custom.elementCount)||1} élément(s)</span><p>${safeText(custom.notes||I18n.t('Aucune instruction supplémentaire.'))}</p></div>`:I18n.html`<div class="admin-event-expert-brief"><strong>Vision du client</strong><p>${safeText(request.expertBrief||request.message||I18n.t('Aucune description fournie.'))}</p></div>`;
  content.innerHTML=I18n.html`<header class="admin-order-detail-head"><div><span>${safeText(request.reference||`EVT-${request.id}`)}</span><h2>${safeText(request.eventType||I18n.t('Demande d’événement'))}</h2><p>Reçue le ${profileDate(request.createdAt,true)}</p></div><div><b class="admin-status-badge">${safeText(I18n.status(request.status||'nouvelle'))}</b><b class="admin-status ${request.quoteRefundStatus==='refunded'?'out':request.quoteRefundStatus==='refund_pending'?'pending':request.quotePaymentStatus==='paid'?'ok':'pending'}">${safeText(eventRequestPaymentLabel(request))}</b></div></header><div class="admin-event-request-overview"><div>${eventExperienceIcon('people')}<span>Participants<strong>${Number(request.guests)||I18n.t('À confirmer')}</strong></span></div><div>${eventExperienceIcon('calendar')}<span>Date et heure<strong>${safeText([request.preferredDate,request.eventTime].filter(Boolean).join(' · ')||I18n.t('À confirmer'))}</strong></span></div><div>${eventExperienceIcon('location')}<span>Lieu<strong>${safeText(request.location||I18n.t('À confirmer'))}</strong></span></div></div><div class="admin-order-detail-columns admin-event-request-columns"><main><section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${profileIcon('user')}<h3>Coordonnées du client</h3></div><span>Contact préféré : ${request.contactPreference==='phone'?I18n.t('téléphone'):'courriel'}</span></div><div class="admin-customer-info"><div><span>Nom</span><strong>${safeText(request.name||'')}</strong></div><div><span>Courriel</span><strong>${safeText(request.email||'')}</strong></div><div><span>Téléphone</span><strong>${safeText(request.phone||I18n.t('Non indiqué'))}</strong></div></div><div class="admin-contact-actions"><a href="mailto:${safeAttr(request.email||'')}">Envoyer un courriel</a>${phoneHref?I18n.html`<a href="tel:${safeAttr(phoneHref)}">Appeler le client</a>`:''}</div></section><section class="admin-order-detail-card"><div class="admin-order-detail-title"><div>${eventExperienceIcon(request.servicePath||'expert')}<h3>${safeText(adminEventPathLabel(request.servicePath))}</h3></div></div>${project}</section>${request.message?I18n.html`<section class="admin-order-detail-card"><div class="admin-order-detail-title"><div><h3>Informations supplémentaires</h3></div></div><p>${safeText(request.message)}</p></section>`:''}</main><aside><section class="admin-order-detail-card admin-order-management"><div class="admin-order-detail-title"><div>${profileIcon('receipt')}<h3>Préparer le devis</h3></div></div><div class="form-group"><label>Statut</label><select id="eventRequestStatus-${request.id}">${adminEventRequestStatusOptions(request.status)}</select></div><div class="form-group"><label>Prix avant taxes ($ CAD)</label><input id="eventRequestAmount-${request.id}" type="number" min="0.50" step="0.01" value="${Number(request.quoteSubtotal??request.quoteAmount)||''}" placeholder="Ex: 850.00" oninput="previewAdminEventQuote(${request.id})" ${lockAttr}></div><div class="form-group"><label>Livraison / déplacement ($ CAD)</label><input id="eventRequestShipping-${request.id}" type="number" min="0" step="0.01" value="${Number(request.quoteShipping||0)}" oninput="previewAdminEventQuote(${request.id})" ${lockAttr}></div><div class="form-group"><label>Adresse</label><input id="eventRequestAddress-${request.id}" value="${safeAttr(address.line1||'')}" oninput="previewAdminEventQuote(${request.id})" ${lockAttr}></div><div class="form-row"><div class="form-group"><label>Ville</label><input id="eventRequestCity-${request.id}" value="${safeAttr(address.city||'')}" oninput="previewAdminEventQuote(${request.id})" ${lockAttr}></div><div class="form-group"><label>Province</label><select id="eventRequestProvince-${request.id}" onchange="previewAdminEventQuote(${request.id})" ${lockAttr}>${eventQuoteProvinceOptions(address.province||'QC')}</select></div></div><div class="form-group"><label>Code postal</label><input id="eventRequestPostal-${request.id}" value="${safeAttr(address.postal||'')}" ${lockAttr}></div><div id="eventRequestQuoteTotal-${request.id}" class="admin-event-quote-total"><span>Calcul du total…</span></div><div class="form-group"><label>Détails visibles par le client</label><textarea id="eventRequestQuote-${request.id}" placeholder="Matériel, livraison, préparation, échéancier...">${safeText(request.quoteDescription||'')}</textarea></div><div class="form-group"><label>Note interne</label><textarea id="eventRequestNote-${request.id}" placeholder="Suivi téléphonique, préférences, décisions...">${safeText(request.adminNote||'')}</textarea></div><button type="button" class="btn btn-teal" onclick="saveAdminEventRequest(${request.id},false)">Enregistrer le suivi</button><button type="button" class="btn btn-orange" onclick="saveAdminEventRequest(${request.id},true)" ${financialLocked?'disabled':''}>${request.paymentLinkUrl?I18n.t('Créer un nouveau lien et l’envoyer'):I18n.t('Créer et envoyer le lien de paiement')}</button><p class="admin-quote-action-note">Utilisez le lien de paiement seulement après avoir confirmé le devis avec le client.</p></section>${request.paymentLinkUrl&&request.quoteRefundStatus!=='refunded'?I18n.html`<section class="admin-order-detail-card admin-payment-link-card"><span>Lien de paiement</span><strong>${safeText(eventRequestPaymentLabel(request))}</strong><a href="${safeAttr(request.paymentLinkUrl)}" target="_blank" rel="noopener">Ouvrir le lien</a><button type="button" onclick="copyEventQuotePaymentLink(${request.id})">Copier le lien</button>${request.quoteEmailDelivery?.status?I18n.html`<small>Courriel : ${safeText(I18n.status(request.quoteEmailDelivery.status))}</small>`:''}</section>`:''}${eventRefundSummaryHTML(request)}</aside></div>`;modal.hidden=false;document.body.style.overflow='hidden';previewAdminEventQuote(request.id);
}
...
async function previewAdminEventQuote(id){
  const box=document.getElementById(`eventRequestQuoteTotal-${id}`);if(!box)return null;
  const payload={quoteSubtotal:Number(document.getElementById(`eventRequestAmount-${id}`)?.value)||0,quoteShipping:Number(document.getElementById(`eventRequestShipping-${id}`)?.value)||0,quoteAddress:{line1:document.getElementById(`eventRequestAddress-${id}`)?.value.trim()||'',city:document.getElementById(`eventRequestCity-${id}`)?.value.trim()||'',province:document.getElementById(`eventRequestProvince-${id}`)?.value||'QC',postal:document.getElementById(`eventRequestPostal-${id}`)?.value.trim()||'',country:'Canada'}};
  if(payload.quoteSubtotal<.5){box.innerHTML='<span>'+I18n.t('Entrez un prix avant taxes')+'</span>';return null}
  try{const response=await artyFetch(`/api/admin/event-requests/${id}/quote-preview`,{method:'POST',headers:authH(),body:JSON.stringify(payload)}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||I18n.t('Calcul impossible'));box.innerHTML=`<div><span>${I18n.t('Sous-total')}</span><strong>${I18n.currency(toMoney(data.quoteSubtotal))}</strong></div><div><span>${I18n.t('Livraison / déplacement')}</span><strong>${I18n.currency(toMoney(data.quoteShipping))}</strong></div>${(data.quoteTaxLines||[]).map(line=>`<div><span>${safeText(line.label)} ${Number(line.rate)}%</span><strong>${I18n.currency(toMoney(line.amount))}</strong></div>`).join('')}<div class="total"><span>${I18n.t('Total à payer')}</span><strong>${I18n.currency(toMoney(data.quoteAmount))}</strong></div>`;return data}catch(error){box.innerHTML=`<span class="error">${safeText(error.message)}</span>`;return null}
}
async function saveAdminEventRequest(id,createPaymentLink){
  const payload={status:document.getElementById(`eventRequestStatus-${id}`)?.value,quoteSubtotal:document.getElementById(`eventRequestAmount-${id}`)?.value,quoteShipping:document.getElementById(`eventRequestShipping-${id}`)?.value,quoteAddress:{line1:document.getElementById(`eventRequestAddress-${id}`)?.value.trim()||'',city:document.getElementById(`eventRequestCity-${id}`)?.value.trim()||'',province:document.getElementById(`eventRequestProvince-${id}`)?.value||'QC',postal:document.getElementById(`eventRequestPostal-${id}`)?.value.trim()||'',country:'Canada'},quoteDescription:document.getElementById(`eventRequestQuote-${id}`)?.value.trim()||'',adminNote:document.getElementById(`eventRequestNote-${id}`)?.value.trim()||''};
  try{const pricing=await previewAdminEventQuote(id);if(createPaymentLink&&!pricing)return;let response=await artyFetch(`/api/admin/event-requests/${id}`,{method:'PATCH',headers:authH(),body:JSON.stringify(payload)}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||I18n.t('Mise à jour impossible'));if(createPaymentLink){if(!confirm(I18n.msg`Envoyer ce devis de ${I18n.currency(toMoney(pricing.quoteAmount))} au client?`))return;response=await artyFetch(`/api/admin/event-requests/${id}/payment-link`,{method:'POST',headers:authH(),body:JSON.stringify(payload)});data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||I18n.t('Lien de paiement impossible'));showToast(data.emailStatus==='sent'?I18n.t('Lien créé et envoyé au client'):I18n.t('Lien créé; vérifiez la livraison du courriel'),'success')}else showToast(I18n.t('Demande mise à jour'),'success');await loadEventRequests();renderAdminEvents();openAdminEventRequest(id)}catch(error){showToast(error.message||I18n.t('Erreur'),'error')}
}
async function refundPrivateEvent(id){
  const request=(eventRequests||[]).find(item=>String(item.id)===String(id));if(!request)return;
  const paid=Number(request.paymentAmountReceived||request.quoteAmount||0),refunded=Number(request.quoteRefundedTotal||0),pending=Number(request.quoteRefundPendingTotal||0),remaining=Math.max(0,paid-refunded-pending);
  if(remaining<=0)return showToast(I18n.t('Aucun montant remboursable'),'error');
  const raw=prompt(`Montant à rembourser (max ${I18n.currency(toMoney(remaining))})`,toMoney(remaining));if(raw===null)return;
  const amount=Number(raw);if(!Number.isFinite(amount)||amount<=0||amount>remaining)return showToast(I18n.t('Montant de remboursement invalide'),'error');
  const reason=prompt(I18n.t('Raison du remboursement'),I18n.t('Demande client'))||I18n.t('Demande client');
  if(!confirm(I18n.msg`Confirmer le remboursement de ${I18n.currency(toMoney(amount))}? ARTY enverra le remboursement directement à Stripe et au mode de paiement original du client.`))return;
  try{
    const response=await artyFetch(`/api/admin/event-requests/${encodeURIComponent(id)}/refund`,{method:'POST',headers:authH(),body:JSON.stringify({amount,reason})}),data=await response.json().catch(()=>({}));
    if(!response.ok)return showToast(data.error||I18n.t('Remboursement impossible'),'error');
    const processing=['pending','requires_action'].includes(String(data.stripeStatus||''));
    showToast(processing?I18n.t('Remboursement accepté par Stripe et en traitement'):I18n.t('Remboursement effectué dans Stripe'),'success');
    await loadEventRequests();renderAdminEvents();openAdminEventRequest(id);
  }catch{showToast(I18n.t('Erreur de connexion'),'error')}
}
function copyEventQuotePaymentLink(id){const request=(eventRequests||[]).find(item=>String(item.id)===String(id));if(!request?.paymentLinkUrl)return;navigator.clipboard?.writeText(request.paymentLinkUrl).then(()=>showToast(I18n.t('Lien copié'),'success')).catch(()=>showToast(I18n.t('Impossible de copier le lien'),'error'))}
...
const designStudioProducts={
  canvas:{get label(){return I18n.t('Toile rectangulaire')},get shortLabel(){return I18n.t('Toile')},get description(){return I18n.t('Un canevas personnalisé à tracer et à peindre.')}},
  bag:{get label(){return I18n.t('Sac en toile')},get shortLabel(){return I18n.t('Sac')},get description(){return I18n.t('Un sac réutilisable avec votre création à peindre.')}}
};
...
function designStudioInitialState(){return{product:'canvas',size:'moyen',orientation:'portrait',view:'edit',activePanel:'templates',elements:[],selectedId:null,quantity:1,notes:'',zoom:.72,zoomAuto:true,history:[],future:[]}}
let designStudioState=designStudioInitialState();
let designStudioRuntime={canvas:null,ctx:null,dragMode:'',resizeHandle:'',pointerId:null,startPoint:null,startElement:null,images:new Map(),busy:false};
let designStudioContext='shop';
const DESIGN_STUDIO_DRAFT_KEY='arty-design-studio-draft-v2';
let designStudioDraftRestored=false;

function cloneDesignStudioState(state){return{...state,elements:(state.elements||[]).map(item=>({...item})),history:[],future:[]}}
function scheduleDesignStudioDraftSave(){
  if(isEventDesignStudio()||typeof localStorage==='undefined')return;
  clearTimeout(designStudioRuntime.draftTimer);
  designStudioRuntime.draftTimer=setTimeout(()=>{
    const snapshot={product:designStudioState.product,size:designStudioState.size,orientation:designStudioState.orientation,studioOptions:Array.isArray(designStudioState.studioOptions)?[...designStudioState.studioOptions]:[],view:designStudioState.view,activePanel:designStudioState.activePanel,elements:designStudioState.elements.map(item=>({...item})),selectedId:designStudioState.selectedId,quantity:designStudioState.quantity,notes:designStudioState.notes,zoom:designStudioState.zoom,zoomAuto:designStudioState.zoomAuto,savedAt:Date.now()};
    try{localStorage.setItem(DESIGN_STUDIO_DRAFT_KEY,JSON.stringify(snapshot))}catch{
      try{localStorage.setItem(DESIGN_STUDIO_DRAFT_KEY,JSON.stringify({...snapshot,elements:snapshot.elements.map(item=>item.type==='image'?{...item,trace:''}:item)}))}catch{}
    }
  },220);
}
function restoreDesignStudioDraft(){
  designStudioDraftRestored=true;
  if(isEventDesignStudio()||typeof localStorage==='undefined')return false;
  try{
    const saved=JSON.parse(localStorage.getItem(DESIGN_STUDIO_DRAFT_KEY)||'null');
    if(!saved||Date.now()-Number(saved.savedAt||0)>30*24*60*60*1000||!Array.isArray(saved.elements))return false;
    designStudioState={...designStudioInitialState(),...saved,elements:saved.elements.map(item=>({...item})),history:[],future:[]};
    return designStudioState.elements.length>0;
  }catch{return false}
}
function clearDesignStudioDraft(){if(typeof localStorage!=='undefined')try{localStorage.removeItem(DESIGN_STUDIO_DRAFT_KEY)}catch{}designStudioDraftRestored=true}
function isEventDesignStudio(){return designStudioContext==='event'}
function prepareDesignStudioRoute(hash){
  const requested=new URLSearchParams(String(hash||'').split('?')[1]||'').get('mode')==='event'?'event':'shop';
  if(requested===designStudioContext){if(requested==='shop'&&!designStudioDraftRestored&&!designStudioState.elements.length)restoreDesignStudioDraft();return}
  if(requested==='event'){
    designStudioState=eventStudioDraftState?cloneDesignStudioState(eventStudioDraftState):designStudioInitialState();
    if(!eventStudioDraftState){designStudioState.quantity=Number(eventBuilderState.customKit?.quantity)||Number(eventBuilderState.guests)||1;designStudioState.notes=eventBuilderState.customKit?.notes||''}
  }else{designStudioState=designStudioInitialState();restoreDesignStudioDraft()}
  designStudioRuntime.images=new Map();designStudioContext=requested;
}
function openEventDesignStudio(){if(designStudioContext==='event'){designStudioState.quantity=Number(eventBuilderState.customKit?.quantity)||Number(eventBuilderState.guests)||1;designStudioState.notes=eventBuilderState.customKit?.notes||designStudioState.notes||''}eventBuilderState.step=3;navigate('#/studio?mode=event')}
...
function renderDesignStudioPage(){
  ...
  const eventMode=isEventDesignStudio(),primaryLabel=eventMode?I18n.t('Continuer mon événement'):I18n.t('Ajouter au panier');
  container.innerHTML=I18n.html`<div class="design-studio-app">
    ${mobileStudioChrome()}
    <header class="design-studio-topbar">
      <div class="design-studio-brand"><button type="button" class="design-studio-exit" onclick="exitDesignStudio()" aria-label="${eventMode?I18n.t('Retourner à la demande'):I18n.t('Quitter le studio')}">${designStudioIcon('back')}<span>${eventMode?I18n.t('Retour'):I18n.t('Quitter')}</span></button><i aria-hidden="true"></i><img src="logoarty.png" alt="ARTY"><span><strong>${eventMode?I18n.t('Studio pour votre événement'):I18n.t('Studio de création')}</strong><small>${eventMode?safeText(eventBuilderState.eventName||I18n.t('Création liée au devis')):I18n.t('Votre design, prêt à peindre')}</small></span></div>
      ${languageSwitchHTML()}<div class="design-studio-history"><button type="button" onclick="designStudioUndo()" data-studio-action="undo" aria-label="Annuler" ${designStudioState.history.length?'':'disabled'}>${designStudioIcon('undo')}</button><button type="button" onclick="designStudioRedo()" data-studio-action="redo" aria-label="Rétablir" ${designStudioState.future.length?'':'disabled'}>${designStudioIcon('redo')}</button></div>
      ...
      <div class="design-studio-order"><span><small id="designStudioProductTop">${safeText(designStudioProductLabel())}</small><strong id="designStudioPriceTop">${eventMode?I18n.t('Devis personnalisé'):`${I18n.currency(toMoney(designStudioPrice()))}`}</strong></span><button type="button" id="designStudioCartButton" onclick="handleDesignStudioPrimaryAction()">${designStudioIcon(eventMode?'check':'cart')}<b>${primaryLabel}</b></button></div>
    </header>
    <div class="design-studio-body">
      <nav class="design-studio-tools" aria-label="Outils de création">
        ${designStudioToolButton('templates','template',I18n.t('Modèles'))}
        ${designStudioToolButton('images','image','Images')}
        ${designStudioToolButton('text','text',I18n.t('Texte'))}
        ${designStudioToolButton('shapes','shape',I18n.t('Formes'))}
        ${designStudioToolButton('layers','layers',I18n.t('Calques'))}
      </nav>
      <aside class="design-studio-library" id="designStudioLibrary"></aside>
      <main class="design-studio-workspace">
        <div class="design-studio-workspace-head"><span id="designStudioViewLabel">${designStudioViewLabel()}</span><small id="designStudioHint">${designStudioViewHint()}</small><button type="button" id="designStudioEditReturn" onclick="setDesignStudioView('edit')" ${designStudioState.view==='edit'?'hidden':''}>Modifier le design</button></div>
        ...
        ${mobileStudioWorkspaceControls()}
        <div class="design-studio-zoom">${designStudioIcon('zoom')}<input type="range" min="30" max="110" value="${Math.round(designStudioState.zoom*100)}" oninput="setDesignStudioZoom(this.value)"><span id="designStudioZoomValue">${Math.round(designStudioState.zoom*100)}%</span></div>
      </main>
  ...
  renderDesignStudioLibrary();renderDesignStudioInspector();initDesignStudioCanvas();syncMobileStudioUI();updateMobileViewport();requestAnimationFrame(()=>fitDesignStudioCanvas());
}
function exitDesignStudio(){if(isEventDesignStudio()){eventStudioDraftState=cloneDesignStudioState(designStudioState);eventBuilderState.step=3;navigate('#/event-builder');return}navigate('#/paintings')}
...
function designStudioViewLabel(){return{edit:I18n.t('Mode création'),trace:I18n.t('Tracé livré dans le kit'),painted:I18n.t('Simulation du résultat peint')}[designStudioState.view]||I18n.t('Mode création')}
function designStudioViewHint(){return{edit:I18n.t('Sélectionnez et déplacez les éléments directement sur le produit.'),trace:I18n.t('Aperçu des lignes qui serviront à tracer et à peindre.'),painted:I18n.t('Simulation colorée basée sur vos images et vos réglages.')}[designStudioState.view]||''}
function setDesignStudioPanel(panel){if(isMobileDesignStudio()){setMobileStudioSheet(panel);return}designStudioState.activePanel=panel;document.querySelectorAll('[data-studio-panel]').forEach(button=>button.classList.toggle('active',button.dataset.studioPanel===panel));renderDesignStudioLibrary()}
function renderDesignStudioLibrary(){
  ...
  if(designStudioState.activePanel==='layers')panel.innerHTML=I18n.html`<div class="studio-panel-head"><span>Calques</span><h2>Organisez le design</h2><p>Les éléments du haut apparaissent devant les autres.</p></div><div class="studio-layer-list">${[...designStudioState.elements].reverse().map(item=>`<button type="button" class="${designStudioState.selectedId===item.id?'active':''}" onclick="selectDesignStudioElement('${safeAttr(item.id)}')"><span class="studio-layer-thumb">${item.type==='image'?`<img src="${safeAttr(item.source)}" alt="">`:item.type==='text'?designStudioIcon('text'):designStudioIcon('shape')}</span><span><strong>${safeText(designStudioElementName(item))}</strong><small>${item.type==='image'?'Image':item.type==='text'?I18n.t('Texte'):I18n.t('Forme')}</small></span></button>`).join('')||I18n.html('<div class="studio-panel-empty">Ajoutez une image, un texte ou une forme.</div>')}</div>`;
  setupDesignStudioDropZone();syncMobileStudioUI();
}
function designStudioElementName(item){if(item.type==='text')return item.text||I18n.t('Texte');if(item.type==='shape')return{circle:I18n.t('Cercle'),rectangle:'Rectangle',star:I18n.t('Étoile'),heart:'Cœur'}[item.shape]||I18n.t('Forme');return item.name||'Image'}
function designStudioPrintSpec(){
  const bag=designStudioState.product==='bag';
  let width=16,height=20;
  if(!bag){const label=String(designStudioSizes[designStudioState.size]?.label||'').match(/(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)/);if(label){width=Number(label[1])||width;height=Number(label[2])||height}}
  else{width=12;height=12}
  if(!bag&&designStudioState.orientation==='landscape')[width,height]=[height,width];
  const dpi=150;
  return{width,height,dpi,pixelWidth:Math.round(width*dpi),pixelHeight:Math.round(height*dpi),label:`${width} × ${height} po · ${dpi} DPI`};
}
function designStudioQualitySummary(){
  const images=designStudioState.elements.filter(item=>item.type==='image');
  if(!images.length)return{status:'good',title:'Qualité d’impression',detail:'Votre composition utilise des éléments vectoriels, prêts pour le tracé.'};
  const spec=designStudioPrintSpec(),dpis=images.map(item=>{const sourceWidth=Number(item.sourceWidth)||0;if(!sourceWidth)return null;return sourceWidth/Math.max(.1,spec.width*Math.max(.08,Number(item.w)||.08))}).filter(value=>Number.isFinite(value));
  if(!dpis.length)return{status:'warn',title:'Vérifiez la qualité de vos images',detail:'La résolution de vos fichiers sera vérifiée avant la production.'};
  const minDpi=Math.round(Math.min(...dpis));
  return minDpi<150?{status:'warn',title:'Une image peut manquer de netteté',detail:`Résolution estimée : ${minDpi} DPI · agrandissez la source ou réduisez son cadrage.`}:{status:'good',title:'Qualité d’impression vérifiée',detail:`Résolution minimale estimée : ${minDpi} DPI · export ${spec.label}.`};
}
function designStudioQualityCard(){const summary=designStudioQualitySummary();return I18n.html`<div class="studio-print-quality ${summary.status}"><span>${summary.title}</span><strong>${summary.status==='good'?'Prêt pour la production':'À vérifier'}</strong><small>${summary.detail}</small></div>`}
function setDesignStudioNotes(value){designStudioState.notes=String(value||'').slice(0,400);scheduleDesignStudioDraftSave()}
function designStudioOrderCard(){const eventMode=isEventDesignStudio();return I18n.html`<div class="studio-order-card ${eventMode?'event-mode':''}"><div><span>${eventMode?I18n.t('Projet de l’événement'):I18n.t('Votre produit')}</span><strong>${safeText(designStudioProductLabel())}</strong></div><label>${eventMode?I18n.t('Nombre de kits identiques'):I18n.t('Quantité')}<input type="number" min="1" max="${eventMode?'1000':'50'}" value="${designStudioState.quantity}" onchange="setDesignStudioQuantity(this.value)"></label><label>Note pour la production<textarea maxlength="400" placeholder="Une précision pour notre équipe..." oninput="setDesignStudioNotes(this.value)">${safeText(designStudioState.notes||'')}</textarea></label>${eventMode?'':designStudioQualityCard()}<div class="studio-order-total"><span>${eventMode?I18n.t('Tarification'):I18n.t('Prix unitaire')}</span><strong>${eventMode?I18n.t('Incluse au devis'):`${I18n.currency(toMoney(designStudioPrice()))}`}</strong></div><button type="button" onclick="handleDesignStudioPrimaryAction()">${designStudioIcon(eventMode?'check':'cart')}${eventMode?I18n.t('Continuer mon événement'):I18n.t('Ajouter au panier')}</button></div>`}
...
function renderDesignStudioInspector(){
  ...
  if(!item){panel.innerHTML=I18n.html`<div class="studio-inspector-head"><span>Propriétés</span><h2>Votre création</h2><p>Sélectionnez un élément sur le produit pour le modifier.</p></div><div class="studio-inspector-empty">${designStudioIcon('center')}<strong>Tout est prêt</strong><span>Ajoutez une image, un texte ou une forme depuis les outils à gauche.</span></div>${designStudioOrderCard()}`;syncMobileStudioUI();return}
  const imageControls=item.type==='image'?I18n.html`<div class="studio-property-group"><label>Remplissage</label><div class="studio-segmented"><button type="button" class="${item.fit==='contain'?'active':''}" onclick="updateDesignStudioElement('fit','contain',true)">Image entière</button><button type="button" class="${item.fit==='cover'?'active':''}" onclick="updateDesignStudioElement('fit','cover',true)">Remplir</button></div></div><div class="studio-property-group"><label>Réglages de l’image</label>${designStudioRange(I18n.t('Contraste'),'contrast',item.contrast,50,170)}${designStudioRange(I18n.t('Luminosité'),'brightness',item.brightness,50,150)}${designStudioRange(I18n.t('Couleurs'),'saturation',item.saturation,0,190)}${designStudioRange(I18n.t('Détail du tracé'),'traceDetail',item.traceDetail||48,20,90)}<div class="studio-preset-row"><button type="button" onclick="applyDesignStudioImagePreset('natural')">Naturel</button><button type="button" onclick="applyDesignStudioImagePreset('vivid')">Vif</button><button type="button" onclick="applyDesignStudioImagePreset('soft')">Doux</button></div></div>`:'';
  const textControls=item.type==='text'?I18n.html`<div class="studio-property-group"><label>Contenu du texte</label><textarea maxlength="120" onfocus="designStudioPushHistory()" oninput="updateDesignStudioElement('text',this.value)">${safeText(item.text||'')}</textarea></div><div class="studio-property-grid"><label>Police<select onchange="updateDesignStudioElement('fontFamily',this.value,true)"><option value="Outfit" ${item.fontFamily==='Outfit'?'selected':''}>Moderne</option><option value="Caveat" ${item.fontFamily==='Caveat'?'selected':''}>Créative</option><option value="Georgia" ${item.fontFamily==='Georgia'?'selected':''}>Classique</option></select></label><label>Couleur<input type="color" value="${safeAttr(item.color||'#1f6d78')}" onchange="updateDesignStudioElement('color',this.value,true)"></label></div>${designStudioRange(I18n.t('Taille du texte'),'fontSize',item.fontSize,4,24)}`:'';
  const shapeControls=item.type==='shape'?I18n.html`<div class="studio-property-grid"><label>Couleur<input type="color" value="${safeAttr(item.color||'#e8863a')}" onchange="updateDesignStudioElement('color',this.value,true)"></label><label>Style<select onchange="updateDesignStudioElement('filled',this.value==='true',true)"><option value="true" ${item.filled!==false?'selected':''}>Plein</option><option value="false" ${item.filled===false?'selected':''}>Contour</option></select></label></div>`:'';
  panel.innerHTML=I18n.html`<div class="studio-inspector-head"><span>Élément sélectionné</span><h2>${safeText(designStudioElementName(item))}</h2><p>${item.type==='image'?I18n.t('Ajustez le cadrage et le rendu de cette image.'):item.type==='text'?I18n.t('Personnalisez le texte et sa présentation.'):I18n.t('Modifiez cette forme décorative.')}</p></div>${imageControls}${textControls}${shapeControls}<div class="studio-property-group"><label>Position et taille</label><div class="studio-resize-tip">Faites glisser une poignée autour de l’élément pour le redimensionner.</div>${designStudioRange(I18n.t('Largeur'),'scale',Math.round(item.w*100),8,160)}${designStudioRange('Rotation','rotation',item.rotation||0,-180,180)}${designStudioRange(I18n.t('Opacité'),'opacity',item.opacity??100,10,100)}<div class="studio-align-row"><button type="button" onclick="alignDesignStudioElement('horizontal')">Centrer horizontalement</button><button type="button" onclick="alignDesignStudioElement('vertical')">Centrer verticalement</button></div></div><div class="studio-element-actions"><button type="button" onclick="moveDesignStudioElement(-1)">${designStudioIcon('backward')}Derrière</button><button type="button" onclick="moveDesignStudioElement(1)">${designStudioIcon('forward')}Devant</button><button type="button" onclick="duplicateDesignStudioElement()">${designStudioIcon('copy')}Dupliquer</button><button type="button" class="danger" onclick="removeDesignStudioElement()">${designStudioIcon('trash')}Supprimer</button></div>${designStudioOrderCard()}`;syncMobileStudioUI();
}
function designStudioGeometry(){
  if(designStudioState.product==='bag')return{product:{x:285,y:210,w:630,h:625},print:{x:345,y:288,w:510,h:505}};
...
function initDesignStudioCanvas(){
  ...
  canvas.onpointerdown=designStudioPointerDown;canvas.onpointermove=designStudioPointerMove;canvas.onpointerup=designStudioPointerUp;canvas.onpointercancel=designStudioPointerUp;canvas.onlostpointercapture=designStudioPointerUp;
  designStudioState.elements.forEach(designStudioHydrateElement);drawDesignStudio();initMobileStudioCanvas(canvas);
}
...
function renderDesignStudioScene(ctx,view,showSelection=false){
  ...
  if(designStudioState.product==='canvas'){
    ...
    const bagGradient=ctx.createLinearGradient(product.x,product.y,product.x+product.w,product.y+product.h);bagGradient.addColorStop(0,'#f4e5cf');bagGradient.addColorStop(.48,'#ead5b8');bagGradient.addColorStop(1,'#dfc29d');
    ctx.shadowColor='transparent';ctx.lineCap='round';ctx.lineJoin='round';
    ctx.strokeStyle='#b98b59';ctx.lineWidth=34;ctx.beginPath();ctx.moveTo(product.x+142,product.y+46);ctx.bezierCurveTo(product.x+150,product.y-170,product.x+480,product.y-170,product.x+488,product.y+46);ctx.stroke();
    ctx.strokeStyle='#ead2b1';ctx.lineWidth=22;ctx.beginPath();ctx.moveTo(product.x+142,product.y+46);ctx.bezierCurveTo(product.x+150,product.y-170,product.x+480,product.y-170,product.x+488,product.y+46);ctx.stroke();
    ctx.strokeStyle='#c69b6c';ctx.lineWidth=27;ctx.beginPath();ctx.moveTo(product.x+188,product.y+48);ctx.bezierCurveTo(product.x+198,product.y-125,product.x+432,product.y-125,product.x+442,product.y+48);ctx.stroke();
    ctx.strokeStyle='#f0dcc0';ctx.lineWidth=17;ctx.beginPath();ctx.moveTo(product.x+188,product.y+48);ctx.bezierCurveTo(product.x+198,product.y-125,product.x+432,product.y-125,product.x+442,product.y+48);ctx.stroke();
    ctx.shadowColor='rgba(38,34,28,.2)';ctx.shadowBlur=30;ctx.shadowOffsetY=20;ctx.fillStyle=bagGradient;ctx.beginPath();ctx.moveTo(product.x+55,product.y);ctx.quadraticCurveTo(product.x+8,product.y+18,product.x+18,product.y+88);ctx.lineTo(product.x+48,product.y+product.h-42);ctx.quadraticCurveTo(product.x+52,product.y+product.h,product.x+105,product.y+product.h);ctx.lineTo(product.x+product.w-105,product.y+product.h);ctx.quadraticCurveTo(product.x+product.w-52,product.y+product.h,product.x+product.w-48,product.y+product.h-42);ctx.lineTo(product.x+product.w-18,product.y+88);ctx.quadraticCurveTo(product.x+product.w-8,product.y+18,product.x+product.w-55,product.y);ctx.closePath();ctx.fill();
    ctx.shadowColor='transparent';ctx.strokeStyle='#c59a6b';ctx.lineWidth=5;ctx.stroke();
    ctx.strokeStyle='rgba(151,109,67,.35)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(product.x+53,product.y+62);ctx.quadraticCurveTo(product.x+product.w/2,product.y+83,product.x+product.w-53,product.y+62);ctx.stroke();
    ctx.save();ctx.globalAlpha=.12;ctx.strokeStyle='#9c744e';ctx.lineWidth=1;for(let y=product.y+95;y<product.y+product.h-30;y+=12){ctx.beginPath();ctx.moveTo(product.x+48,y);ctx.lineTo(product.x+product.w-48,y+2);ctx.stroke()}ctx.restore();
  }
  ctx.save();drawRoundedRect(ctx,print.x,print.y,print.w,print.h,designStudioState.product==='bag'?12:2);ctx.clip();
  if(view!=='edit'){ctx.fillStyle=designStudioState.product==='bag'?'rgba(255,250,242,.3)':'#fffdf9';ctx.fillRect(print.x,print.y,print.w,print.h)}
  ...
  if(designStudioState.product==='bag'){
    ctx.strokeStyle=view==='edit'?'rgba(20,137,151,.7)':'rgba(139,99,58,.3)';ctx.lineWidth=3;ctx.setLineDash(view==='edit'?[13,9]:[]);drawRoundedRect(ctx,print.x,print.y,print.w,print.h,14);ctx.stroke();ctx.setLineDash([]);
    ctx.strokeStyle='rgba(139,99,58,.25)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(product.x+49,product.y+70);ctx.lineTo(product.x+77,product.y+product.h-38);ctx.moveTo(product.x+product.w-49,product.y+70);ctx.lineTo(product.x+product.w-77,product.y+product.h-38);ctx.stroke();
    if(view==='edit'){ctx.font='700 15px Outfit, sans-serif';ctx.textAlign='left';ctx.textBaseline='bottom';ctx.fillStyle='#137d8a';ctx.fillText(I18n.t('ZONE IMPRIMABLE'),print.x+4,print.y-11)}
  }
...
function drawDesignStudioSelection(ctx,item,print){
  const rect=designStudioElementRect(item,print);ctx.save();ctx.translate(rect.cx,rect.cy);ctx.rotate((Number(item.rotation)||0)*Math.PI/180);ctx.strokeStyle='#0d94a6';ctx.lineWidth=3;ctx.setLineDash([9,7]);ctx.strokeRect(-rect.w/2,-rect.h/2,rect.w,rect.h);ctx.setLineDash([]);designStudioHandleDefinitions().forEach(handle=>{const x=handle.hx*rect.w/2,y=handle.hy*rect.h/2;ctx.shadowColor='rgba(13,148,166,.2)';ctx.shadowBlur=7;ctx.fillStyle='#fff';ctx.strokeStyle='#0d94a6';ctx.lineWidth=3;ctx.beginPath();ctx.arc(x,y,isMobileDesignStudio()?Math.max(11,9/designStudioState.zoom):(handle.hx&&handle.hy?11:9),0,Math.PI*2);ctx.fill();ctx.stroke();ctx.shadowColor='transparent'});ctx.restore();
}
...
function designStudioHandleDefinitions(){const handles=[{key:'nw',hx:-1,hy:-1,cursor:'nwse-resize'},{key:'n',hx:0,hy:-1,cursor:'ns-resize'},{key:'ne',hx:1,hy:-1,cursor:'nesw-resize'},{key:'e',hx:1,hy:0,cursor:'ew-resize'},{key:'se',hx:1,hy:1,cursor:'nwse-resize'},{key:'s',hx:0,hy:1,cursor:'ns-resize'},{key:'sw',hx:-1,hy:1,cursor:'nesw-resize'},{key:'w',hx:-1,hy:0,cursor:'ew-resize'}];return isMobileDesignStudio()?handles.filter(handle=>handle.hx&&handle.hy):handles}
function designStudioRotatedLocalPoint(point,rect,rotation){const angle=-(Number(rotation)||0)*Math.PI/180,dx=point.x-rect.cx,dy=point.y-rect.cy;return{x:dx*Math.cos(angle)-dy*Math.sin(angle),y:dx*Math.sin(angle)+dy*Math.cos(angle)}}
function designStudioHitTest(point,pointerType='mouse'){
  const print=designStudioGeometry().print,selected=designStudioSelected(),touch=isMobileDesignStudio()&&pointerType==='touch';
  if(selected){
    const rect=designStudioElementRect(selected,print),local=designStudioRotatedLocalPoint(point,rect,selected.rotation),handles=designStudioHandleDefinitions();
    const distance=handle=>Math.hypot(local.x-handle.hx*rect.w/2,local.y-handle.hy*rect.h/2);
    // Keep the middle of a small selected item available for dragging.
    const inCenter=touch&&Math.abs(local.x)<rect.w*.25&&Math.abs(local.y)<rect.h*.25;
    const handle=inCenter?null:touch?handles.filter(handle=>distance(handle)<=22/designStudioState.zoom).sort((a,b)=>distance(a)-distance(b))[0]:handles.find(handle=>distance(handle)<=24);
    if(handle)return{item:selected,rect,lx:local.x,ly:local.y,handle};
  }
  for(let index=designStudioState.elements.length-1;index>=0;index--){const item=designStudioState.elements[index],rect=designStudioElementRect(item,print),local=designStudioRotatedLocalPoint(point,rect,item.rotation);if(Math.abs(local.x)<=rect.w/2+10&&Math.abs(local.y)<=rect.h/2+10)return{item,rect,lx:local.x,ly:local.y,handle:null}}
  return null;
}
function designStudioPointerDown(event){
  if(designStudioState.view!=='edit')return;event.preventDefault();const point=designStudioCanvasPoint(event),hit=designStudioHitTest(point,event.pointerType);
  ...
  designStudioRuntime.dragMode=hit.handle?'resize':'move';designStudioRuntime.resizeHandle=hit.handle?.key||'';designStudioRuntime.canvas.style.cursor=hit.handle?.cursor||'grabbing';designStudioRuntime.canvas.setPointerCapture?.(event.pointerId);renderDesignStudioInspector();renderDesignStudioLibrary();drawDesignStudio();
}
function designStudioPointerMove(event){
  const point=designStudioCanvasPoint(event);if(!designStudioRuntime.dragMode){const hover=designStudioState.view==='edit'?designStudioHitTest(point):null;designStudioRuntime.canvas.style.cursor=hover?.handle?.cursor||(hover?'grab':'default');return}event.preventDefault();const item=designStudioSelected(),start=designStudioRuntime.startElement,print=designStudioGeometry().print;if(!item||!start)return;
  ...
  else{const handle=designStudioHandleDefinitions().find(candidate=>candidate.key===designStudioRuntime.resizeHandle);if(handle){const startRect=designStudioElementRect(start,print),local=designStudioRotatedLocalPoint(point,startRect,start.rotation),startW=start.w*print.w,startH=start.h*print.h,anchorX=-handle.hx*startW/2,anchorY=-handle.hy*startH/2;let nextW=startW,nextH=startH;if(handle.hx)nextW=Math.max(print.w*.08,Math.min(print.w*1.6,handle.hx*(local.x-anchorX)));if(handle.hy)nextH=Math.max(print.h*.06,Math.min(print.h*1.6,handle.hy*(local.y-anchorY)));if(handle.hx&&handle.hy){const factor=Math.max(nextW/startW,nextH/startH);nextW=Math.max(print.w*.08,Math.min(print.w*1.6,startW*factor));nextH=Math.max(print.h*.06,Math.min(print.h*1.6,startH*factor))}const centerLocalX=handle.hx?anchorX+handle.hx*nextW/2:0,centerLocalY=handle.hy?anchorY+handle.hy*nextH/2:0,angle=(Number(start.rotation)||0)*Math.PI/180,centerX=startRect.cx+centerLocalX*Math.cos(angle)-centerLocalY*Math.sin(angle),centerY=startRect.cy+centerLocalX*Math.sin(angle)+centerLocalY*Math.cos(angle);item.w=nextW/print.w;item.h=nextH/print.h;item.x=Math.max(0,Math.min(1,(centerX-print.x)/print.w));item.y=Math.max(0,Math.min(1,(centerY-print.y)/print.h))}}
  drawDesignStudio();
}
function designStudioPointerUp(){const changed=!!designStudioRuntime.dragMode;designStudioRuntime.dragMode='';designStudioRuntime.resizeHandle='';designStudioRuntime.pointerId=null;designStudioRuntime.startPoint=null;designStudioRuntime.startElement=null;if(designStudioRuntime.canvas)designStudioRuntime.canvas.style.cursor=designStudioState.selectedId?'grab':'default';if(changed){renderDesignStudioInspector();renderDesignStudioLibrary();drawDesignStudio();scheduleDesignStudioDraftSave()}}
...
function syncDesignStudioHistoryButtons(){document.querySelectorAll('[data-studio-action="undo"]').forEach(button=>button.disabled=!designStudioState.history.length);document.querySelectorAll('[data-studio-action="redo"]').forEach(button=>button.disabled=!designStudioState.future.length)}
...
function setDesignStudioView(view){designStudioState.view=view;document.querySelectorAll('[data-studio-view]').forEach(button=>button.classList.toggle('active',button.dataset.studioView===view));const label=document.getElementById('designStudioViewLabel'),hint=document.getElementById('designStudioHint'),editReturn=document.getElementById('designStudioEditReturn');if(label)label.textContent=designStudioViewLabel();if(hint)hint.textContent=designStudioViewHint();if(editReturn)editReturn.hidden=view==='edit';drawDesignStudio();syncMobileStudioUI()}
function setDesignStudioZoom(value,automatic=false){designStudioState.zoom=Math.max(isMobileDesignStudio()?.08:.3,Math.min(isMobileDesignStudio()?2.5:1.1,(Number(value)||72)/100));if(!automatic)designStudioState.zoomAuto=false;const canvas=document.getElementById('designStudioCanvas'),label=document.getElementById('designStudioZoomValue'),input=document.querySelector('.design-studio-zoom input');if(canvas)canvas.style.width=`${Math.round(1200*designStudioState.zoom)}px`;if(label)label.textContent=`${Math.round(designStudioState.zoom*100)}%`;if(input&&automatic)input.value=Math.round(designStudioState.zoom*100);applyMobileStudioPan();if(isMobileDesignStudio())drawDesignStudio()}
function fitDesignStudioCanvas(force=false){if(fitMobileStudioCanvas(force))return;if(!force&&!designStudioState.zoomAuto)return;const scroll=document.getElementById('designStudioCanvasScroll');if(!scroll)return;const scale=Math.max(.3,Math.min(1.05,(scroll.clientWidth-52)/1200,(scroll.clientHeight-52)/900));setDesignStudioZoom(Math.round(scale*100),true)}
function refreshDesignStudioProductUI(){const product=document.getElementById('designStudioProductTop'),price=document.getElementById('designStudioPriceTop');if(product)product.textContent=designStudioProductLabel();if(price)price.textContent=isEventDesignStudio()?I18n.t('Devis personnalisé'):I18n.currency(designStudioPrice());renderDesignStudioLibrary();renderDesignStudioInspector();drawDesignStudio();scheduleDesignStudioDraftSave()}
function selectDesignStudioProduct(product){if(!designStudioProducts[product]||designStudioState.product===product)return;designStudioPushHistory();designStudioState.product=product;designStudioState.selectedId=null;refreshDesignStudioProductUI();if(isMobileDesignStudio())fitDesignStudioCanvas(true)}
...
function setDesignStudioOrientation(orientation){if(!['portrait','landscape'].includes(orientation)||designStudioState.orientation===orientation)return;designStudioPushHistory();designStudioState.orientation=orientation;refreshDesignStudioProductUI();if(isMobileDesignStudio())fitDesignStudioCanvas(true)}
function setDesignStudioQuantity(value){designStudioState.quantity=Math.max(1,Math.min(isEventDesignStudio()?1000:50,parseInt(value)||1));if(isEventDesignStudio()){eventBuilderState.customKit=eventBuilderState.customKit||{};eventBuilderState.customKit.quantity=designStudioState.quantity}else scheduleDesignStudioDraftSave()}
...
function setDesignStudioBusy(busy,message=I18n.t('Préparation de votre image...')){designStudioRuntime.busy=busy;const overlay=document.getElementById('designStudioBusy'),label=document.getElementById('designStudioBusyText');if(overlay)overlay.hidden=!busy;if(label)label.textContent=message}
function compressDesignStudioImage(file){
  return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=reject;reader.onload=()=>{const image=new Image();image.onerror=reject;image.onload=()=>{const max=2400,scale=Math.min(1,max/Math.max(image.width,image.height)),width=Math.max(1,Math.round(image.width*scale)),height=Math.max(1,Math.round(image.height*scale)),canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const context=canvas.getContext('2d');context.fillStyle='#fff';context.fillRect(0,0,width,height);context.drawImage(image,0,0,width,height);resolve({data:canvas.toDataURL('image/jpeg',.88),ratio:width/height,width,height})};image.src=String(reader.result||'')};reader.readAsDataURL(file)});
}
function designStudioTraceDimensions(ratio,max=960){const safeRatio=Math.max(.2,Number(ratio)||1);return safeRatio>=1?{width:max,height:Math.max(160,Math.round(max/safeRatio))}:{width:Math.max(160,Math.round(max*safeRatio)),height:max}}
async function processDesignStudioFiles(files){
  ...
  if(!accepted.length)return showToast(I18n.t('Choisissez une image JPG, PNG ou WEBP de moins de 10 Mo'),'error');if(!remaining)return showToast(I18n.t('Vous pouvez utiliser un maximum de six images'),'error');
  const selected=accepted.slice(0,remaining);designStudioPushHistory();setDesignStudioBusy(true,selected.length>1?I18n.t('Préparation de vos images...'):I18n.t('Préparation de votre image...'));
  try{
    for(const file of selected){const compressed=await compressDesignStudioImage(file),trace=await traceImageToLineArt(compressed.data,...Object.values(designStudioTraceDimensions(compressed.ratio)),{threshold:48,detail:1.08,transparent:true}),print=designStudioGeometry().print,w=.64,h=Math.max(.16,Math.min(.78,w*(print.w/print.h)/Math.max(.2,compressed.ratio))),item={id:`studio-image-${Date.now()}-${Math.floor(Math.random()*999999)}`,type:'image',name:file.name.replace(/\.[^.]+$/,''),source:compressed.data,sourceRatio:compressed.ratio,sourceWidth:compressed.width,sourceHeight:compressed.height,trace,x:.5,y:.5,w,h,rotation:0,opacity:100,contrast:100,brightness:100,saturation:100,traceDetail:48,fit:'contain'};designStudioState.elements.push(item);designStudioState.selectedId=item.id;designStudioHydrateElement(item)}
    designStudioState.activePanel='images';renderDesignStudioLibrary();renderDesignStudioInspector();refreshDesignStudioProductUI();scheduleDesignStudioDraftSave();showToast(I18n.msg`${selected.length} image${selected.length>1?'s':''} ajoutée${selected.length>1?'s':''}`,'success');
    if(accepted.length>remaining)showToast(I18n.t('Le studio accepte un maximum de six images'),'error');
  }catch(error){console.error(error);showToast(I18n.t('Impossible de préparer cette image'),'error')}finally{setDesignStudioBusy(false)}
}
function addDesignStudioText(value='',preset='title'){
  const input=document.getElementById('designStudioNewText'),text=String(value||input?.value||I18n.t('Votre texte')).trim();if(!text)return showToast(I18n.t('Écrivez un texte à ajouter'),'error');
...
function selectDesignStudioElement(id){designStudioState.selectedId=id;designStudioState.view='edit';document.querySelectorAll('[data-studio-view]').forEach(button=>button.classList.toggle('active',button.dataset.studioView==='edit'));const label=document.getElementById('designStudioViewLabel'),hint=document.getElementById('designStudioHint'),editReturn=document.getElementById('designStudioEditReturn');if(label)label.textContent=designStudioViewLabel();if(hint)hint.textContent=designStudioViewHint();if(editReturn)editReturn.hidden=true;renderDesignStudioLibrary();renderDesignStudioInspector();drawDesignStudio()}
function updateDesignStudioElement(key,value,record=false){
  ...
  if(key==='scale'){const next=Math.max(.08,Math.min(1.6,Number(value)/100)),ratio=next/Math.max(.01,item.w);item.w=next;item.h=Math.max(.05,Math.min(1.6,item.h*ratio))}
  ...
  if(key==='traceDetail')scheduleDesignStudioTrace(item);drawDesignStudio();scheduleDesignStudioDraftSave();
}
function scheduleDesignStudioTrace(item){
  if(item.type!=='image'||!item.source)return;designStudioRuntime.traceTimers=designStudioRuntime.traceTimers||new Map();clearTimeout(designStudioRuntime.traceTimers.get(item.id));designStudioRuntime.traceTimers.set(item.id,setTimeout(async()=>{try{const dimensions=designStudioTraceDimensions(item.sourceRatio,960);item.trace=await traceImageToLineArt(item.source,dimensions.width,dimensions.height,{threshold:Number(item.traceDetail)||48,detail:1.08,transparent:true});designStudioHydrateElement(item);drawDesignStudio();scheduleDesignStudioDraftSave()}catch{}},260));
}
...
async function exportDesignStudioProductionTrace(){
  await waitForDesignStudioImages();
  const spec=designStudioPrintSpec(),renderScale=2,full=document.createElement('canvas');
  full.width=1200*renderScale;full.height=900*renderScale;
  const fullContext=full.getContext('2d');fullContext.scale(renderScale,renderScale);renderDesignStudioScene(fullContext,'trace',false);
  const print=designStudioGeometry().print,output=document.createElement('canvas');output.width=spec.pixelWidth;output.height=spec.pixelHeight;
  const context=output.getContext('2d');context.fillStyle='#fffdf9';context.fillRect(0,0,output.width,output.height);context.drawImage(full,print.x*renderScale,print.y*renderScale,print.w*renderScale,print.h*renderScale,0,0,output.width,output.height);
...
function handleDesignStudioPrimaryAction(){return isEventDesignStudio()?completeEventStudioDesign():addDesignStudioToCart()}
async function completeEventStudioDesign(){
  if(designStudioRuntime.busy)return;
  if(!designStudioState.elements.length)return showToast(I18n.t('Ajoutez une image, un texte ou une forme à votre création'),'error');
  ...
  try{
    const [paintedPreview,tracePreview,colorArtwork]=await Promise.all([exportDesignStudioView('painted',false),exportDesignStudioView('trace',true),exportDesignStudioView('edit',true)]),isBag=designStudioState.product==='bag',sizeInfo=designStudioSizes[designStudioState.size]||designStudioSizes.moyen,imageCount=designStudioState.elements.filter(item=>item.type==='image').length,textCount=designStudioState.elements.filter(item=>item.type==='text').length;
    eventCustomSourceData=colorArtwork;eventCustomTraceData=tracePreview;
    eventStudioDesign={studioVersion:1,productType:designStudioState.product,productLabel:designStudioProductLabel(),size:isBag?'standard':designStudioState.size,sizeLabel:isBag?I18n.t('Format sac standard'):sizeInfo.label,orientation:isBag?'verticale':designStudioState.orientation==='landscape'?'horizontale':'verticale',imageCount,textCount,elementCount:designStudioState.elements.length,paintedPreview,tracePreview,colorArtwork,elements:designStudioProductionElements()};
    eventBuilderState.customKit={...(eventBuilderState.customKit||{}),size:eventStudioDesign.size,quantity:designStudioState.quantity,notes:designStudioState.notes||'',productType:eventStudioDesign.productType,productLabel:eventStudioDesign.productLabel,orientation:eventStudioDesign.orientation};
    eventStudioDraftState=cloneDesignStudioState(designStudioState);eventBuilderState.step=3;navigate('#/event-builder');setTimeout(()=>showToast(I18n.t('Création jointe à votre demande'),'success'),120);
  }catch(error){console.error(error);showToast(I18n.t('Impossible de préparer votre création'),'error')}finally{setDesignStudioBusy(false);if(button){button.disabled=false;button.querySelector('b').textContent=I18n.t('Continuer mon événement')}}
}
async function addDesignStudioToCart(goCheckout=false){
  if(isEventDesignStudio())return completeEventStudioDesign();
  ...
  try{
    const productionTracePromise=typeof document!=='undefined'&&typeof document.createElement==='function'?exportDesignStudioProductionTrace().catch(()=>null):Promise.resolve(null),[paintedPreview,tracePreview,colorArtwork,productionTrace]=await Promise.all([exportDesignStudioView('painted',false),exportDesignStudioView('trace',true),exportDesignStudioView('edit',true),productionTracePromise]),isBag=designStudioState.product==='bag',type=isBag?'custom-bag':'custom-photo',id=`${type}-${Date.now()}`,sizeInfo=designStudioSizes[designStudioState.size]||designStudioSizes.moyen,imageCount=designStudioState.elements.filter(item=>item.type==='image').length,textCount=designStudioState.elements.filter(item=>item.type==='text').length;
    cart.push({id,type,name:isBag?I18n.t('Sac personnalisé créé dans le Studio ARTY'):I18n.msg`Toile personnalisée ${sizeInfo.label} créée dans le Studio ARTY`,price:designStudioPrice(),image:paintedPreview,qty:designStudioState.quantity,customData:{kind:'design-studio',studioVersion:1,productType:designStudioState.product,productLabel:designStudioProductLabel(),size:isBag?'standard':designStudioState.size,sizeLabel:isBag?I18n.t('Format sac standard'):sizeInfo.label,orientation:isBag?'verticale':designStudioState.orientation==='landscape'?'horizontale':'verticale',notes:designStudioState.notes||'',imageCount,textCount,elementCount:designStudioState.elements.length,paintedPreview,tracePreview,productionTrace,colorArtwork,elements:designStudioProductionElements()}});
    clearDesignStudioDraft();saveCart();updateCartUI();showToast(I18n.msg`${isBag?I18n.t('Sac'):I18n.t('Toile')} personnalisé${isBag?'':'e'} ajouté${isBag?'':'e'} au panier`,'success');if(goCheckout)setTimeout(()=>goToCheckout(),250);else if(isMobileDesignStudio())openCart();
  }catch(error){console.error(error);showToast(I18n.t('Impossible de préparer votre création'),'error')}finally{setDesignStudioBusy(false);if(button){button.disabled=false;button.querySelector('b').textContent=I18n.t('Ajouter au panier')}}
}
function buyDesignStudioNow(){addDesignStudioToCart(true)}
document.addEventListener('keydown',event=>{if(!(window.location.hash||'').startsWith('#/studio')||event.target?.matches?.('input,textarea,select,[contenteditable="true"]'))return;if(event.key==='Escape'){event.preventDefault();exitDesignStudio();return}if((event.key==='Delete'||event.key==='Backspace')&&designStudioState.selectedId){event.preventDefault();removeDesignStudioElement()}if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'){event.preventDefault();event.shiftKey?designStudioRedo():designStudioUndo()}});
window.addEventListener('resize',()=>{if((window.location.hash||'').startsWith('#/studio'))requestAnimationFrame(()=>fitDesignStudioCanvas())});
