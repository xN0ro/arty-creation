...
async function handleRoute(){
  ...
  else if(h==='#/admin'){if(!currentUser||currentUser.role!=='admin'){navigate('#/');showToast(I18n.t('Accès admin requis'),'error');return}show('page-admin');document.getElementById('mainFooter').style.display='none';await loadAdminData();window.scrollTo(0,0)}
...
async function renderProfilePage(){
  ...
  const av=currentUser.picture?`<img src="${currentUser.picture}">`:`${currentUser.name.charAt(0).toUpperCase()}`;
  const badge=currentUser.role==='admin'?'<span class="profile-badge admin">Admin</span>':I18n.html('<span class="profile-badge user">Membre</span>');
