...
function openAdminEventRequest(id){
  const request=(eventRequests||[]).find(item=>String(item.id)===String(id)),modal=document.getElementById('adminEventRequestModal'),content=document.getElementById('adminEventRequestContent');if(!request||!modal||!content)return;const address=request.address||{},items=(request.inventoryItems||[]).map(item=>I18n.html`<article class="admin-event-request-product">${item.image?`<img src="${safeAttr(item.image)}" alt="">`:''}<div><strong>${safeText(item.name)}</strong><span>Quantité ${Number(item.quantity)||1}</span></div></article>`).join('');const custom=request.customKit||{},phoneHref=String(request.phone||'').replace(/[^\d+]/g,'');
