...
function renderAdminOrders(){
  const panel=document.getElementById('adminOrdersPanel');if(!panel)return;const refundRows=(adminRefunds||[]).map(refund=>`<tr><td>${safeText(refund.id)}</td><td>${safeText(refund.orderId)}</td><td>$${toMoney(refund.amount)}</td><td>${safeText(refund.reason||'')}</td><td>${safeText(refund.status||'noté')}</td></tr>`).join('');
  const rows=(adminOrders||[]).map(order=>{const customer=order.customer||{},domId=adminOrderDomId(order.id),tracking=order.tracking||{},itemText=(order.items||[]).map(item=>`${safeText(item.name)} ×${item.qty}`).join('<br>');return `<tr><td><strong>${safeText(order.id)}</strong><br><span class="admin-muted">${profileDate(order.createdAt)}</span></td><td>${safeText(customer.name||'')}<br><span class="admin-muted">${safeText(customer.email||order.guestEmail||'')}</span></td><td>${itemText}</td><td><strong>$${toMoney(order.total)}</strong></td><td><span class="admin-status ${order.paymentStatus==='paid'?'ok':order.paymentStatus==='cancelled'?'out':'pending'}">${safeText(order.paymentStatus||'pending')}</span></td><td><select class="admin-status-select" onchange="updateOrderStatus('${safeAttr(order.id)}',this.value)"><option value="en attente de paiement" ${order.status==='en attente de paiement'?'selected':''}>En attente paiement</option><option value="payée" ${order.status==='payée'?'selected':''}>Payée</option><option value="préparation" ${order.status==='préparation'?'selected':''}>Préparation</option><option value="expédiée" ${order.status==='expédiée'?'selected':''}>Expédiée</option><option value="livrée" ${order.status==='livrée'?'selected':''}>Livrée</option><option value="annulée" ${order.status==='annulée'?'selected':''}>Annulée</option><option value="remboursée" ${order.status==='remboursée'?'selected':''}>Remboursée</option></select><details class="admin-order-tracking"><summary>Livraison et suivi</summary><div><input id="trackCarrier-${domId}" value="${safeAttr(tracking.carrier||'')}" placeholder="Transporteur"><input id="trackNumber-${domId}" value="${safeAttr(tracking.number||'')}" placeholder="Numéro de suivi"><input id="trackDate-${domId}" type="date" value="${safeAttr(tracking.estimatedDelivery||'')}"><input id="trackUrl-${domId}" value="${safeAttr(tracking.url||'')}" placeholder="Lien de suivi https://..."><button class="admin-btn admin-btn-edit" onclick="saveOrderTracking('${safeAttr(order.id)}','${safeAttr(domId)}')">Enregistrer le suivi</button></div></details><div class="admin-actions"><button class="admin-btn admin-btn-edit" onclick="createRefund('${safeAttr(order.id)}')">Rembourser</button></div></td></tr>`}).join('');
  panel.innerHTML=`<div class="admin-form-card"><h3>Commandes et livraisons</h3><p class="admin-help">Mettez à jour le statut, le transporteur et le numéro de suivi visible dans le compte client.</p></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Commande</th><th>Client</th><th>Articles</th><th>Total</th><th>Paiement</th><th>Statut et suivi</th></tr></thead><tbody>${rows||'<tr><td colspan="6" class="admin-muted">Aucune commande pour le moment.</td></tr>'}</tbody></table></div><div class="admin-section-title"><h3>Historique des remboursements</h3></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Remboursement</th><th>Commande</th><th>Montant</th><th>Raison</th><th>Statut</th></tr></thead><tbody>${refundRows||'<tr><td colspan="5" class="admin-muted">Aucun remboursement.</td></tr>'}</tbody></table></div>`;
}
