...
function eventQuotePriceBreakdownHTML(quote){
  ...
  return `<div class="event-quote-price-breakdown"><div><span>${I18n.t('Sous-total')}</span><strong>${I18n.currency(toMoney(quote.quoteSubtotal??quote.quoteAmount))}</strong></div><div><span>${I18n.t('Livraison / déplacement')}</span><strong>${I18n.currency(toMoney(quote.quoteShipping||0))}</strong></div>${taxLines}<div class="total"><span>${I18n.t('Total')}</span><strong>${I18n.currency(toMoney(quote.quoteAmount))} CAD</strong></div></div>`;
}
function renderEventQuotePaymentContent(){
  ...
  if(quote.paymentStatus==='paid'){container.innerHTML=I18n.html`<div class="event-payment-success"><span>${eventExperienceIcon('check')}</span><div class="section-tag">Paiement confirmé</div><h1>Merci, votre événement est confirmé.</h1><p>Nous avons reçu votre paiement de <strong>${I18n.currency(toMoney(quote.quoteAmount))} CAD</strong> pour la demande <strong>${safeText(quote.reference)}</strong>.</p>${eventQuotePriceBreakdownHTML(quote)}<div>Notre équipe communiquera avec vous pour finaliser la production et les détails de l’événement.</div><a href="#/profile" class="btn btn-orange">Voir dans mon compte</a></div>`;return}
