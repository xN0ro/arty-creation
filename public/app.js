...
async function createRefund(orderId){const o=adminOrders.find(x=>String(x.id)===String(orderId));if(!o)return;const remaining=Math.max(0,Number(o.total||0)-Number(o.refundedTotal||0));const amount=prompt(`Montant à rembourser (max ${I18n.currency(toMoney(remaining))})`,toMoney(remaining));if(amount===null)return;const reason=prompt(I18n.t('Raison du remboursement'),I18n.t('Demande client'))||I18n.t('Remboursement');const restock=confirm(I18n.t('Remettre les produits en inventaire?'));try{const r=await artyFetch(`/api/admin/orders/${encodeURIComponent(orderId)}/refund`,{method:'POST',headers:authH(),body:JSON.stringify({amount,reason,restock})});const d=await r.json().catch(()=>({}));if(!r.ok)return showToast(d.error||I18n.t('Erreur'),'error');showToast(I18n.t('Remboursement enregistré'),'success');await loadAdminData()}catch{showToast(I18n.t('Erreur'),'error')}}
...
function adminPaymentMeta(status){const key=String(status||'pending').toLowerCase(),map={paid:[I18n.t('Paiement confirmé'),'ok'],pending:[I18n.t('En attente'),'pending'],cancelled:[I18n.t('Annulé'),'out'],refund_needed:[I18n.t('Remboursement requis'),'pending'],refunded:[I18n.t('Remboursé'),'out'],provider_not_connected:[I18n.t('Paiement à confirmer'),'pending']};const value=map[key]||[status||I18n.t('À confirmer'),'pending'];return{label:value[0],className:value[1]}}
...
function renderAdminOrders(){
  ...
  const refundRows=(adminRefunds||[]).map(refund=>`<tr><td>${safeText(refund.id)}</td><td>${safeText(refund.orderId)}</td><td>${I18n.currency(toMoney(refund.amount))}</td><td>${safeText(refund.reason||'')}</td><td>${safeText(I18n.status(refund.status||'noté'))}</td></tr>`).join('');
