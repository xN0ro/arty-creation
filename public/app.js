...
function renderProfilePrivateEvents(){
  ...
  wrap.innerHTML=profilePrivateEvents.map(request=>{
    ...
    return I18n.html`<article class="account-order-card account-private-event-row"><div class="account-order-card-main"><img src="${safeAttr(preview)}" alt=""><div class="account-order-copy"><div class="account-order-topline"><span>${safeText(request.reference||I18n.t('Événement privé'))}</span><span class="account-status ${paymentMeta.className}">${safeText(paymentMeta.label)}</span></div><h3>${safeText(request.eventType||I18n.t('Événement privé'))}</h3><p>${safeText(request.preferredDate||I18n.t('Date à confirmer'))} · ${Number(request.guests)||1} personne${Number(request.guests)>1?'s':''} · ${safeText(profilePrivateEventPathLabel(request.servicePath))}</p></div><div class="account-order-actions">${Number(request.quoteAmount)>0?`<strong>${I18n.currency(toMoney(request.quoteAmount))}</strong>`:''}<button type="button" onclick="viewProfilePrivateEvent('${safeAttr(request.id)}')">${safeText(I18n.t('Voir les détails'))}</button></div></div></article>`;
  }).join('');
}
...
function mountAdminEventQuoteWorkspace(panel){
  ...
  const cards=(eventRequests||[]).map(request=>I18n.html`<article class="admin-event-quote-card" onclick="openAdminEventRequest(${Number(request.id)})"><div class="admin-event-quote-top"><span>${safeText(request.reference||`EVT-${request.id}`)}</span><b class="admin-status-badge">${safeText(I18n.status(request.status||'nouvelle'))}</b></div><h4>${safeText(request.eventType||I18n.t('Événement à préciser'))}</h4><p>${safeText(request.name||'')} · ${Number(request.guests)||'?'} personne(s)</p><div><span>${safeText(adminEventPathLabel(request.servicePath))}</span><strong>${request.quoteAmount?`${I18n.currency(toMoney(request.quoteAmount))}`:I18n.t('Prix à préparer')}</strong></div><button type="button">Ouvrir la demande</button></article>`).join('');
