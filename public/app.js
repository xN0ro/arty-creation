...
async function renderProductPage(id,{force=false}={}){
  ...
  const regular=Number(kit.originalPrice??kit.price)||0,price=getKitDisplayPrice(kit),inStock=kit.inStock!==false;
  const includedCard=included.length?I18n.html`<section class="product-detail-card product-included-card"><div class="product-detail-icon" aria-hidden="true">✓</div><div><span class="product-detail-kicker">Kit complet</span><h2>Inclus dans ce kit</h2><ul>${included.map(item=>`<li>${safeText(item)}</li>`).join('')}</ul></div></section>`:'';
  c.innerHTML=I18n.html`
    ...
    <div class="product-layout product-layout-pro">
      ...
      <section class="product-info product-info-pro">
        <div class="product-title-row"><div><div class="product-cat">${safeText(cat?cat.name:I18n.t('Kit ARTY'))}</div><h1>${safeText(I18n.field(kit,'name'))}</h1></div>${kit.featured?I18n.html('<span class="product-popular-mark">Populaire</span>'):''}</div>
        <p class="product-desc">${safeText(kit.description||kit.shortDesc||'')}</p>
        ${!inStock?I18n.html('<div class="product-tags"><span class="product-tag low-stock-tag">Épuisé</span></div>'):''}
        <div class="product-purchase-card">
          <div class="product-price-line"><div><span>Votre prix</span><div class="product-configured-price"><strong id="productConfiguredPrice">${I18n.currency(toMoney(price))}</strong><small id="productConfiguredCompare" style="${regular>price?'':'display:none'}">${I18n.currency(toMoney(regular))}</small></div>${kit.discountLabel?`<em>${safeText(kit.discountLabel)}</em>`:''}</div><div class="product-selection-total"><span>Sélection</span><strong id="productSelectionSummary">${sizes[0]?safeText(sizes[0].label):I18n.t('Configuration standard')}</strong></div></div>
          ${sizeOptions}${addOnOptions}
          <div class="product-buy-row"><div class="product-qty-row"><label>Quantité</label><div class="qty-ctrl"><button class="qty-btn" onclick="chgQty(-1)" aria-label="Réduire la quantité">−</button><input class="qty-val" id="pQty" value="1" readonly aria-label="Quantité"><button class="qty-btn" onclick="chgQty(1)" aria-label="Augmenter la quantité">+</button></div></div><button class="btn btn-orange product-add-button" onclick="addToCart(${kit.id})" ${!inStock?'disabled':''}>${inStock?I18n.t('Ajouter au panier'):I18n.t('Produit épuisé')}</button></div>
