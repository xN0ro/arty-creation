...
function adminDesignStudioAssets(item){
  ...
  return `<div class="admin-order-design-previews">${custom.paintedPreview?I18n.html`<a href="${safeAttr(custom.paintedPreview)}" download="apercu-peint-arty.jpg"><img src="${safeAttr(custom.paintedPreview)}" alt="Aperçu peint"><span>Télécharger l’aperçu peint</span></a>`:''}${custom.tracePreview?I18n.html`<a href="${safeAttr(custom.tracePreview)}" download="trace-production-arty.png"><img src="${safeAttr(custom.tracePreview)}" alt="Tracé de production"><span>Télécharger le tracé de production</span></a>`:''}${custom.colorArtwork?I18n.html`<a href="${safeAttr(custom.colorArtwork)}" download="reference-couleur-arty.jpg"><img src="${safeAttr(custom.colorArtwork)}" alt="Référence couleur"><span>Télécharger la référence couleur</span></a>`:''}</div>`;
}
function renderAdminOrders(){
  const panel=document.getElementById('adminOrdersPanel');if(!panel)return;
...
function prepareDesignStudioRoute(hash){
  ...
  if(requested===designStudioContext)return;
  if(requested==='event'){
  ...
  }else designStudioState=designStudioInitialState();
...
function designStudioOrderCard(){const eventMode=isEventDesignStudio();return I18n.html`<div class="studio-order-card ${eventMode?'event-mode':''}"><div><span>${eventMode?I18n.t('Projet de l’événement'):I18n.t('Votre produit')}</span><strong>${safeText(designStudioProductLabel())}</strong></div><label>${eventMode?I18n.t('Nombre de kits identiques'):I18n.t('Quantité')}<input type="number" min="1" max="${eventMode?'1000':'50'}" value="${designStudioState.quantity}" onchange="setDesignStudioQuantity(this.value)"></label><label>Note pour la production<textarea maxlength="400" placeholder="Une précision pour notre équipe..." oninput="designStudioState.notes=this.value">${safeText(designStudioState.notes||'')}</textarea></label><div class="studio-order-total"><span>${eventMode?I18n.t('Tarification'):I18n.t('Prix unitaire')}</span><strong>${eventMode?I18n.t('Incluse au devis'):`${I18n.currency(toMoney(designStudioPrice()))}`}</strong></div><button type="button" onclick="handleDesignStudioPrimaryAction()">${designStudioIcon(eventMode?'check':'cart')}${eventMode?I18n.t('Continuer mon événement'):I18n.t('Ajouter au panier')}</button></div>`}
...
function setDesignStudioQuantity(value){designStudioState.quantity=Math.max(1,Math.min(isEventDesignStudio()?1000:50,parseInt(value)||1));if(isEventDesignStudio()){eventBuilderState.customKit=eventBuilderState.customKit||{};eventBuilderState.customKit.quantity=designStudioState.quantity}}
...
async function processDesignStudioFiles(files){
  ...
  try{
    ...
    designStudioState.activePanel='images';renderDesignStudioLibrary();renderDesignStudioInspector();refreshDesignStudioProductUI();showToast(I18n.msg`${selected.length} image${selected.length>1?'s':''} ajoutée${selected.length>1?'s':''}`,'success');
...
async function addDesignStudioToCart(goCheckout=false){
  ...
  try{
    ...
    saveCart();updateCartUI();showToast(I18n.msg`${isBag?I18n.t('Sac'):I18n.t('Toile')} personnalisé${isBag?'':'e'} ajouté${isBag?'':'e'} au panier`,'success');if(goCheckout)setTimeout(()=>goToCheckout(),250);else if(isMobileDesignStudio())openCart();
