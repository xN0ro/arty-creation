...
function ensureProductDock(){
  ...
  if(!onProduct){
    ...
    document.body.classList.remove('arty-mobile-product-ready');
    return;
  }
...
function setCheckoutAutofill(){
  var attrs={
    coName:{autocomplete:'name'},
    coEmail:{autocomplete:'email',inputmode:'email'},
    coPhone:{autocomplete:'tel',inputmode:'tel'},
    coAddress:{autocomplete:'street-address'},
    coCity:{autocomplete:'address-level2'},
    coProvince:{autocomplete:'address-level1'},
    coPostal:{autocomplete:'postal-code'},
...
function ensureCheckoutDock(){
  ...
  if(!onCheckout){
    ...
    document.body.classList.remove('arty-mobile-checkout-ready');
    return;
  }
  ...
  document.body.classList.add('arty-mobile-checkout-ready');
  dock.hidden=hiddenByOverlay();
}
...
function sync(){
  ...
  requestAnimationFrame(function(){
    ...
    if(!isMobile()){
      removeDock('.arty-mobile-product-dock');
      removeDock('.arty-mobile-checkout-dock');
      document.body.classList.remove('arty-mobile-product-ready','arty-mobile-checkout-ready');
      return;
    }
    ensureProductDock();
    enhanceCart();
...
function setInputFocus(active){
  document.body.classList.toggle('arty-mobile-input-focus',!!active);
  sync();
}

document.addEventListener('focusin',function(e){
  if(e.target&&e.target.matches('input,textarea,select'))setInputFocus(true);
});
document.addEventListener('focusout',function(){
  setTimeout(function(){
    var active=document.activeElement;
    setInputFocus(!!active&&active.matches&&active.matches('input,textarea,select'));
  },0);
});
