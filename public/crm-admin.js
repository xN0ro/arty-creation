...
async function sendQuoteAction(){
  ...
  try{
    ...
    const result=await api('/api/admin/event-requests/'+encodeURIComponent(l.id)+'/payment-link',{method:'POST',body:JSON.stringify({quoteSubtotal:subtotal,quoteShipping:shipping,quoteAddress:address,quoteDescription})});
...
async function dropStage(event,status){
  ...
  if(['won','refunded','lost'].includes(status)){openLeadEditor(lead.kind,encodeURIComponent(lead.id),status);return}
