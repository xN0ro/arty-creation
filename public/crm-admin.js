...
function renderCustomerDetail(){
  ...
  host.innerHTML=`
    ...
    ${a?`<section class="crm-detail-section"><div class="crm-subhead"><h4>${T('Gestion du compte','Account management')}</h4><small>${T('Dernière connexion','Last login')}: ${dateTime(a.lastLoginAt)}</small></div><div class="crm-account-form"><label>${T('Nom','Name')}<input id="crmAccountName" value="${attr(a.name||'')}"></label><label>${T('Téléphone','Phone')}<input id="crmAccountPhone" value="${attr(a.phone||'')}"></label></div><div class="crm-inline-actions"><button class="btn btn-orange btn-sm" onclick="ARTYCRM.saveAccount()">${T('Enregistrer','Save')}</button><button class="btn btn-ghost btn-sm" onclick="ARTYCRM.sendPasswordReset()">${T('Envoyer réinitialisation','Send password reset')}</button><button class="btn btn-ghost btn-sm" onclick="ARTYCRM.resendWelcome()">${T('Renvoyer bienvenue','Resend welcome')}</button><button class="btn ${a.accountDisabledAt?'btn-teal':'btn-ghost'} btn-sm" onclick="ARTYCRM.toggleAccount(${a.accountDisabledAt?'false':'true'})">${a.accountDisabledAt?T('Réactiver','Reactivate'):T('Désactiver','Disable')}</button></div></section>`:''}
